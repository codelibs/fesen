...
public class ClientYamlSuiteRestApiParserFailingTests extends ESTestCase {
    ...
    public void testDuplicateMethods() throws Exception {
        parseAndExpectParsingException("{\n" + "  \"ping\": {" + "    \"documentation\": \"http://www.elasticsearch.org/guide/\","
                + "    \"stability\": \"stable\",\n" + "    \"url\": {"
                + "      \"paths\": [{\"path\":\"/\", \"parts\": {}, \"methods\": [\"PUT\", \"PUT\"]}]," + "      \"params\": {"
                + "        \"type\" : \"boolean\",\n"
                + "        \"description\" : \"Whether specified concrete indices should be ignored when unavailable (missing or closed)\""
                + "      }" + "    }," + "    \"body\": null" + "  }" + "}", "ping.json", "ping API: found duplicate method [PUT]");
    }
    ...
    public void testDuplicatePaths() throws Exception {
        parseAndExpectIllegalArgumentException("{\n" + "  \"ping\": {" + "    \"documentation\": \"http://www.elasticsearch.org/guide/\","
                ...
                + "      \"params\": {" + "        \"type\" : \"boolean\",\n"
                + "        \"description\" : \"Whether specified concrete indices should be ignored when unavailable (missing or closed)\""
                + "      }" + "    }," + "    \"body\": null" + "  }" + "}", "ping.json", "ping API: found duplicate path [/pingtwo]");
    }
    ...
    public void testSpecNameMatchesFilename() throws Exception {
        parseAndExpectIllegalArgumentException("{\"ping\":{}}", "not_matching.json",
    ...
    private static final String BROKEN_SPEC_PARAMS = "{\n" + "  \"ping\": {"
            + "    \"documentation\": \"http://www.elasticsearch.org/guide/\"," + "    \"stability\": \"stable\",\n" + "    \"url\": {"
            + "      \"paths\": [{\"path\": \"path\", \"methods\": [\"HEAD\"]}]" + "    }," + "    \"params\": {"
            + "      \"type\" : \"boolean\",\n"
            + "      \"description\" : \"Whether specified concrete indices should be ignored when unavailable (missing or closed)\"\n"
            + "    }," + "    \"body\": null" + "  }" + "}";
