...
public class MockFSDirectoryFactory implements IndexStorePlugin.DirectoryFactory {
        ...
        if (store.tryIncRef()) {
            ...
            try {
                ...
                try {
                    ...
                    out.flush();
                    if (!status.clean) {
                        IOException failure = new IOException("failed to check index for shard " + shardId + ";index files ["
                                + Arrays.toString(dir.listAll()) + "] os [" + os.bytes().utf8ToString() + "]");
    ...
    private Directory randomDirectoryService(Random random, IndexSettings indexSettings, ShardPath path) throws IOException {
        final IndexMetadata build = IndexMetadata.builder(indexSettings.getIndexMetadata()).settings(Settings.builder()
                ...
                .put(indexSettings.getIndexMetadata().getSettings()).put(IndexModule.INDEX_STORE_TYPE_SETTING.getKey(),
