                    ...
                    try {
                        ...
                        while (!stop.get()) {
                            if (batch) {
                                ...
                                if (useAutoGeneratedIDs) {
                                    try {
                                        IndexResponse indexResponse = client.prepareIndex(index, type).setTimeout(timeout)
                        ...
                        trackFailure(e);
                        final long docId = id;
                        logger.warn((Supplier<?>) () -> new ParameterizedMessage("**** failed indexing thread {} on doc id {}", indexerId,
                                docId), e);
                    } finally {
                        stopLatch.countDown();
                    }
