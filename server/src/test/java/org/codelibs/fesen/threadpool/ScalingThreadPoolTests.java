...
public class ScalingThreadPoolTests extends ESThreadPoolTestCase {
    ...
    public void testScalingThreadPoolConfiguration() throws InterruptedException {
        ...
        if (processorsUsed > availableProcessors) {
            assertWarnings("setting [node.processors] to value [" + processorsUsed + "] which is more than available processors ["
                    + availableProcessors + "] is deprecated");
        }
    }
    ...
    public void testScalingThreadPoolThreadsAreTerminatedAfterKeepAlive() throws InterruptedException {
        ...
        final Settings settings = Settings.builder().put("thread_pool." + threadPoolName + ".max", 128)
                .put("thread_pool." + threadPoolName + ".keep_alive", "1ms").build();
        runScalingThreadPoolTest(settings, ((clusterSettings, threadPool) -> {
            final CountDownLatch latch = new CountDownLatch(1);
