...
public class IndexStorePluginTests extends ESTestCase {
    ...
    public void testIndexStoreFactoryConflictsWithBuiltInIndexStoreType() {
        ...
        assertThat(e, hasToString(
                containsString("registered index store type [" + ConflictingStorePlugin.TYPE + "] conflicts with a built-in type")));
    }
    ...
    public void testDuplicateIndexStoreFactories() {
        ...
        if (JavaVersion.current().compareTo(JavaVersion.parse("9")) >= 0) {
            assertThat(e,
                    hasToString(matches("java.lang.IllegalStateException: Duplicate key store \\(attempted merging values "
                            + "org.codelibs.fesen.index.store.FsDirectoryFactory@[\\w\\d]+ "
                            + "and org.codelibs.fesen.index.store.FsDirectoryFactory@[\\w\\d]+\\)")));
        } else {
            assertThat(e, hasToString(
                    matches("java.lang.IllegalStateException: Duplicate key org.codelibs.fesen.index.store.FsDirectoryFactory@[\\w\\d]+")));
        }
    }
    ...
    public void testDuplicateIndexStoreRecoveryStateFactories() {
        ...
        if (JavaVersion.current().compareTo(JavaVersion.parse("9")) >= 0) {
            ...
            assertThat(e, hasToString(matches("java.lang.IllegalStateException: Duplicate key "
