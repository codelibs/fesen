...
public class HttpInfoTests extends ESTestCase {
    ...
    public void testCorrectlyDisplayPublishedCname() throws Exception {
        ...
        int port = 9200;
        assertPublishAddress(new HttpInfo(new BoundTransportAddress(new TransportAddress[] { new TransportAddress(localhost, port) },
                new TransportAddress(localhost, port)), 0L), "localhost/" + NetworkAddress.format(localhost) + ':' + port);
    }
    ...
    public void testCorrectDisplayPublishedIp() throws Exception {
        ...
        int port = 9200;
        assertPublishAddress(new HttpInfo(new BoundTransportAddress(new TransportAddress[] { new TransportAddress(localhost, port) },
                new TransportAddress(localhost, port)), 0L), NetworkAddress.format(localhost) + ':' + port);
    }
    ...
    public void testCorrectDisplayPublishedIpv6() throws Exception {
        ...
        assertPublishAddress(new HttpInfo(new BoundTransportAddress(new TransportAddress[] { localhost }, localhost), 0L),
                localhost.toString());
    }
