 ...
 * under the License.
 */
package org.codelibs.fesen.indices.analyze;
...
import static org.codelibs.fesen.indices.analysis.HunspellService.HUNSPELL_IGNORE_CASE;
import static org.codelibs.fesen.indices.analysis.HunspellService.HUNSPELL_LAZY_LOAD;
...
public class HunspellServiceTests extends ESTestCase {
    public void testLocaleDirectoryWithNodeLevelConfig() throws Exception {
        Settings settings = Settings.builder().put(HUNSPELL_LAZY_LOAD.getKey(), randomBoolean()).put(HUNSPELL_IGNORE_CASE.getKey(), true)
    ...
    public void testDicWithNoAff() throws Exception {
        Settings settings = Settings.builder().put(HUNSPELL_LAZY_LOAD.getKey(), randomBoolean())
                .put(Environment.PATH_HOME_SETTING.getKey(), createTempDir()).build();
