...
import org.codelibs.fesen.indices.cluster.IndicesClusterStateService.AllocatedIndex;
import org.codelibs.fesen.indices.cluster.IndicesClusterStateService.AllocatedIndices;
import org.codelibs.fesen.indices.cluster.IndicesClusterStateService.Shard;
...
public abstract class AbstractIndicesClusterStateServiceTestCase extends ESTestCase {
                    ...
                    if (shard != null) {
                        ...
                        if (shard.routingEntry().primary() && shard.routingEntry().active()) {
                            ...
                            Set<String> inSyncIds =
                                    state.metadata().index(shard.shardId().getIndex()).inSyncAllocationIds(shard.shardId().id());
    ...
    protected class MockIndicesService implements AllocatedIndices<MockIndexShard, MockIndexService> {
        ...
        @Override
        public synchronized MockIndexService createIndex(IndexMetadata indexMetadata, List<IndexEventListener> buildInIndexListener,
        ...
        @Override
        public MockIndexShard createShard(final ShardRouting shardRouting, final PeerRecoveryTargetService recoveryTargetService,
    ...
    protected class MockIndexShard implements IndicesClusterStateService.Shard {
        ...
        @Override
        public void updateShardState(ShardRouting shardRouting, long newPrimaryTerm,
