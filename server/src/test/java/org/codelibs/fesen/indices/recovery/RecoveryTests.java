...
import org.codelibs.fesen.index.replication.RecoveryDuringReplicationTests;
...
public class RecoveryTests extends ESIndexLevelReplicationTestCase {
    ...
    public void testRetentionPolicyChangeDuringRecovery() throws Exception {
        try (ReplicationGroup shards = createGroup(0)) {
            ...
            Future<Void> future = shards.asyncRecoverReplica(replica,
                    (indexShard, node) -> new RecoveryDuringReplicationTests.BlockingTarget(RecoveryState.Stage.TRANSLOG, recoveryBlocked,
                            releaseRecovery, indexShard, node, recoveryListener, logger));
    ...
    public void testRecoveryWithOutOfOrderDeleteWithSoftDeletes() throws Exception {
        Settings settings = Settings.builder().put(IndexSettings.INDEX_SOFT_DELETES_SETTING.getKey(), true)
    ...
    public void testPeerRecoverySendSafeCommitInFileBased() throws Exception {
        ...
        for (int i = 0; i < numDocs; i++) {
            Engine.IndexResult result = primaryShard.applyIndexOperationOnPrimary(
                    Versions.MATCH_ANY, VersionType.INTERNAL, new SourceToParse(primaryShard.shardId().getIndexName(), "_doc",
                            Integer.toString(i), new BytesArray("{}"), XContentType.JSON),
        ...
        try (ReplicationGroup shards = createGroup(1)) {
            ...
            try (Translog.Snapshot snapshot = getTranslog(newReplica).newSnapshot()) {
                if (newReplica.indexSettings().isSoftDeleteEnabled()) {
                    ...
                    assertThat("Sequence based recovery should keep existing translog", snapshot,
