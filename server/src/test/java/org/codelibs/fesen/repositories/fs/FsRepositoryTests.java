...
public class FsRepositoryTests extends ESTestCase {
    ...
    public void testSnapshotAndRestore() throws IOException, InterruptedException {
        ...
        try (Directory directory = newDirectory()) {
            ...
            Settings settings = Settings.builder().put(Environment.PATH_HOME_SETTING.getKey(), createTempDir().toAbsolutePath())
                    ...
                    .putList(Environment.PATH_DATA_SETTING.getKey(), tmpPaths()).put("location", repo).put("compress", randomBoolean())
            ...
            ShardRouting routing = ShardRouting.newUnassigned(shardId, true,
            ...
            runGeneric(threadPool, () -> {
                ...
                repository.snapshotShard(store, null, incSnapshotId, indexId, incIndexCommit, null, snapshotStatus, Version.CURRENT,
            ...
            futureB.actionGet();
            assertEquals("should reuse everything except of .liv and .si", commitFileNames.size() - 2,
    ...
    private void deleteRandomDoc(Directory directory) throws IOException {
        try (IndexWriter writer = new IndexWriter(directory, newIndexWriterConfig(random(), new MockAnalyzer(random()))
                .setCodec(TestUtil.getDefaultCodec()).setMergePolicy(new FilterMergePolicy(NoMergePolicy.INSTANCE) {
                    @Override
