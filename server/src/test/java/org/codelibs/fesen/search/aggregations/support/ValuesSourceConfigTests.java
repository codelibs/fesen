...
public class ValuesSourceConfigTests extends ESSingleNodeTestCase {
    ...
    public void testKeyword() throws Exception {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, null, "bytes", null, null, null, null, CoreValuesSourceType.BYTES);
    ...
    public void testUnmappedKeyword() throws Exception {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            QueryShardContext context = indexService.newQueryShardContext(0, searcher, () -> 42L, null);
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, ValueType.STRING, "bytes", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Bytes valuesSource = (ValuesSource.Bytes) config.getValuesSource();
    ...
    public void testLong() throws Exception {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, null, "long", null, null, null, null, CoreValuesSourceType.BYTES);
    ...
    public void testUnmappedLong() throws Exception {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, ValueType.NUMBER, "long", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Numeric valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            assertNotNull(valuesSource);
            assertFalse(config.hasValues());
    ...
    public void testBoolean() throws Exception {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, null, "bool", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Numeric valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            LeafReaderContext ctx = searcher.getIndexReader().leaves().get(0);
            SortedNumericDocValues values = valuesSource.longValues(ctx);
            assertTrue(values.advanceExact(0));
    ...
    public void testUnmappedBoolean() throws Exception {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, ValueType.BOOLEAN, "bool", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Numeric valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            assertNotNull(valuesSource);
            assertFalse(config.hasValues());
    ...
    public void testFieldAlias() throws Exception {
        ...
        client().prepareIndex("index", "type", "1").setSource("field", "value").setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE)
                .get();

        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            QueryShardContext context = indexService.newQueryShardContext(0, searcher, () -> 42L, null);
            ValuesSourceConfig config =
                    ValuesSourceConfig.resolve(context, ValueType.STRING, "alias", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Bytes valuesSource = (ValuesSource.Bytes) config.getValuesSource();
