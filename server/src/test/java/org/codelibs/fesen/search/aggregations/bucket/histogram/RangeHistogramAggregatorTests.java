...
public class RangeHistogramAggregatorTests extends AggregatorTestCase {
    ...
    public void testLongs() throws Exception {
        ...
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            for (RangeFieldMapper.Range range : new RangeFieldMapper.Range[] { new RangeFieldMapper.Range(rangeType, 1L, 5L, true, true), // bucket 0 5
    ...
    public void testIpRangesUnsupported() throws Exception {
        ...
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            Document doc = new Document();
            BytesRef encodedRange = rangeType.encodeRanges(Collections.singleton(new RangeFieldMapper.Range(rangeType,
                    InetAddresses.forString("10.0.0.1"), InetAddresses.forString("10.0.0.10"), true, true)));
            doc.add(new BinaryDocValuesField("field", encodedRange));
            w.addDocument(doc);
    ...
    public void testAsSubAgg() throws IOException {
        ...
        CheckedConsumer<RandomIndexWriter, IOException> buildIndex = iw -> {
            ...
            for (int n = 0; n < 10000; n++) {
                BytesRef outerRange = RangeType.LONG.encodeRanges(
                        org.codelibs.fesen.core.Set.of(new RangeFieldMapper.Range(RangeType.LONG, n % 100, n % 100 + 10, true, true)));
                BytesRef innerRange = RangeType.LONG.encodeRanges(
                        org.codelibs.fesen.core.Set.of(new RangeFieldMapper.Range(RangeType.LONG, n / 100, n / 100 + 10, true, true)));

                docs.add(org.codelibs.fesen.core.List.of(new BinaryDocValuesField("outer", outerRange),
                        new BinaryDocValuesField("inner", innerRange), new SortedNumericDocValuesField("n", n)));
            }
        ...
        testCase(request, new MatchAllDocsQuery(), buildIndex, verify, rangeField("outer", RangeType.LONG),
                rangeField("inner", RangeType.LONG), longField("n"));
    }
}
