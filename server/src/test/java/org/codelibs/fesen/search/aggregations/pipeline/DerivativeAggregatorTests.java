...
import org.codelibs.fesen.search.aggregations.pipeline.BucketHelpers;
import org.codelibs.fesen.search.aggregations.pipeline.Derivative;
...
import org.codelibs.fesen.search.aggregations.pipeline.SimpleValue;
...
public class DerivativeAggregatorTests extends AggregatorTestCase {
    ...
    public void testDocCountDerivative() throws IOException {
        ...
        HistogramAggregationBuilder aggBuilder =
                new HistogramAggregationBuilder("histo").field(SINGLE_VALUED_FIELD_NAME).interval(interval);
    ...
    public void testSingleValuedField_normalised() throws IOException {
        ...
        Query query = new MatchAllDocsQuery();
        HistogramAggregationBuilder aggBuilder =
                new HistogramAggregationBuilder("histo").field(SINGLE_VALUED_FIELD_NAME).interval(interval).minDocCount(0);
            ...
            for (int i = 0; i < numValueBuckets; ++i) {
                ...
                if (i > 0) {
                    ...
                    assertThat(((InternalMultiBucketAggregation.InternalBucket) bucket).getProperty("histo",
                            AggregationPath.parse("deriv.value").getPathElementsAsStringList()), equalTo((double) sumDerivValue));
                } else {
    ...
    public void testSingleValueAggDerivative_invalidPath() throws IOException {
        try {
            Query query = new MatchAllDocsQuery();
            HistogramAggregationBuilder aggBuilder = new HistogramAggregationBuilder("histo").field(SINGLE_VALUED_FIELD_NAME).interval(1);
            aggBuilder.subAggregation(new FiltersAggregationBuilder("filters", QueryBuilders.termQuery("tag", "foo"))
