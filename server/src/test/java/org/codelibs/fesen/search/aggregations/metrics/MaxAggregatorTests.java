...
import org.codelibs.fesen.search.aggregations.bucket.global.GlobalAggregator;
...
import org.codelibs.fesen.search.aggregations.metrics.MaxAggregator;
import org.codelibs.fesen.search.aggregations.metrics.ValueCountAggregationBuilder;
import org.codelibs.fesen.search.aggregations.metrics.ValueCountAggregator;
...
public class MaxAggregatorTests extends AggregatorTestCase {
    ...
    private void testAggregation(AggregationBuilder aggregationBuilder, Query query,
            CheckedConsumer<RandomIndexWriter, IOException> buildIndex, Consumer<InternalMax> verify, MappedFieldType fieldType)
            throws IOException {
        testCase(aggregationBuilder, query, buildIndex, verify, fieldType);
    }

    public void testMaxShortcutRandom() throws Exception {
        testMaxShortcutCase(() -> randomLongBetween(Integer.MIN_VALUE, Integer.MAX_VALUE), (n) -> new LongPoint("number", n.longValue()),
        ...
        testMaxShortcutCase(() -> randomDouble(), (n) -> new DoublePoint("number", n.doubleValue()),
                (v) -> DoublePoint.decodeDimension(v, 0));
    }

    private void testMaxShortcutCase(Supplier<Number> randomNumber, Function<Number, Field> pointFieldFunc,
    ...
    public void testSingleValuedFieldGetProperty() throws IOException {
        ...
        AggregationBuilder aggregationBuilder =
                AggregationBuilders.global("global").subAggregation(AggregationBuilders.max("max").field("value"));
    ...
    public void testOrderByEmptyAggregation() throws IOException {
        ...
        TermsAggregationBuilder aggregationBuilder = new TermsAggregationBuilder("terms").userValueTypeHint(ValueType.NUMERIC)
                .field("value").order(BucketOrder.compound(BucketOrder.aggregation("filter>max", true))).subAggregation(AggregationBuilders
                        .filter("filter", termQuery("value", 100)).subAggregation(AggregationBuilders.max("max").field("value")));
