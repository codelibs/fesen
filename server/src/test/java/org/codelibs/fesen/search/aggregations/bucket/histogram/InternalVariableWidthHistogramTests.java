...
import org.codelibs.fesen.search.aggregations.bucket.histogram.ParsedVariableWidthHistogram;
...
public class InternalVariableWidthHistogramTests extends InternalMultiBucketAggregationTestCase<InternalVariableWidthHistogram> {
    ...
    @Override
    protected InternalVariableWidthHistogram createTestInstance(String name, Map<String, Object> metaData,
        ...
        for (int i = 0; i < numBuckets; ++i) {
            ...
            curKey += add;
            buckets.add(new InternalVariableWidthHistogram.Bucket(curKey,
                    new InternalVariableWidthHistogram.Bucket.BucketBounds(curKey - (add / 3), curKey + (add / 3)), docCount, format,
    ...
    protected InternalVariableWidthHistogram mutateInstance(InternalVariableWidthHistogram instance) {
        ...
        case 1:
            ...
            buckets.add(new InternalVariableWidthHistogram.Bucket(randomDouble(),
                    new InternalVariableWidthHistogram.Bucket.BucketBounds(boundMin, boundMax), randomIntBetween(1, 100), format,
                    InternalAggregations.EMPTY));
            break;
        case 2:
    ...
    public void testSingleShardReduceLong() {
        ...
        for (long value : new long[] { 1, 2, 5, 10, 12, 200 }) {
            ...
            InternalVariableWidthHistogram.Bucket bucket =
                    new InternalVariableWidthHistogram.Bucket(value, bounds, 3, format, InternalAggregations.EMPTY);
            buckets.add(bucket);
        }
        ...
        MultiBucketConsumerService.MultiBucketConsumer bucketConsumer = new MultiBucketConsumerService.MultiBucketConsumer(
                DEFAULT_MAX_BUCKETS, new NoneCircuitBreakerService().getBreaker(CircuitBreaker.REQUEST));
        InternalAggregation.ReduceContext context = InternalAggregation.ReduceContext.forFinalReduction(bigArrays, mockScriptService,
                bucketConsumer, PipelineAggregator.PipelineTree.EMPTY);
    ...
    public void testSingleShardReduceDouble() {
        ...
        for (double value : new double[] { -1.3, -1.3, 12.0, 13.0, 20.0, 21.5, 23.0, 24.5 }) {
            ...
            InternalVariableWidthHistogram.Bucket bucket =
                    new InternalVariableWidthHistogram.Bucket(value, bounds, 1, format, InternalAggregations.EMPTY);
            buckets.add(bucket);
        }
        InternalVariableWidthHistogram histogram = dummy_histogram.create(buckets);
    ...
    public void testOverlappingReduceResult() {
        ...
        for (long value : new long[] { 1, 2, 4, 10 }) {
            ...
            InternalVariableWidthHistogram.Bucket bucket =
                    new InternalVariableWidthHistogram.Bucket(value, bounds, 4, format, InternalAggregations.EMPTY);
            buckets.add(bucket);
        }
        InternalVariableWidthHistogram histogram = dummy_histogram.create(buckets);
