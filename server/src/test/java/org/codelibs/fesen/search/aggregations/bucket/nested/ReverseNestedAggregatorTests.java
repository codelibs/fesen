...
import static org.codelibs.fesen.search.aggregations.AggregationBuilders.max;
import static org.codelibs.fesen.search.aggregations.AggregationBuilders.nested;
import static org.codelibs.fesen.search.aggregations.AggregationBuilders.reverseNested;
...
public class ReverseNestedAggregatorTests extends AggregatorTestCase {
    ...
    public void testFieldAlias() throws IOException {
        ...
        try (Directory directory = newDirectory()) {
            ...
            try (IndexReader indexReader = wrapInMockESDirectoryReader(DirectoryReader.open(directory))) {
                ...
                NestedAggregationBuilder agg =
                        nested(NESTED_AGG, NESTED_OBJECT).subAggregation(reverseNested(REVERSE_AGG_NAME).subAggregation(maxAgg));
                NestedAggregationBuilder aliasAgg =
                        nested(NESTED_AGG, NESTED_OBJECT).subAggregation(reverseNested(REVERSE_AGG_NAME).subAggregation(aliasMaxAgg));
    ...
    public void testNestedUnderTerms() throws IOException {
        ...
        AggregationBuilder b =
                new NestedAggregationBuilder("nested", "nested_reseller").subAggregation(new TermsAggregationBuilder("resellers")
                        .field("reseller_id").size(numResellers).subAggregation(new ReverseNestedAggregationBuilder("reverse_nested")
