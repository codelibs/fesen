...
public class InternalCompositeTests extends InternalMultiBucketAggregationTestCase<InternalComposite> {
    ...
    protected void assertReduced(InternalComposite reduced, List<InternalComposite> inputs) {
        List<CompositeKey> expectedKeys =
                inputs.stream().flatMap((s) -> s.getBuckets().stream()).map(InternalComposite.InternalBucket::getRawKey)
                        .sorted(getKeyComparator()).distinct().limit(reduced.getSize()).collect(Collectors.toList());
    ...
    public void testReduceUnmapped() throws IOException {
        ...
        InternalComposite unmapped = new InternalComposite(mapped.getName(), mapped.getSize(), sourceNames, rawFormats, emptyList(), null,
                reverseMuls, true, emptyMap());
