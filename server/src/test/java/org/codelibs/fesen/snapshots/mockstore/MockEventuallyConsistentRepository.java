...
public class MockEventuallyConsistentRepository extends BlobStoreRepository {
    ...
    public MockEventuallyConsistentRepository(final RepositoryMetadata metadata, final NamedXContentRegistry namedXContentRegistry,
            final ClusterService clusterService, final RecoverySettings recoverySettings, final Context context, final Random random) {
    ...
    private class MockBlobStore implements BlobStore {
        ...
        private class MockBlobContainer implements BlobContainer {
            ...
            public Map<String, BlobMetadata> listBlobs() {
                ...
                synchronized (context.actions) {
                    return maybeMissLatestIndexN(consistentView(context.actions).stream()
                            .filter(action -> action.path.startsWith(thisPath)
                                    && action.path.substring(thisPath.length()).indexOf('/') == -1 && action.operation == Operation.PUT)
                            .collect(Collectors.toMap(action -> action.path.substring(thisPath.length()),
            ...
            public Map<String, BlobContainer> children() {
                ...
                synchronized (context.actions) {
                    return consistentView(context.actions).stream()
                            .filter(action -> action.operation == Operation.PUT && action.path.startsWith(thisPath)
                                    && action.path.substring(thisPath.length()).indexOf('/') != -1)
                            .map(action -> action.path.substring(thisPath.length()).split("/")[0]).distinct()
            ...
            public Map<String, BlobMetadata> listBlobsByPrefix(String blobNamePrefix) {
                return maybeMissLatestIndexN(listBlobs().entrySet().stream().filter(entry -> entry.getKey().startsWith(blobNamePrefix))
                    ...
                    if (BlobStoreRepository.INDEX_LATEST_BLOB.equals(blobName)
                        ...
                        if (hasConsistentContent) {
                            if (basePath().buildAsString().equals(path().buildAsString())) {
                                try {
                                    final SnapshotInfo updatedInfo = BlobStoreRepository.SNAPSHOT_FORMAT.deserialize(blobName,
                                            namedXContentRegistry, new BytesArray(data));
                                    ...
                                    final SnapshotInfo existingInfo = SNAPSHOT_FORMAT.deserialize(blobName, namedXContentRegistry,
