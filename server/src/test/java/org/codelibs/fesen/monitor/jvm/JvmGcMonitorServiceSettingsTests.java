...
public class JvmGcMonitorServiceSettingsTests extends ESTestCase {
    ...
    public void testNegativeSetting() throws InterruptedException {
        ...
        execute(settings, (command, interval, name) -> null, e -> {
            ...
            assertThat(e.getMessage(), equalTo("failed to parse setting [monitor.jvm.gc.collector." + collector + ".warn] " + "with value ["
                    + timeValue + "] as a time value"));
        }, true, null);
    }
    ...
    public void testNegativeOneSetting() throws InterruptedException {
        ...
        execute(settings, (command, interval, name) -> null, e -> {
            assertThat(e, instanceOf(IllegalArgumentException.class));
            assertThat(e.getMessage(), equalTo("invalid gc_threshold [monitor.jvm.gc.collector." + collector + ".warn] " + "value ["
                    + timeValue + "]: value cannot be negative"));
        }, true, null);
    }
    ...
    public void testMissingSetting() throws InterruptedException {
        ...
        AbstractMap.SimpleEntry<String, String> entry : randomSubsetOf(randomIntBetween(1, 2),
    ...
    private static void execute(Settings settings, TriFunction<Runnable, TimeValue, String, Cancellable> scheduler,
            Consumer<Throwable> consumer, boolean constructionShouldFail, Runnable asserts) throws InterruptedException {
