...
import static org.codelibs.fesen.cluster.coordination.ElectionSchedulerFactory.ELECTION_MAX_TIMEOUT_SETTING;
import static org.codelibs.fesen.cluster.coordination.ElectionSchedulerFactory.toPositiveLongAtMost;
...
public class ElectionSchedulerFactoryTests extends ESTestCase {
    ...
    private void assertElectionSchedule(final DeterministicTaskQueue deterministicTaskQueue,
            final ElectionSchedulerFactory electionSchedulerFactory, final long initialTimeout, final long backOffTime,
            final long maxTimeout, final long duration) {
    ...
    public void testSettingsValidation() {
...

        {
            ...
            final Settings settings = Settings.builder().put(ELECTION_INITIAL_TIMEOUT_SETTING.getKey(), initialTimeoutMillis + "ms")
                    .put(ELECTION_BACK_OFF_TIME_SETTING.getKey(), backOffMillis + "ms")
                    .put(ELECTION_MAX_TIMEOUT_SETTING.getKey(), maxTimeoutMillis + "ms").build();
...

        {
            ...
            IllegalArgumentException e =
                    expectThrows(IllegalArgumentException.class, () -> new ElectionSchedulerFactory(settings, random(), null));
            assertThat(e.getMessage(),
                    equalTo("[cluster.election.max_timeout] is [" + TimeValue.timeValueMillis(maxTimeoutMillis)
