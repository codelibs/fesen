...
public class RetryFailedAllocationTests extends ESAllocationTestCase {
    ...
    public void setUp() throws Exception {
        ...
        Metadata metadata = Metadata.builder()
                .put(IndexMetadata.builder(INDEX_NAME).settings(settings(Version.CURRENT)).numberOfShards(1).numberOfReplicas(1)).build();
    ...
    public void testRetryFailedResetForAllocationCommands() {
        ...
        for (int i = 0; i < retries; i++) {
            List<FailedShard> failedShards = Collections.singletonList(
                    new FailedShard(getReplica(), "failing-shard::attempt-" + i, new FesenException("simulated"), randomBoolean()));
        ...
        AllocationService.CommandsResult result = strategy.reroute(clusterState, new AllocationCommands(
