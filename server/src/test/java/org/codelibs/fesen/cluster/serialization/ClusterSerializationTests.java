...
package org.codelibs.fesen.cluster.serialization;
...
public class ClusterSerializationTests extends ESAllocationTestCase {
    ...
    public void testClusterStateSerialization() throws Exception {
        ...
        DiscoveryNodes nodes = DiscoveryNodes.builder().add(newNode("node1")).add(newNode("node2")).add(newNode("node3"))
                .localNodeId("node1").masterNodeId("node2").build();

        ClusterState clusterState = ClusterState.builder(new ClusterName("clusterName1")).nodes(nodes).metadata(metadata)
                .routingTable(routingTable).minimumMasterNodesOnPublishingMaster(randomIntBetween(-1, 10)).build();
    ...
    public void testSnapshotDeletionsInProgressSerialization() throws Exception {
        ...
        ClusterState.Builder builder =
                ClusterState.builder(ClusterState.EMPTY_STATE).putCustom(SnapshotDeletionsInProgress.TYPE,
                        SnapshotDeletionsInProgress.of(Collections.singletonList(new SnapshotDeletionsInProgress.Entry(
        ...
        if (includeRestore) {
            builder.putCustom(RestoreInProgress.TYPE,
                    new RestoreInProgress.Builder()
                            .add(new RestoreInProgress.Entry(UUIDs.randomBase64UUID(),
                                    new Snapshot("repo2", new SnapshotId("snap2", UUIDs.randomBase64UUID())),
                                    RestoreInProgress.State.STARTED, Collections.singletonList("index_name"), ImmutableOpenMap.of()))
                            .build());
        }
    ...
    public void testObjectReuseWhenApplyingClusterStateDiff() throws Exception {
        ...
        ClusterState clusterState1 =
                ClusterState.builder(new ClusterName("clusterName1")).metadata(metadata).routingTable(routingTable).build();
        BytesStreamOutput outStream = new BytesStreamOutput();
        outStream.setVersion(Version.CURRENT);
        ...
        assertSame("Unchanged metadata should not create new IndexMetadata objects", serializedClusterState1.metadata().index("test"),
        ...
        assertNotEquals("Should have a new IndexMetadata object", serializedClusterState2.metadata().index("test"),
    ...
    public void testCustomSerialization() throws Exception {
        ClusterState.Builder builder = ClusterState.builder(ClusterState.EMPTY_STATE)
                .putCustom(TestCustomOne.TYPE, new TestCustomOne("test_custom_one")).putCustom(TestCustomTwo.TYPE, new TestCustomTwo(10));
