...
import org.codelibs.fesen.cluster.routing.allocation.decider.EnableAllocationDecider.Allocation;
import org.codelibs.fesen.cluster.routing.allocation.decider.EnableAllocationDecider.Rebalance;
...
import static org.codelibs.fesen.cluster.routing.allocation.decider.EnableAllocationDecider.CLUSTER_ROUTING_ALLOCATION_ENABLE_SETTING;
...
import static org.codelibs.fesen.cluster.routing.allocation.decider.EnableAllocationDecider.INDEX_ROUTING_ALLOCATION_ENABLE_SETTING;
...
public class EnableAllocationTests extends ESAllocationTestCase {
    ...
    public void testIndexEnableNone() {
        ...
        Metadata metadata = Metadata.builder()
                .put(IndexMetadata.builder("disabled")
                        .settings(settings(Version.CURRENT).put(INDEX_ROUTING_ALLOCATION_ENABLE_SETTING.getKey(), Allocation.NONE.name()))
                        .numberOfShards(1).numberOfReplicas(1))
                .put(IndexMetadata.builder("enabled").settings(settings(Version.CURRENT)).numberOfShards(1).numberOfReplicas(1)).build();

        RoutingTable initialRoutingTable =
                RoutingTable.builder().addAsNew(metadata.index("disabled")).addAsNew(metadata.index("enabled")).build();
    ...
    public void testEnableClusterBalance() {
        ...
        Settings build = Settings.builder().put(CLUSTER_ROUTING_REBALANCE_ENABLE_SETTING.getKey(),
        ...
        Settings indexSettings = useClusterSetting ? Settings.EMPTY
                : Settings.builder().put(EnableAllocationDecider.INDEX_ROUTING_REBALANCE_ENABLE_SETTING.getKey(), Rebalance.NONE).build();

        logger.info("Building initial routing table");
        Metadata metadata = Metadata.builder()
                .put(IndexMetadata.builder("test").settings(settings(Version.CURRENT).put(indexSettings)).numberOfShards(3)
                        .numberOfReplicas(1))
                .put(IndexMetadata
                        .builder("always_disabled").settings(settings(Version.CURRENT)
                                .put(EnableAllocationDecider.INDEX_ROUTING_REBALANCE_ENABLE_SETTING.getKey(), Rebalance.NONE))
                        .numberOfShards(1).numberOfReplicas(1))
                .build();

        RoutingTable initialRoutingTable =
                RoutingTable.builder().addAsNew(metadata.index("test")).addAsNew(metadata.index("always_disabled")).build();
        ...
        if (useClusterSetting) {
            clusterState =
                    ClusterState.builder(clusterState)
                            .metadata(Metadata.builder(clusterState.metadata()).transientSettings(
                                    Settings.builder().put(CLUSTER_ROUTING_REBALANCE_ENABLE_SETTING.getKey(), allowedOnes).build()))
                            .build();
        } else {
            IndexMetadata meta = clusterState.getMetadata().index("test");
            IndexMetadata meta1 = clusterState.getMetadata().index("always_disabled");
            clusterState = ClusterState.builder(clusterState)
                    .metadata(Metadata.builder(clusterState.metadata()).removeAllIndices().put(IndexMetadata.builder(meta1))
                            .put(IndexMetadata.builder(meta).settings(Settings.builder().put(meta.getSettings())
                                    .put(EnableAllocationDecider.INDEX_ROUTING_REBALANCE_ENABLE_SETTING.getKey(), allowedOnes).build())))
                    .build();
    ...
    public void testEnableClusterBalanceNoReplicas() {
        ...
        Metadata metadata = Metadata.builder().put(
                IndexMetadata.builder("test").settings(settings(Version.CURRENT).put(indexSettings)).numberOfShards(6).numberOfReplicas(0))
                .build();
        ...
        if (useClusterSetting) {
            ...
            clusterState = ClusterState
                    .builder(
                            clusterState)
                    .metadata(
                            Metadata.builder(metadata).removeAllIndices()
                                    .put(IndexMetadata.builder(meta)
                                            .settings(
                                                    Settings.builder().put(meta.getSettings())
        ...
        clusterSettings.applySettings(clusterState.metadata().settings());
        clusterState = strategy.reroute(clusterState, "reroute");
        assertThat("expected 4 primaries to be started and 2 to relocate useClusterSettings: " + useClusterSetting,
                clusterState.getRoutingNodes().shardsWithState(STARTED).size(), equalTo(4));
