...
import org.codelibs.fesen.cluster.coordination.PreVoteRequest;
import org.codelibs.fesen.cluster.coordination.PreVoteResponse;
...
public class MessagesTests extends ESTestCase {
    ...
    public void testJoinEqualsHashCodeSerialization() {
        ...
        EqualsHashCodeTestUtils.checkEqualsAndHashCode(initialJoin,
                (CopyFunction<Join>) join -> copyWriteable(join, writableRegistry(), Join::new), join -> {
                    switch (randomInt(4)) {
                    case 0:
                        // change sourceNode
                    ...
                    case 4:
                        // change version
                        return new Join(join.getSourceNode(), join.getTargetNode(), join.getTerm(), join.getLastAcceptedTerm(),
    ...
    public void testPublishRequestEqualsHashCode() {
        ...
        EqualsHashCodeTestUtils.checkEqualsAndHashCode(initialPublishRequest,
                publishRequest -> new PublishRequest(publishRequest.getAcceptedState()), in -> new PublishRequest(randomClusterState()));
    }
    ...
    public void testPublishWithJoinResponseEqualsHashCodeSerialization() {
        ...
        Join initialJoin = new Join(createNode(randomAlphaOfLength(10)), createNode(randomAlphaOfLength(10)), randomNonNegativeLong(),
                randomNonNegativeLong(), randomNonNegativeLong());
        PublishWithJoinResponse initialPublishWithJoinResponse =
                new PublishWithJoinResponse(initialPublishResponse, randomBoolean() ? Optional.empty() : Optional.of(initialJoin));
        // Note: the explicit cast of the CopyFunction is needed for some IDE (specifically Eclipse 4.8.0) to infer the right type
    ...
    public void testApplyCommitEqualsHashCodeSerialization() {
        ApplyCommitRequest initialApplyCommit =
                new ApplyCommitRequest(createNode(randomAlphaOfLength(10)), randomNonNegativeLong(), randomNonNegativeLong());
        // Note: the explicit cast of the CopyFunction is needed for some IDE (specifically Eclipse 4.8.0) to infer the right type
    ...
    public void testJoinRequestEqualsHashCodeSerialization() {
        Join initialJoin = new Join(createNode(randomAlphaOfLength(10)), createNode(randomAlphaOfLength(10)), randomNonNegativeLong(),
                randomNonNegativeLong(), randomNonNegativeLong());
        JoinRequest initialJoinRequest = new JoinRequest(initialJoin.getSourceNode(), randomNonNegativeLong(),
        ...
        EqualsHashCodeTestUtils.checkEqualsAndHashCode(initialJoinRequest,
                ...
                joinRequest -> {
                    if (randomBoolean() && joinRequest.getOptionalJoin().isPresent() == false) {
                        return new JoinRequest(createNode(randomAlphaOfLength(10)), joinRequest.getMinimumTerm(),
    ...
    public ClusterState randomClusterState() {
        return CoordinationStateTests.clusterState(randomNonNegativeLong(), randomNonNegativeLong(), createNode(randomAlphaOfLength(10)),
                ...
                new CoordinationMetadata.VotingConfiguration(Sets.newHashSet(generateRandomStringArray(10, 10, false))), randomLong());
    }
        ...
        EqualsHashCodeTestUtils.checkEqualsAndHashCode(initialPreVoteResponse,
                ...
                preVoteResponse -> {
                    ...
                    case 1:
                        ...
                        return new PreVoteResponse(preVoteResponse.getCurrentTerm(),
