...
import org.codelibs.fesen.test.TestCustomMetadata;
...
import static org.codelibs.fesen.cluster.metadata.AliasMetadata.newAliasMetadataBuilder;
import static org.codelibs.fesen.cluster.metadata.IndexMetadata.SETTING_VERSION_CREATED;
import static org.codelibs.fesen.cluster.metadata.Metadata.CONTEXT_MODE_API;
import static org.codelibs.fesen.cluster.metadata.Metadata.CONTEXT_MODE_GATEWAY;
...
public class ToAndFromJsonMetadataTests extends ESTestCase {
    ...
    public void testSimpleJsonFromAndTo() throws IOException {
        ...
        Metadata metadata = Metadata.builder()
                .put(IndexTemplateMetadata.builder("foo").patterns(Collections.singletonList("bar")).order(1)
                ...
                .put("component_template", new ComponentTemplate(
                        new Template(Settings.builder().put("setting", "value").build(), new CompressedXContent("{\"baz\":\"eggplant\"}"),
                ...
                .put("index_templatev2", new ComposableIndexTemplate(Arrays.asList("foo", "bar*"),
                        new Template(Settings.builder().put("setting", "value").build(), new CompressedXContent("{\"baz\":\"eggplant\"}"),
                                Collections.singletonMap("alias", AliasMetadata.builder("alias").build())),
                        Collections.singletonList("component_template"), 5L, 4L,
                ...
                .put(IndexMetadata.builder("test12").settings(settings(Version.CURRENT).put("setting1", "value1").put("setting2", "value2"))
                        .creationDate(2L).numberOfShards(1).numberOfReplicas(2).putMapping("mapping1", MAPPING_SOURCE1)
                        .putMapping("mapping2", MAPPING_SOURCE2).putAlias(newAliasMetadataBuilder("alias1").filter(ALIAS_FILTER1))
                ...
                .put(IndexTemplateMetadata.builder("foo").patterns(Collections.singletonList("bar")).order(1)
                        ...
                        .putAlias(newAliasMetadataBuilder("alias-bar2").filter("{\"term\":{\"user\":\"kimchy\"}}"))
                        .putAlias(newAliasMetadataBuilder("alias-bar3").routing("routing-bar")))
                .put(idx1, false).put(idx2, false)
                .put(new DataStream("data-stream1", createTimestampField("@timestamp"), org.codelibs.fesen.core.List.of(idx1.getIndex())))
                .put(new DataStream("data-stream2", createTimestampField("@timestamp"), org.codelibs.fesen.core.List.of(idx2.getIndex())))
                .build();
        ...
        assertThat(parsedMetadata.componentTemplates().get("component_template").template(),
                equalTo(new Template(Settings.builder().put("setting", "value").build(), new CompressedXContent("{\"baz\":\"eggplant\"}"),
                        Collections.singletonMap("alias", AliasMetadata.builder("alias").build()))));
    ...
    public void testToXContentAPI_SameTypeName() throws IOException {
        ...
        Metadata metadata = Metadata.builder().clusterUUID("clusterUUID").coordinationMetadata(CoordinationMetadata.builder().build())
                .put(IndexMetadata.builder("index").state(IndexMetadata.State.OPEN)
                        ...
                        .numberOfShards(1).primaryTerm(0, 1L).numberOfReplicas(2))
                .build();
        XContentBuilder builder = JsonXContent.contentBuilder().prettyPrint();
        builder.startObject();
        metadata.toXContent(builder, new ToXContent.MapParams(mapParams));
    ...
    private Metadata buildMetadata() throws IOException {
        return Metadata.builder().clusterUUID("clusterUUID").coordinationMetadata(CoordinationMetadata.builder().term(1)
                ...
                })).addVotingConfigExclusion(new CoordinationMetadata.VotingConfigExclusion("exlucdedNodeId", "excludedNodeName")).build())
                ...
                .put(IndexMetadata.builder("index").state(IndexMetadata.State.OPEN)
                        ...
                        })).putAlias(AliasMetadata.builder("alias").indexRouting("indexRouting").build()).numberOfShards(1)
                        .primaryTerm(0, 1L).putInSyncAllocationIds(0, new HashSet<String>() {
                            {
                                add("allocationId");
                            }
                        }).numberOfReplicas(2).putRolloverInfo(new RolloverInfo("rolloveAlias", new ArrayList<>(), 1L)))
                .put(IndexTemplateMetadata.builder("template").patterns(Arrays.asList("pattern1", "pattern2")).order(0)
                        .settings(Settings.builder().put(SETTING_VERSION_CREATED, Version.CURRENT.id))
                        .putMapping("type", "{ \"key1\": {} }").build())
                .build();
    }
