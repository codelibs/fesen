...
import static org.codelibs.fesen.cluster.metadata.MetadataIndexStateService.INDEX_CLOSED_BLOCK;
import static org.codelibs.fesen.cluster.metadata.MetadataIndexStateService.INDEX_CLOSED_BLOCK_ID;
...
public class MetadataIndexStateServiceTests extends ESTestCase {
    ...
    public void testCloseRoutingTableWithRestoredIndex() {
        ...
        final Index index = state.metadata().index(indexName).getIndex();
        final ClusterState updatedState = MetadataIndexStateService
                .closeRoutingTable(state, singletonMap(index, block), singletonMap(index, new IndexResult(index))).v1();
            ...
            if (results.get(blockedIndex).hasFailures() == false) {
                ...
                assertThat("Index must have only 1 block with [id=" + MetadataIndexStateService.INDEX_CLOSED_BLOCK_ID + "]",
                        state.blocks().indices().getOrDefault(blockedIndex.getName(), emptySet()).stream()
                                .filter(clusterBlock -> clusterBlock.id() == MetadataIndexStateService.INDEX_CLOSED_BLOCK_ID).count(),
                        equalTo(1L));
    ...
    public void testIsIndexVerifiedBeforeClosed() {
        ...
        }
        {
            ...
            IndexMetadata indexMetadata = IndexMetadata.builder(indexName).state(IndexMetadata.State.CLOSE)
                    .creationDate(randomNonNegativeLong())
                    .settings(Settings.builder().put(SETTING_VERSION_CREATED, Version.CURRENT)
                            .put(SETTING_NUMBER_OF_SHARDS, randomIntBetween(1, 3)).put(SETTING_NUMBER_OF_REPLICAS, randomIntBetween(0, 3)))
                    .build();
    ...
    public void testCloseFailedIfBlockDisappeared() {
        ...
        for (int i = 0; i < numIndices; i++) {
            ...
            if (randomBoolean()) {
                state = ClusterState.builder(state)
                        .blocks(ClusterBlocks.builder().blocks(state.blocks()).removeIndexBlocks(indexName).build()).build();
        ...
        Set<Index> failedIndices =
                closingResults.stream().filter(IndexResult::hasFailures).map(IndexResult::getIndex).collect(Collectors.toSet());
    ...
    public void testCloseCurrentWriteIndexForDataStream() {
        ...
        ClusterState cs = DataStreamTestHelper.getClusterStateWithDataStreams(dataStreamsToCreate, org.codelibs.fesen.core.List.of());
    ...
    private static ClusterState addRestoredIndex(final String index, final int numShards, final int numReplicas, final ClusterState state) {
        ...
        final RestoreInProgress.Entry entry = new RestoreInProgress.Entry("_uuid", snapshot, RestoreInProgress.State.INIT,
    ...
    private static ClusterState addSnapshotIndex(final String index, final int numShards, final int numReplicas, final ClusterState state) {
        ...
        final Snapshot snapshot = new Snapshot(randomAlphaOfLength(10), new SnapshotId(randomAlphaOfLength(5), randomAlphaOfLength(5)));
        final SnapshotsInProgress.Entry entry = new SnapshotsInProgress.Entry(snapshot, randomBoolean(), false,
                SnapshotsInProgress.State.INIT, Collections.singletonList(new IndexId(index, index)), Collections.emptyList(),
                randomNonNegativeLong(), randomLong(), shardsBuilder.build(), null, SnapshotInfoTests.randomUserMetadata(),
                VersionUtils.randomVersion(random()));
        return ClusterState.builder(newState).putCustom(SnapshotsInProgress.TYPE, SnapshotsInProgress.of(Collections.singletonList(entry)))
                .build();
    }

    private static ClusterState addIndex(final ClusterState currentState, final String index, final int numShards, final int numReplicas,
            final IndexMetadata.State state, @Nullable final ClusterBlock block) {

        final Settings.Builder settings = Settings.builder().put(SETTING_VERSION_CREATED, Version.CURRENT)
                .put(SETTING_NUMBER_OF_SHARDS, numShards).put(SETTING_NUMBER_OF_REPLICAS, numReplicas);
        if (state == IndexMetadata.State.CLOSE) {
            settings.put(MetadataIndexStateService.VERIFIED_BEFORE_CLOSE_SETTING.getKey(), true);
        }
        final IndexMetadata indexMetadata =
                IndexMetadata.builder(index).state(state).creationDate(randomNonNegativeLong()).settings(settings).build();
