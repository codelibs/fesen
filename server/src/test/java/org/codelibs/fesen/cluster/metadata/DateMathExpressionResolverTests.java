...
import org.codelibs.fesen.cluster.metadata.IndexNameExpressionResolver.Context;
import org.codelibs.fesen.cluster.metadata.IndexNameExpressionResolver.DateMathExpressionResolver;
...
public class DateMathExpressionResolverTests extends ESTestCase {
    ...
    private final Context context =
            new Context(ClusterState.builder(new ClusterName("_name")).build(), IndicesOptions.strictExpand(), false);
    ...
    public void testExpression_MultiParts() throws Exception {
        ...
        assertThat(result.size(), equalTo(1));
        assertThat(result.get(0),
                equalTo(".text1-" + DateTimeFormat.forPattern("YYYY.MM.dd").print(new DateTime(context.getStartTime(), UTC)) + "-text2-"
    ...
    public void testExpression_MixedArray() throws Exception {
        List<String> result =
                expressionResolver.resolve(context, Arrays.asList("name1", "<.marvel-{now/d}>", "name2", "<.logstash-{now/M{uuuu.MM}}>"));
        assertThat(result.size(), equalTo(4));
        assertThat(result.get(0), equalTo("name1"));
        assertThat(result.get(1),
                equalTo(".marvel-" + DateTimeFormat.forPattern("yyyy.MM.dd").print(new DateTime(context.getStartTime(), UTC))));
        assertThat(result.get(2), equalTo("name2"));
        assertThat(result.get(3), equalTo(
                ".logstash-" + DateTimeFormat.forPattern("yyyy.MM").print(new DateTime(context.getStartTime(), UTC).withDayOfMonth(1))));
    }
    ...
    public void testExpression_CustomTimeZoneInIndexName() throws Exception {
        ...
        if (hoursOffset >= 0) {
            ...
            now = DateTime.now(UTC).plusHours(hoursOffset).plusMinutes(minutesOffset).withHourOfDay(0).withMinuteOfHour(0)
    ...
    public void testExpressionInvalidUnescaped() throws Exception {
        Exception e =
                expectThrows(FesenParseException.class, () -> expressionResolver.resolve(context, Arrays.asList("<.mar}vel-{now/d}>")));
    ...
    public void testExpressionInvalidDateMathFormat() throws Exception {
        Exception e =
                expectThrows(FesenParseException.class, () -> expressionResolver.resolve(context, Arrays.asList("<.marvel-{now/d{}>")));
        assertThat(e.getMessage(), containsString("invalid dynamic name expression"));
    ...
    public void testExpressionInvalidOpenEnded() throws Exception {
        Exception e = expectThrows(FesenParseException.class, () -> expressionResolver.resolve(context, Arrays.asList("<.marvel-{now/d>")));
        assertThat(e.getMessage(), containsString("invalid dynamic name expression"));
        assertThat(e.getMessage(), containsString("date math placeholder is open ended"));
    }
