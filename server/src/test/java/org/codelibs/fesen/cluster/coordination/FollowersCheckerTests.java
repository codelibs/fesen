...
import static org.codelibs.fesen.cluster.coordination.FollowersChecker.FOLLOWER_CHECK_ACTION_NAME;
...
public class FollowersCheckerTests extends ESTestCase {
    ...
    public void testChecksExpectedNodes() {
        ...
        checkCount.set(0);
        followersChecker
                .setCurrentNodes(discoveryNodesHolder[0] = DiscoveryNodes.builder(discoveryNodesHolder[0]).remove(otherNode1).build());
        while (checkCount.get() < 10) {
            if (deterministicTaskQueue.hasRunnableTasks()) {
                deterministicTaskQueue.runRandomTask();
            } else {
    ...
    public void testFailsNodeThatDoesNotRespond() {
        ...
        testBehaviourOfFailingNode(settings, () -> null, "followers check retry count exceeded",
    ...
    public void testFailureCounterResetsOnSuccess() {
        ...
        }, "followers check retry count exceeded", (FOLLOWER_CHECK_RETRY_COUNT_SETTING.get(settings) * (maxRecoveries + 1) - 1)
    ...
    public void testFollowerCheckRequestEqualsHashCodeSerialization() {
        ...
        EqualsHashCodeTestUtils.checkEqualsAndHashCode(
                new FollowerCheckRequest(randomNonNegativeLong(),
                        new DiscoveryNode(randomAlphaOfLength(10), buildNewFakeTransportAddress(), Version.CURRENT)),
                (CopyFunction<FollowerCheckRequest>) rq -> copyWriteable(rq, writableRegistry(), FollowerCheckRequest::new), rq -> {
                    if (randomBoolean()) {
    ...
    public void testPreferMasterNodes() {
        ...
        followersChecker.setCurrentNodes(discoveryNodes);
        List<DiscoveryNode> followerTargets =
                Stream.of(capturingTransport.getCapturedRequestsAndClear()).map(cr -> cr.node).collect(Collectors.toList());
