...
import org.codelibs.fesen.cluster.metadata.IndexMetadata.State;
...
import static org.codelibs.fesen.cluster.DataStreamTestHelper.createBackingIndex;
...
import static org.codelibs.fesen.cluster.metadata.IndexNameExpressionResolver.SYSTEM_INDEX_ACCESS_CONTROL_HEADER_KEY;
...
public class IndexNameExpressionResolverTests extends ESTestCase {
    ...
    public void testIndexOptionsStrict() {
        Metadata.Builder mdBuilder = Metadata.builder().put(indexBuilder("foo").putAlias(AliasMetadata.builder("foofoobar")))
        ...
        for (IndicesOptions options : indicesOptions) {
...

            {
                IndexNotFoundException infe =
                        expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndexNames(context, "bar"));
    ...
    public void testIndexOptionsAllowUnavailableDisallowEmpty() {
        Metadata.Builder mdBuilder = Metadata.builder().put(indexBuilder("foo")).put(indexBuilder("foobar"))
                .put(indexBuilder("foofoo-closed").state(IndexMetadata.State.CLOSE))
                .put(indexBuilder("foofoo").putAlias(AliasMetadata.builder("barbaz")));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
        ...
        for (IndicesOptions options : indicesOptions) {
            ...
            {
                IndexNotFoundException infe =
                        expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndexNames(context, "baz*"));
    ...
    public void testIndexOptionsWildcardExpansion() {
        Metadata.Builder mdBuilder = Metadata.builder().put(indexBuilder("foo").state(IndexMetadata.State.CLOSE)).put(indexBuilder("bar"))
        ...
        IndexNotFoundException infe =
                expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndexNames(context2, "-*"));
    ...
    private static Settings.Builder settings(Settings additionalSettings) {
        return settings(Version.CURRENT).put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1).put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0)
    ...
    public void testConcreteIndicesIgnoreIndicesOneMissingIndex() {
        ...
        IndexNotFoundException infe =
                expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndexNames(context, "testZZZ"));
    ...
    public void testConcreteIndicesNoIndicesErrorMessage() {
        ...
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
        IndexNameExpressionResolver.Context context =
                new IndexNameExpressionResolver.Context(state, IndicesOptions.fromOptions(false, false, true, true), false);
        IndexNotFoundException infe =
                expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndices(context, new String[] {}));
    ...
    public void testConcreteIndicesNoIndicesErrorMessageNoExpand() {
        ...
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
        IndexNameExpressionResolver.Context context =
                new IndexNameExpressionResolver.Context(state, IndicesOptions.fromOptions(false, false, false, false), false);
        IndexNotFoundException infe =
                expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndices(context, new String[] {}));
    ...
    public void testConcreteIndicesWildcardExpansion() {
        Metadata.Builder mdBuilder = Metadata.builder().put(indexBuilder("testXXX").state(State.OPEN))
                .put(indexBuilder("testXXY").state(State.OPEN)).put(indexBuilder("testXYY").state(State.CLOSE))
                .put(indexBuilder("testYYY").state(State.OPEN)).put(indexBuilder("testYYX").state(State.OPEN));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
    ...
    public void testConcreteIndicesWildcardWithNegation() {
        Metadata.Builder mdBuilder =
                Metadata.builder().put(indexBuilder("testXXX").state(State.OPEN)).put(indexBuilder("testXXY").state(State.OPEN))
                        .put(indexBuilder("testXYY").state(State.OPEN)).put(indexBuilder("-testXYZ").state(State.OPEN))
                        .put(indexBuilder("-testXZZ").state(State.OPEN)).put(indexBuilder("-testYYY").state(State.OPEN))
                        .put(indexBuilder("testYYY").state(State.OPEN)).put(indexBuilder("testYYX").state(State.OPEN));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();

        IndexNameExpressionResolver.Context context =
                new IndexNameExpressionResolver.Context(state, IndicesOptions.fromOptions(true, true, true, true), false);
        assertThat(newHashSet(indexNameExpressionResolver.concreteIndexNames(context, "testX*")),
                equalTo(newHashSet("testXXX", "testXXY", "testXYY")));
        ...
        assertThat(newHashSet(indexNameExpressionResolver.concreteIndexNames(state, IndicesOptions.lenientExpandOpen(), "testXXX",
                "testXXY", "testXYY", "-testXXY")), equalTo(newHashSet("testXXX", "testXYY", "testXXY")));
    ...
    public void testConcreteIndicesWildcardAndAliases() {
        Metadata.Builder mdBuilder =
                Metadata.builder().put(indexBuilder("foo_foo").state(State.OPEN).putAlias(AliasMetadata.builder("foo")))
    ...
    public void testHiddenAliasAndHiddenIndexResolution() {
...

        {
            ...
            Metadata.Builder mdBuilder =
                    Metadata.builder().put(indexBuilder(visibleIndex).state(State.OPEN).putAlias(AliasMetadata.builder(visibleAlias)))
                            .put(indexBuilder(hiddenIndex, Settings.builder().put(INDEX_HIDDEN_SETTING.getKey(), true).build())
                                    .state(State.OPEN).putAlias(AliasMetadata.builder(hiddenAlias).isHidden(true)));
            ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
...

        {
            ...
            Metadata.Builder mdBuilder =
                    Metadata.builder().put(indexBuilder(visibleIndex).state(State.OPEN).putAlias(AliasMetadata.builder(visibleAlias)))
                            .put(indexBuilder(hiddenIndex, Settings.builder().put(INDEX_HIDDEN_SETTING.getKey(), true).build())
                                    .state(State.OPEN).putAlias(AliasMetadata.builder(visibleAlias)));
            ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
...

        {
            ...
            Metadata.Builder mdBuilder = Metadata.builder()
                    ...
                    .put(indexBuilder(hiddenIndex, Settings.builder().put(INDEX_HIDDEN_SETTING.getKey(), true).build()).state(State.OPEN)
...

        {
            ...
            Metadata.Builder mdBuilder = Metadata.builder()
                    .put(indexBuilder(dottedHiddenIndex, Settings.builder().put(INDEX_HIDDEN_SETTING.getKey(), true).build())
                            .state(State.OPEN).putAlias(AliasMetadata.builder(dottedHiddenAlias).isHidden(true)))
                    .put(indexBuilder(hiddenIndex, Settings.builder().put(INDEX_HIDDEN_SETTING.getKey(), true).build()).state(State.OPEN)
    ...
    public void testHiddenIndexWithVisibleAliasOverlappingNameResolution() {
        ...
        Metadata.Builder mdBuilder = Metadata.builder()
                .put(indexBuilder(hiddenIndex, Settings.builder().put(INDEX_HIDDEN_SETTING.getKey(), true).build()).state(State.OPEN)
                        .putAlias(AliasMetadata.builder(hiddenAlias).isHidden(true)).putAlias(AliasMetadata.builder(visibleAlias).build()));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
    ...
    public void testConcreteIndicesAllPatternRandom() {
        for (int i = 0; i < 10; i++) {
...

            {
                ...
                Metadata.Builder mdBuilder =
                        Metadata.builder().put(indexBuilder("aaa").state(State.OPEN).putAlias(AliasMetadata.builder("aaa_alias1")))
    ...
    public void testIndexOptionsFailClosedIndicesAndAliases() {
        Metadata.Builder mdBuilder = Metadata.builder()
                .put(indexBuilder("foo1-closed").state(IndexMetadata.State.CLOSE).putAlias(AliasMetadata.builder("foobar1-closed"))
        ...
        IndexNameExpressionResolver.Context context = new IndexNameExpressionResolver.Context(state,
                IndicesOptions.fromOptions(true, contextICE.getOptions().allowNoIndices(), contextICE.getOptions().expandWildcardsOpen(),
        ...
        context = new IndexNameExpressionResolver.Context(state, IndicesOptions.fromOptions(true, context.getOptions().allowNoIndices(),
                context.getOptions().expandWildcardsOpen(), context.getOptions().expandWildcardsClosed(), context.getOptions()), false);
    ...
    public void testFilterClosedIndicesOnAliases() {
        Metadata.Builder mdBuilder =
                Metadata.builder().put(indexBuilder("test-0").state(State.OPEN).putAlias(AliasMetadata.builder("alias-0")))
    ...
    public void testIndexAliases() {
        Metadata.Builder mdBuilder = Metadata.builder()
                .put(indexBuilder("test-0").state(State.OPEN).putAlias(AliasMetadata.builder("test-alias-0").filter("{ \"term\": \"foo\"}"))
        ...
        strings =
                indexNameExpressionResolver.indexAliases(state, "test-0", x -> x.alias().equals("test-alias-1"), true, resolvedExpressions);
        assertArrayEquals(null, strings);
    }

    public void testIndexAliasesSkipIdentity() {
        Metadata.Builder mdBuilder = Metadata.builder().put(indexBuilder("test-0").state(State.OPEN)
                .putAlias(AliasMetadata.builder("test-alias")).putAlias(AliasMetadata.builder("other-alias")));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
    ...
    public void testConcreteWriteIndexSuccessful() {
        ...
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
        String[] strings = indexNameExpressionResolver.indexAliases(state, "test-0", x -> true, true,
        ...
        state = ClusterState.builder(state)
                .metadata(Metadata.builder(state.metadata())
                        .put(indexBuilder("test-1").putAlias(
    ...
    public void testConcreteWriteIndexWithInvalidIndicesRequest() {
        Metadata.Builder mdBuilder =
                Metadata.builder().put(indexBuilder("test-0").state(State.OPEN).putAlias(AliasMetadata.builder("test-alias")));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
    ...
    public void testConcreteWriteIndexWithWildcardExpansion() {
        ...
        IllegalArgumentException exception =
                expectThrows(IllegalArgumentException.class, () -> indexNameExpressionResolver.concreteWriteIndex(state, request));
    ...
    public void testConcreteWriteIndexWithNoWriteIndexWithSingleIndex() {
        ...
        Arrays.sort(strings);
        assertArrayEquals(new String[] { "test-alias" }, strings);
        DocWriteRequest request = randomFrom(new IndexRequest("test-alias"), new UpdateRequest("test-alias", "_type", "_id"),
        ...
        assertThat(exception.getMessage(),
                equalTo("no write index is defined for alias [test-alias]."
    ...
    public void testAliasResolutionNotAllowingMultipleIndices() {
        ...
        Arrays.sort(strings);
        assertArrayEquals(new String[] { "test-alias" }, strings);
        IllegalArgumentException exception = expectThrows(IllegalArgumentException.class, () -> indexNameExpressionResolver
    ...
    public void testDeleteIndexIgnoresAliases() {
        Metadata.Builder mdBuilder =
                Metadata.builder().put(indexBuilder("test-index").state(State.OPEN).putAlias(AliasMetadata.builder("test-alias")))
        ...
        }
        {
            ...
            assertEquals("The provided expression [test-alias] matches an alias, " + "specify the corresponding concrete indices instead.",
                    iae.getMessage());
        }
        {
    ...
    public void testIndicesAliasesRequestIgnoresAliases() {
        ...
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
        {
            ...
            IllegalArgumentException iae =
                    expectThrows(IllegalArgumentException.class, () -> indexNameExpressionResolver.concreteIndexNames(state, aliasActions));
            assertEquals("The provided expression [test-alias] matches an alias, " + "specify the corresponding concrete indices instead.",
                    iae.getMessage());
        }
        {
    ...
    public void testIndicesAliasesRequestTargetDataStreams() {
        ...
        Metadata.Builder mdBuilder = Metadata.builder().put(backingIndex, false).put(new DataStream(dataStreamName,
                createTimestampField("@timestamp"), org.codelibs.fesen.core.List.of(backingIndex.getIndex()), 1));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();

        {
            IndicesAliasesRequest.AliasActions aliasActions = IndicesAliasesRequest.AliasActions.add().index(dataStreamName);
            IndexNotFoundException iae =
                    expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndexNames(state, aliasActions));
...

        {
            IndicesAliasesRequest.AliasActions aliasActions =
                    IndicesAliasesRequest.AliasActions.add().index(dataStreamName).alias("my-data");
            IndexNotFoundException iae =
                    expectThrows(IndexNotFoundException.class, () -> indexNameExpressionResolver.concreteIndexNames(state, aliasActions));
            assertEquals("no such index [" + dataStreamName + "]", iae.getMessage());
        }
    ...
    public void testInvalidIndex() {
        ...
        InvalidIndexNameException iine =
                expectThrows(InvalidIndexNameException.class, () -> indexNameExpressionResolver.concreteIndexNames(context, "_foo"));
        assertEquals("Invalid index name [_foo], must not start with '_'.", iine.getMessage());
    }

    public void testIgnoreThrottled() {
        Metadata.Builder mdBuilder = Metadata.builder()
                .put(indexBuilder("test-index", Settings.builder().put("index.frozen", true).build()).state(State.OPEN)
                ...
                .put(indexBuilder("index", Settings.builder().put(IndexSettings.INDEX_SEARCH_THROTTLED.getKey(), true).build())
                        .state(State.OPEN).putAlias(AliasMetadata.builder("test-alias2")))
                .put(indexBuilder("index-closed", Settings.builder().put("index.frozen", true).build()).state(State.CLOSE)
        ...
        {
            Index[] indices =
                    indexNameExpressionResolver.concreteIndices(state, IndicesOptions.STRICT_EXPAND_OPEN_FORBID_CLOSED, "test-alias");
            assertEquals(1, indices.length);
        ...
        {
            Index[] indices = indexNameExpressionResolver.concreteIndices(state,
                    new IndicesOptions(EnumSet.of(IndicesOptions.Option.ALLOW_NO_INDICES, IndicesOptions.Option.IGNORE_THROTTLED),
        ...
        {
            Index[] indices =
                    indexNameExpressionResolver.concreteIndices(state,
    ...
    public void testDataStreams() {
        ...
        Metadata.Builder mdBuilder = Metadata.builder().put(index1, false).put(index2, false).put(new DataStream(dataStreamName,
                createTimestampField("@timestamp"), org.codelibs.fesen.core.List.of(index1.getIndex(), index2.getIndex()), 2));
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(mdBuilder).build();
        ...
        }
        {
            ...
            IndicesOptions indicesOptions =
                    new IndicesOptions(EnumSet.of(IndicesOptions.Option.ALLOW_NO_INDICES, IndicesOptions.Option.IGNORE_UNAVAILABLE),
                            EnumSet.of(IndicesOptions.WildcardStates.OPEN));
    ...
    public void testDataStreamsWithWildcardExpression() {
        ...
        Metadata.Builder mdBuilder = Metadata.builder().put(index1, false).put(index2, false).put(index3, false).put(index4, false)
    ...
    public void testDataStreamsWithRegularIndexAndAlias() {
        ...
        IndexMetadata index1 = createBackingIndex(dataStream1, 1).build();
        IndexMetadata index2 = createBackingIndex(dataStream1, 2).build();
        IndexMetadata justAnIndex = IndexMetadata.builder("logs-foobarbaz-0").settings(ESTestCase.settings(Version.CURRENT))
                .numberOfShards(1).numberOfReplicas(1).putAlias(new AliasMetadata.Builder("logs-foobarbaz")).build();

        ClusterState state =
                ClusterState.builder(new ClusterName("_name"))
                        .metadata(Metadata.builder().put(index1, false).put(index2, false).put(justAnIndex, false).put(new DataStream(
                                dataStream1, createTimestampField("@timestamp"), Arrays.asList(index1.getIndex(), index2.getIndex()))))
                        .build();
    ...
    public void testDataStreamsNames() {
        ...
        ClusterState state = ClusterState.builder(new ClusterName("_name")).metadata(Metadata.builder().put(index1, false)
                .put(index2, false).put(index3, false).put(index4, false).put(justAnIndex, false)
    ...
    private ClusterState systemIndexTestClusterState() {
        ...
        Metadata.Builder mdBuilder = Metadata.builder().put(indexBuilder(".ml-meta", settings).state(State.OPEN).system(true))
    ...
    private List<String> resolveConcreteIndexNameList(ClusterState state, SearchRequest request) {
        return Arrays.stream(indexNameExpressionResolver.concreteIndices(state, request)).map(i -> i.getName())
