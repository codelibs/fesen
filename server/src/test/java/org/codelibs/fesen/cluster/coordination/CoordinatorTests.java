...
import org.codelibs.fesen.cluster.coordination.DetachClusterCommand;
...
import org.codelibs.fesen.cluster.coordination.Publication;
import org.codelibs.fesen.cluster.coordination.AbstractCoordinatorTestCase.Cluster.ClusterNode;
import org.codelibs.fesen.cluster.coordination.CoordinationMetadata.VotingConfiguration;
import org.codelibs.fesen.cluster.coordination.Coordinator.Mode;
...
import static org.codelibs.fesen.cluster.coordination.AbstractCoordinatorTestCase.Cluster.DEFAULT_DELAY_VARIABILITY;
import static org.codelibs.fesen.cluster.coordination.AbstractCoordinatorTestCase.Cluster.EXTREME_DELAY_VARIABILITY;
import static org.codelibs.fesen.cluster.coordination.Coordinator.PUBLISH_TIMEOUT_SETTING;
import static org.codelibs.fesen.cluster.coordination.Coordinator.Mode.CANDIDATE;
import static org.codelibs.fesen.cluster.coordination.ElectionSchedulerFactory.ELECTION_INITIAL_TIMEOUT_SETTING;
import static org.codelibs.fesen.cluster.coordination.FollowersChecker.FOLLOWER_CHECK_INTERVAL_SETTING;
import static org.codelibs.fesen.cluster.coordination.FollowersChecker.FOLLOWER_CHECK_RETRY_COUNT_SETTING;
import static org.codelibs.fesen.cluster.coordination.FollowersChecker.FOLLOWER_CHECK_TIMEOUT_SETTING;
import static org.codelibs.fesen.cluster.coordination.LeaderChecker.LEADER_CHECK_INTERVAL_SETTING;
import static org.codelibs.fesen.cluster.coordination.LeaderChecker.LEADER_CHECK_RETRY_COUNT_SETTING;
import static org.codelibs.fesen.cluster.coordination.LeaderChecker.LEADER_CHECK_TIMEOUT_SETTING;
import static org.codelibs.fesen.cluster.coordination.NoMasterBlockService.NO_MASTER_BLOCK_ALL;
import static org.codelibs.fesen.cluster.coordination.NoMasterBlockService.NO_MASTER_BLOCK_METADATA_WRITES;
import static org.codelibs.fesen.cluster.coordination.NoMasterBlockService.NO_MASTER_BLOCK_SETTING;
import static org.codelibs.fesen.cluster.coordination.NoMasterBlockService.NO_MASTER_BLOCK_WRITES;
import static org.codelibs.fesen.cluster.coordination.Reconfigurator.CLUSTER_AUTO_SHRINK_VOTING_CONFIGURATION;
import static org.codelibs.fesen.discovery.PeerFinder.DISCOVERY_FIND_PEERS_INTERVAL_SETTING;
...
import static org.codelibs.fesen.test.NodeRoles.nonMasterNode;
...
public class CoordinatorTests extends AbstractCoordinatorTestCase {
    ...
    public void testUnhealthyNodesGetsRemoved() {
        ...
        try (Cluster cluster = new Cluster(3)) {
            ...
            ClusterNode newNode1 =
                    cluster.new ClusterNode(nextNodeIndex.getAndIncrement(), true, leader.nodeSettings, () -> healthStatusInfo.get());
            ClusterNode newNode2 =
                    cluster.new ClusterNode(nextNodeIndex.getAndIncrement(), true, leader.nodeSettings, () -> healthStatusInfo.get());
    ...
    public void testUnHealthyLeaderRemoved() {
        ...
        try (Cluster cluster = new Cluster(randomIntBetween(1, 3), true, Settings.EMPTY, () -> nodeHealthServiceStatus.get())) {
            ...
            assertThat(cluster.getAnyLeader().getId(),
    ...
    public void testAckListenerReceivesNacksFromFollowerInHigherTerm() {
        try (Cluster cluster = new Cluster(3)) {
            ...
            follower0.coordinator
                    .onFollowerCheckRequest(new FollowersChecker.FollowerCheckRequest(originalTerm + 1, leader.coordinator.getLocalNode()));
    ...
    public void testCannotSetInitialConfigurationWithoutQuorum() {
        try (Cluster cluster = new Cluster(randomIntBetween(1, 5))) {
            final Coordinator coordinator = cluster.getAnyNode().coordinator;
            final VotingConfiguration unknownNodeConfiguration =
                    new VotingConfiguration(Sets.newHashSet(coordinator.getLocalNode().getId(), "unknown-node"));
    ...
    private void testAppliesNoMasterBlock(String noMasterBlockSetting, ClusterBlock expectedBlock) {
        try (Cluster cluster = new Cluster(3)) {
            ...
            leader.submitUpdateTask("update NO_MASTER_BLOCK_SETTING", cs -> {
                ...
                return ClusterState.builder(cs).metadata(Metadata.builder(cs.metadata()).persistentSettings(settingsBuilder.build()))
                        .build();
            }, (source, e) -> {});
            ...
            cluster.runFor(Math.max(defaultMillis(FOLLOWER_CHECK_TIMEOUT_SETTING) + defaultMillis(FOLLOWER_CHECK_INTERVAL_SETTING),
    ...
    public void testClusterCannotFormWithFailingJoinValidation() {
        try (Cluster cluster = new Cluster(randomIntBetween(1, 5))) {
            ...
            randomValueOtherThanMany(
                    ...
                    () -> randomSubsetOf(cluster.clusterNodes)).forEach(cn -> cn.extraJoinValidators.add((discoveryNode, clusterState) -> {
                        throw new IllegalArgumentException("join validation failed");
                    }));
    ...
    public void testCannotJoinClusterWithDifferentUUID() throws IllegalAccessException {
        try (Cluster cluster1 = new Cluster(randomIntBetween(1, 3))) {
            ...
            final ClusterNode newNode = cluster1.new ClusterNode(nextNodeIndex.getAndIncrement(), nodeInOtherCluster.getLocalNode(),
                    n -> cluster1.new MockPersistedState(n, nodeInOtherCluster.persistedState, Function.identity(), Function.identity()),
            ...
            newNode.close();
            final ClusterNode detachedNode = newNode.restartedNode(DetachClusterCommand::updateMetadata,
    ...
    public void testFollowerRemovedIfUnableToSendRequestsToMaster() {
        try (Cluster cluster = new Cluster(3)) {
            ...
            assertThat(leader.getLastAppliedClusterState().nodes().toString(), leader.getLastAppliedClusterState().nodes().getSize(),
                    equalTo(2));
    ...
    public void testSingleNodeDiscoveryWithoutQuorum() {
        try (Cluster cluster = new Cluster(3)) {
            ...
            IllegalStateException ise = expectThrows(IllegalStateException.class, () -> cluster.clusterNodes
                    .replaceAll(cn -> cn == clusterNode ? cn.restartedNode(Function.identity(), Function.identity(), Settings.builder()
                            .put(DiscoveryModule.DISCOVERY_TYPE_SETTING.getKey(), DiscoveryModule.SINGLE_NODE_DISCOVERY_TYPE).build())
                            : cn));
            assertThat(ise.getMessage(), allOf(containsString("cannot start with [discovery.type] set to [single-node] when local node"),
    ...
    public void testSingleNodeDiscoveryStabilisesEvenWhenDisrupted() {
        try (Cluster cluster = new Cluster(1, randomBoolean(), Settings.builder()
            ...
            cluster.runFor(
                    ELECTION_INITIAL_TIMEOUT_SETTING.get(Settings.EMPTY).millis() + delayVariabilityMillis + 4 * delayVariabilityMillis // two round trips for pre-voting and voting
    ...
    public void testClusterRecoversAfterExceptionDuringSerialization() {
        try (Cluster cluster = new Cluster(randomIntBetween(1, 5))) {
            ...
            leader1.submitUpdateTask("broken-task", cs -> ClusterState.builder(cs).putCustom("broken", new BrokenCustom()).build(),
                    (source, e) -> {
    ...
    public void testLogsWarningPeriodicallyIfClusterNotFormed() throws IllegalAccessException {
        ...
        if (randomBoolean()) {
            ...
            settings = Settings.builder().put(ClusterFormationFailureHelper.DISCOVERY_CLUSTER_FORMATION_WARNING_TIMEOUT_SETTING.getKey(),
                    warningDelayMillis + "ms").build();
        }
                ...
                try {
                    ...
                    mockLogAppender.addExpectation(new MockLogAppender.LoggingExpectation() {
                        ...
                        public void match(LogEvent event) {
                            ...
                            final List<ClusterNode> matchingNodes = cluster.clusterNodes.stream().filter(n -> event.getContextData()
                                    .<String> getValue(NODE_ID_LOG_CONTEXT_KEY).equals(getNodeIdForLogContext(n.getLocalNode())))
                                    .collect(Collectors.toList());
                            assertThat(matchingNodes, hasSize(1));

                            assertTrue(Regex.simpleMatch(
                                    "*have discovered *" + matchingNodes.get(0).toString() + "*discovery will continue*", message));
    ...
    public void testLogsMessagesIfPublicationDelayed() throws IllegalAccessException {
        try (Cluster cluster = new Cluster(between(3, 5))) {
            ...
            try {
                ...
                mockLogAppender.addExpectation(new MockLogAppender.SeenEventExpectation("lag warning", LagDetector.class.getCanonicalName(),
                        Level.WARN, "node [" + brokenNode + "] is lagging at cluster state version [*], "
                ...
                cluster.deterministicTaskQueue.scheduleAt(
                        cluster.deterministicTaskQueue.getCurrentTimeMillis() + DEFAULT_CLUSTER_STATE_UPDATE_DELAY, new Runnable() {
                            @Override
                            public void run() {
                                brokenNode.heal();
                            }
                ...
                cluster.runFor(
                        defaultMillis(PUBLISH_TIMEOUT_SETTING) + 2 * DEFAULT_DELAY_VARIABILITY
    ...
    public void testReconfiguresToExcludeMasterIneligibleNodesInVotingConfig() {
        try (Cluster cluster = new Cluster(3)) {
            ...
            cluster.clusterNodes
                    .replaceAll(cn -> cn == chosenNode ? cn.restartedNode(Function.identity(), Function.identity(), nonMasterNode()) : cn);
            cluster.stabilise();
    ...
    private ClusterState buildNewClusterStateWithVotingConfigExclusion(ClusterState currentState,
        ...
        DiscoveryNodes newNodes =
                DiscoveryNodes
                        .builder(currentState.nodes()).add(new DiscoveryNode("resolvableNodeName", "resolvableNodeId",
                                buildNewFakeTransportAddress(), emptyMap(), singleton(DiscoveryNodeRole.MASTER_ROLE), Version.CURRENT))
                        .build();
