...
public class StoredScriptTests extends AbstractSerializingTestCase<StoredScriptSource> {
    ...
    public void testSourceParsing() throws Exception {
        ...
        try (XContentBuilder builder = XContentFactory.contentBuilder(XContentType.JSON)) {
            builder.startObject().startObject("script").field("lang", "mustache").startObject("source").field("query", "code").endObject()
                    .endObject().endObject();
            String code;
            ...
            StoredScriptSource source =
                    new StoredScriptSource("mustache", code, Collections.singletonMap("content_type", "application/json; charset=UTF-8"));
        ...
        try (XContentBuilder builder = XContentFactory.contentBuilder(XContentType.JSON)) {
            builder.startObject().field("script").startObject().field("lang", "lang").field("source", "code").field("options").startObject()
        ...
        try (XContentBuilder builder = XContentFactory.contentBuilder(XContentType.JSON)) {
            Strings.toString(builder.startObject().field("script").startObject().field("lang", "lang").startObject("source")
                    .field("query", "code").endObject().startObject("options").endObject().endObject().endObject());
            String code;
    ...
    public void testSourceParsingErrors() throws Exception {
        ...
        try (XContentBuilder builder = XContentFactory.contentBuilder(XContentType.JSON)) {
            builder.startObject().field("script").startObject().field("lang", "lang").field("source", "code").startObject("options")
                    .field("option", "option").endObject().endObject().endObject();
        ...
        try (XContentBuilder builder = XContentFactory.contentBuilder(XContentType.JSON)) {
            ...
            ParsingException pEx =
                    expectThrows(ParsingException.class, () -> StoredScriptSource.parse(BytesReference.bytes(builder), XContentType.JSON));
    ...
    protected StoredScriptSource createTestInstance() {
        return new StoredScriptSource(randomAlphaOfLength(randomIntBetween(4, 32)), randomAlphaOfLength(randomIntBetween(4, 16383)),
