...
public class ScriptLanguagesInfoTests extends ESTestCase {
    ...
    private ScriptService getMockScriptService(Settings settings) {
        MockScriptEngine scriptEngine =
                new MockScriptEngine(MockScriptEngine.NAME, Collections.singletonMap("test_script", script -> 1), Collections.emptyMap());
    ...
    public void testOnlyScriptEngineContextsReturned() {
        ...
        Map<String, ScriptContext<?>> mockContexts =
                scriptEngine.getSupportedContexts().stream().collect(Collectors.toMap(c -> c.name, Function.identity()));
