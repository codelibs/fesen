...
import static org.codelibs.fesen.action.ingest.SimulatePipelineRequest.Fields;
import static org.codelibs.fesen.action.ingest.SimulatePipelineRequest.SIMULATED_PIPELINE_ID;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.ID;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.IF_PRIMARY_TERM;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.IF_SEQ_NO;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.INDEX;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.ROUTING;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.TYPE;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.VERSION;
import static org.codelibs.fesen.ingest.IngestDocument.Metadata.VERSION_TYPE;
...
public class SimulatePipelineRequestParsingTests extends ESTestCase {
    ...
    private void innerTestParseWithProvidedPipeline(boolean useExplicitType) throws Exception {
        ...
        for (int i = 0; i < numDocs; i++) {
            Map<String, Object> doc = new HashMap<>();
            Map<String, Object> expectedDoc = new HashMap<>();
            List<IngestDocument.Metadata> fields =
                    Arrays.asList(INDEX, TYPE, ID, ROUTING, VERSION, VERSION_TYPE, IF_SEQ_NO, IF_PRIMARY_TERM);
    ...
    public void testNotValidDocs() {
        ...
        requestContent.put(Fields.PIPELINE, pipelineConfig);
        Exception e1 =
                expectThrows(IllegalArgumentException.class, () -> SimulatePipelineRequest.parse(requestContent, false, ingestService));
        ...
        requestContent.put(Fields.DOCS, docs);
        requestContent.put(Fields.PIPELINE, pipelineConfig);
        Exception e3 = expectThrows(FesenParseException.class, () -> SimulatePipelineRequest.parse(requestContent, false, ingestService));
