...
import org.codelibs.fesen.action.bulk.TransportBulkActionTookTests.Resolver;
...
import org.codelibs.fesen.cluster.metadata.IndexAbstraction.Index;
...
import static org.codelibs.fesen.action.bulk.TransportBulkAction.prohibitCustomRoutingOnDataStream;
import static org.codelibs.fesen.cluster.metadata.MetadataCreateDataStreamServiceTests.createDataStream;
...
public class TransportBulkActionTests extends ESTestCase {
    ...
    class TestTransportBulkAction extends TransportBulkAction {
        ...
        TestTransportBulkAction() {
            super(TransportBulkActionTests.this.threadPool, transportService, clusterService, null, null, null,
                    new ActionFilters(Collections.emptySet()), new Resolver(),
    ...
    public void testDeleteNonExistingDocExternalVersionCreatesIndex() throws Exception {
        BulkRequest bulkRequest =
                new BulkRequest().add(new DeleteRequest("index", "type", "id").versionType(VersionType.EXTERNAL).version(0));
    ...
    public void testDeleteNonExistingDocExternalGteVersionCreatesIndex() throws Exception {
        BulkRequest bulkRequest =
                new BulkRequest().add(new DeleteRequest("index2", "type", "id").versionType(VersionType.EXTERNAL_GTE).version(0));
    ...
    public void testGetIndexWriteRequest() throws Exception {
        ...
        UpdateRequest scriptedUpsert =
                new UpdateRequest("index", "type", "id2").upsert(indexRequest).script(mockScript("1")).scriptedUpsert(true);
    ...
    public void testProhibitAppendWritesInBackingIndices() throws Exception {
        ...
        DocWriteRequest<?> validRequest =
                new IndexRequest(backingIndexName).opType(DocWriteRequest.OpType.INDEX).setIfSeqNo(1).setIfPrimaryTerm(1);
    ...
    public void testProhibitCustomRoutingOnDataStream() throws Exception {
        ...
        DocWriteRequest<?> writeRequestAgainstDataStream =
                new IndexRequest(dataStreamName).opType(DocWriteRequest.OpType.INDEX).routing("custom");
        ...
        DocWriteRequest<?> writeRequestAgainstIndex = new IndexRequest(DataStream.getDefaultBackingIndexName(dataStreamName, 1L))
                .opType(DocWriteRequest.OpType.INDEX).routing("custom");
    ...
    public void testIncludesSystem() {
        ...
        SystemIndices systemIndices = new SystemIndices(
                org.codelibs.fesen.core.Map.of("plugin", org.codelibs.fesen.core.List.of(new SystemIndexDescriptor(".test", ""))));
