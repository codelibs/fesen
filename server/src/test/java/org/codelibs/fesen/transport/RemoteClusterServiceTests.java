...
public class RemoteClusterServiceTests extends ESTestCase {
    ...
    private MockTransportService startTransport(final String id, final List<DiscoveryNode> knownNodes, final Version version,
    ...
    public void testRemoteClusterSeedSetting() {
        // simple validation
        Settings settings = Settings.builder().put("cluster.remote.foo.seeds", "192.168.0.1:8080")
        ...
        expectThrows(IllegalArgumentException.class, () -> SniffConnectionStrategy.REMOTE_CLUSTER_SEEDS
        ...
        Exception e = expectThrows(IllegalArgumentException.class, () -> SniffConnectionStrategy.REMOTE_CLUSTER_SEEDS
                .getAllConcreteSettings(brokenSettings).forEach(setting -> setting.get(brokenPortSettings)));
        ...
        try (MockTransportService cluster1Transport = startTransport("cluster_1_node", knownNodes, Version.CURRENT);
            ...
            try (MockTransportService transportService =
                ...
                try (RemoteClusterService service = new RemoteClusterService(builder.build(), transportService)) {
                    ...
                    Map<String, List<String>> perClusterIndices =
                            service.groupClusterIndices(
                                    service.getRemoteClusterNames(), new String[] { "foo:bar", "cluster_1:bar", "cluster_2:foo:bar",
                                            "cluster_1:test", "cluster_2:foo*", "foo", "cluster*:baz", "*:boo", "no*match:boo" },
                                    i -> false);
                    ...
                    IllegalArgumentException iae = expectThrows(IllegalArgumentException.class, () -> service.groupClusterIndices(
        ...
        try (MockTransportService cluster1Transport = startTransport("cluster_1_node", knownNodes, Version.CURRENT);
            ...
            try (MockTransportService transportService =
                ...
                try (RemoteClusterService service = new RemoteClusterService(builder.build(), transportService)) {
                    ...
                    assertFalse(service.isRemoteClusterRegistered("foo"));
                    {
                        Map<String, OriginalIndices> perClusterIndices =
                                service.groupIndices(
                                        IndicesOptions.LENIENT_EXPAND_OPEN, new String[] { "foo:bar", "cluster_1:bar", "cluster_2:foo:bar",
                                                "cluster_1:test", "cluster_2:foo*", "foo", "cluster*:baz", "*:boo", "no*match:boo" },
                                        i -> false);
                    ...
                    }
                    {
                        IllegalArgumentException iae =
                                expectThrows(IllegalArgumentException.class,
                                        () -> service.groupClusterIndices(
                                                service.getRemoteClusterNames(), new String[] { "foo:bar", "cluster_1:bar",
                    ...
                    }
                    {
                        Map<String, OriginalIndices> perClusterIndices = service.groupIndices(IndicesOptions.LENIENT_EXPAND_OPEN,
                                new String[] { "cluster_1:bar", "cluster_2:foo*" }, i -> false);
                        assertEquals(2, perClusterIndices.size());
                    ...
                    {
                        Map<String, OriginalIndices> perClusterIndices =
                                service.groupIndices(IndicesOptions.LENIENT_EXPAND_OPEN, Strings.EMPTY_ARRAY, i -> false);
        ...
        try (MockTransportService cluster1Transport = startTransport("cluster_1_node", knownNodes, Version.CURRENT);
            ...
            try (MockTransportService transportService =
                ...
                try (RemoteClusterService service = new RemoteClusterService(Settings.EMPTY, transportService)) {
                    ...
                    service.initializeRemoteClusters();
                    assertFalse(service.isCrossClusterSearchEnabled());
                    Settings cluster1Settings =
                            createSettings("cluster_1", Collections.singletonList(cluster1Seed.getAddress().toString()));
                    ...
                    assertTrue(service.isCrossClusterSearchEnabled());
                    assertTrue(service.isRemoteClusterRegistered("cluster_1"));
                    Settings cluster2Settings =
                            createSettings("cluster_2", Collections.singletonList(cluster2Seed.getAddress().toString()));
    ...
    public void testCustomPingSchedule() throws IOException {
        ...
        try (MockTransportService cluster1Transport = startTransport("cluster_1_node", knownNodes, Version.CURRENT);
            ...
            try (MockTransportService transportService =
                    MockTransportService.createNewService(transportSettings, Version.CURRENT, threadPool, null)) {
                transportService.start();
                transportService.acceptIncomingRequests();
                Settings.Builder builder = Settings.builder();
                builder.putList("cluster.remote.cluster_1.seeds", cluster1Seed.getAddress().toString());
    ...
    public void testRemoteNodeAttribute() throws IOException, InterruptedException {
        ...
        try (MockTransportService c1N1 = startTransport("cluster_1_node_1", knownNodes, Version.CURRENT);
            ...
            try (MockTransportService transportService =
                ...
                try (RemoteClusterService service = new RemoteClusterService(settings, transportService)) {
                    ...
                    service.updateRemoteCluster("cluster_2",
    ...
    public void testCollectNodes() throws InterruptedException, IOException {
        ...
        try (MockTransportService c1N1 = startTransport("cluster_1_node_1", knownNodes_c1, Version.CURRENT, settings);
                    ...
                    }
                    {
                        ...
                        if (ex.get() instanceof IllegalStateException) {
                            ...
                            assertThat(ex.get(), either(instanceOf(TransportException.class))
                                    .or(instanceOf(NoSuchRemoteClusterException.class)).or(instanceOf(NoSeedNodeLeftException.class)));
                        }
                    }
                }
            }
    ...
    public void testRemoteClusterSkipIfDisconnectedSetting() {
        {
            Settings settings = Settings.builder().put("cluster.remote.foo.seeds", "127.0.0.1:9300")
        ...
        }
        {
            ...
            IllegalArgumentException iae =
                    expectThrows(IllegalArgumentException.class, () -> RemoteClusterService.REMOTE_CLUSTER_SKIP_UNAVAILABLE
                            .getAllConcreteSettings(brokenSettingsDependency).forEach(setting -> setting.get(brokenSettingsDependency)));
        ...
        }
        {
            ...
            IllegalArgumentException iae =
                    expectThrows(IllegalArgumentException.class, () -> RemoteClusterService.REMOTE_CLUSTER_SKIP_UNAVAILABLE
                            .getAllConcreteSettings(brokenSettingsType).forEach(setting -> setting.get(brokenSettingsType)));
        }

        {
            Settings settings =
                    Settings.builder().put("cluster.remote.foo.mode", "proxy").put("cluster.remote.foo.proxy_address", "127.0.0.1:9300")
        ...
        {
            Settings brokenSettingsDependency = Settings.builder().put("cluster.remote.foo.proxy_address", "127.0.0.1:9300")
                    .put("cluster.remote.foo.transport.ping_schedule", "5s").build();
            IllegalArgumentException iae =
                    expectThrows(IllegalArgumentException.class, () -> RemoteClusterService.REMOTE_CLUSTER_PING_SCHEDULE
                            .getAllConcreteSettings(brokenSettingsDependency).forEach(setting -> setting.get(brokenSettingsDependency)));
        ...
        }
        {
            ...
            IllegalArgumentException iae = expectThrows(IllegalArgumentException.class, () -> RemoteClusterService.REMOTE_CLUSTER_COMPRESS
                    .getAllConcreteSettings(brokenSettingsDependency).forEach(setting -> setting.get(brokenSettingsDependency)));
            assertEquals("Cannot configure setting [cluster.remote.foo.transport.compress] if remote cluster is not enabled.",
                    iae.getMessage());
        }

        AbstractScopedSettings service = new ClusterSettings(Settings.EMPTY, new HashSet<>(
                Arrays.asList(SniffConnectionStrategy.REMOTE_CLUSTER_SEEDS, RemoteClusterService.REMOTE_CLUSTER_SKIP_UNAVAILABLE)));
        {
            ...
            IllegalArgumentException iae =
                    expectThrows(IllegalArgumentException.class, () -> service.validate(brokenSettingsDependency, true));
            assertEquals("Cannot configure setting [cluster.remote.foo.skip_unavailable] if remote cluster is not enabled.",
                    iae.getMessage());
        }
    }
