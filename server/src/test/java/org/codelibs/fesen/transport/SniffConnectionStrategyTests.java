...
import org.codelibs.fesen.transport.RemoteConnectionManager;
...
public class SniffConnectionStrategyTests extends ESTestCase {
    ...
    public MockTransportService startTransport(final String id, final List<DiscoveryNode> knownNodes, final Version version,
        ...
        final Settings s =
                Settings.builder().put(ClusterName.CLUSTER_NAME_SETTING.getKey(), clusterAlias).put("node.name", id).put(settings).build();
            ...
            try (MockTransportService localService = MockTransportService.createNewService(Settings.EMPTY, Version.CURRENT, threadPool)) {
                ...
                try (RemoteConnectionManager remoteConnectionManager = new RemoteConnectionManager(clusterAlias, connectionManager);
                    ...
                    assertThat(ise.getMessage(),
                            allOf(startsWith("handshake with [{cluster-alias#"), endsWith(" failed: remote cluster name [otherCluster] "
    ...
    public void testConfiguredProxyAddressModeWillReplaceNodeAddress() {
        ...
        try (MockTransportService accessible = startTransport("seed_node", knownNodes, Version.CURRENT);
            ...
            DiscoveryNode unaddressableSeedNode =
                    new DiscoveryNode(accessibleNode.getName(), accessibleNode.getId(), accessibleNode.getEphemeralId(),
                            accessibleNode.getHostName(), accessibleNode.getHostAddress(), unresponsive1.getLocalNode().getAddress(),
                            accessibleNode.getAttributes(), accessibleNode.getRoles(), accessibleNode.getVersion());
        ...
        try (MockTransportService seedTransport = startTransport("seed_node", knownNodes, Version.CURRENT);
            ...
            try (MockTransportService localService = MockTransportService.createNewService(Settings.EMPTY, Version.CURRENT, threadPool)) {
                ...
                try (RemoteConnectionManager remoteConnectionManager = new RemoteConnectionManager(clusterAlias, connectionManager);
                    ...
                    Setting<?> numConnections =
                            SniffConnectionStrategy.REMOTE_NODE_CONNECTIONS.getConcreteSettingForNamespace("cluster-alias");

                    Settings noChange =
                            Settings.builder().put(seedSetting.getKey(), Strings.arrayToCommaDelimitedString(seedNodes(seedNode).toArray()))
                                    .put(numConnections.getKey(), 3).build();
                    assertFalse(strategy.shouldRebuildConnection(noChange));
                    Settings seedsChanged = Settings.builder()
                            .put(seedSetting.getKey(), Strings.arrayToCommaDelimitedString(seedNodes(discoverableNode).toArray())).build();
                    assertTrue(strategy.shouldRebuildConnection(seedsChanged));
                    Settings proxyChanged =
                            Settings.builder().put(seedSetting.getKey(), Strings.arrayToCommaDelimitedString(seedNodes(seedNode).toArray()))
                                    .put(proxySetting.getKey(), "proxy_address:9300").build();
                    assertTrue(strategy.shouldRebuildConnection(proxyChanged));
                    Settings connectionsChanged =
                            Settings.builder().put(seedSetting.getKey(), Strings.arrayToCommaDelimitedString(seedNodes(seedNode).toArray()))
                                    .put(numConnections.getKey(), 4).build();
    ...
    public void testGetNodePredicateNodeAttrs() {
        ...
        {
            DiscoveryNode nonGatewayNode =
                    new DiscoveryNode("id", address, Collections.singletonMap("gateway", "false"), roles, Version.CURRENT);
        ...
        {
            DiscoveryNode gatewayNode =
                    new DiscoveryNode("id", address, Collections.singletonMap("gateway", "true"), roles, Version.CURRENT);
    ...
    public void testGetNodePredicatesCombination() {
        ...
        {
            DiscoveryNode node =
                    new DiscoveryNode("id", address, Collections.singletonMap("gateway", "true"), dedicatedMasterRoles, Version.CURRENT);
            assertFalse(nodePredicate.test(node));
        }
        {
            DiscoveryNode node =
                    new DiscoveryNode("id", address, Collections.singletonMap("gateway", "false"), dedicatedMasterRoles, Version.CURRENT);
            assertFalse(nodePredicate.test(node));
        }
        {
            DiscoveryNode node =
    ...
    public void testModeSettingsCannotBeUsedWhenInDifferentMode() {
        List<Tuple<Setting.AffixSetting<?>, String>> restrictedSettings =
                Arrays.asList(new Tuple<>(SniffConnectionStrategy.REMOTE_CLUSTER_SEEDS, "192.168.0.1:8080"),
