...
import org.codelibs.fesen.transport.OutboundHandler;
import org.codelibs.fesen.transport.StatsTracker;
...
public class TcpTransportTests extends ESTestCase {
    ...
    public void testDefaultSeedAddressesWithDefaultPort() {
        final Matcher<Iterable<? extends String>> seedAddressMatcher = NetworkUtils.SUPPORTS_V6
                ? containsInAnyOrder("[::1]:9300", "[::1]:9301", "[::1]:9302", "[::1]:9303", "[::1]:9304", "[::1]:9305", "127.0.0.1:9300",
                        "127.0.0.1:9301", "127.0.0.1:9302", "127.0.0.1:9303", "127.0.0.1:9304", "127.0.0.1:9305")
                : containsInAnyOrder("127.0.0.1:9300", "127.0.0.1:9301", "127.0.0.1:9302", "127.0.0.1:9303", "127.0.0.1:9304",
                        "127.0.0.1:9305");
    ...
    public void testDefaultSeedAddressesWithNonstandardGlobalPortRange() {
        final Matcher<Iterable<? extends String>> seedAddressMatcher = NetworkUtils.SUPPORTS_V6
                ? containsInAnyOrder("[::1]:9500", "[::1]:9501", "[::1]:9502", "[::1]:9503", "[::1]:9504", "[::1]:9505", "127.0.0.1:9500",
                        "127.0.0.1:9501", "127.0.0.1:9502", "127.0.0.1:9503", "127.0.0.1:9504", "127.0.0.1:9505")
                : containsInAnyOrder("127.0.0.1:9500", "127.0.0.1:9501", "127.0.0.1:9502", "127.0.0.1:9503", "127.0.0.1:9504",
                        "127.0.0.1:9505");
    ...
    public void testDefaultSeedAddressesPrefersProfileSettingToGlobalSetting() {
        ...
        testDefaultSeedAddresses(Settings.builder()
                .put(TransportSettings.PORT_PROFILE.getConcreteSettingForNamespace(TransportSettings.DEFAULT_PROFILE).getKey(), "9300-9302")
                .put(TransportSettings.PORT.getKey(), "9500-9600").build(), seedAddressMatcher);
    }
    ...
    private void testDefaultSeedAddresses(final Settings settings, Matcher<Iterable<? extends String>> seedAddressesMatcher) {
        ...
        try {
            final TcpTransport tcpTransport =
                    new TcpTransport(settings, Version.CURRENT, testThreadPool, new MockPageCacheRecycler(settings),
    ...
    public void testInvalidHeader() throws IOException {
        ...
        try {
            ...
            assertThat(ex, instanceOf(StreamCorruptedException.class));
            String expected = "invalid internal transport message format, got (45,43," + Integer.toHexString(byte1 & 0xFF) + ","
    ...
    public void testTLSHeader() throws IOException {
        ...
        try {
            ...
            String expected = "SSL/TLS request received but SSL/TLS is not enabled on this node, got (16,3,"
                    + Integer.toHexString(byte1 & 0xFF) + "," + Integer.toHexString(byte2 & 0xFF) + ")";
            assertEquals(expected, ex.getMessage());
        }
    }
    ...
    public void testExceptionHandling() throws IllegalAccessException {
        ...
        testExceptionHandling(new FesenException("simulated"), new MockLogAppender.SeenEventExpectation("message",
                "org.codelibs.fesen.transport.TcpTransport", Level.WARN, "exception caught on transport layer [*], closing connection"));
        testExceptionHandling(new ClosedChannelException(),
                new MockLogAppender.SeenEventExpectation("message", "org.codelibs.fesen.transport.TcpTransport", Level.DEBUG,
        ...
        testExceptionHandling(new FesenException("Connection reset"),
                new MockLogAppender.SeenEventExpectation("message", "org.codelibs.fesen.transport.TcpTransport", Level.DEBUG,
                        "close connection exception caught on transport layer [*], disconnecting from relevant node"));
        testExceptionHandling(new BindException(), new MockLogAppender.SeenEventExpectation("message",
                "org.codelibs.fesen.transport.TcpTransport", Level.DEBUG, "bind exception caught on transport layer [*]"));
        ...
        testExceptionHandling(true, new TcpTransport.HttpRequestOnTransportException("test"), false,
                ...
                new MockLogAppender.UnseenEventExpectation("message", "org.codelibs.fesen.transport.TcpTransport", Level.INFO, "*"),
                new MockLogAppender.UnseenEventExpectation("message", "org.codelibs.fesen.transport.TcpTransport", Level.DEBUG, "*"));
        testExceptionHandling(new StreamCorruptedException("simulated"), new MockLogAppender.SeenEventExpectation("message",
                "org.codelibs.fesen.transport.TcpTransport", Level.WARN, "simulated, [*], closing connection"));
    }
    ...
    private void testExceptionHandling(boolean startTransport, Exception exception, boolean expectClosed,
        ...
        try {
            ...
            TcpTransport.handleException(channel, exception, lifecycle, new OutboundHandler(randomAlphaOfLength(10), Version.CURRENT,
                    new String[0], new StatsTracker(), testThreadPool, BigArrays.NON_RECYCLING_INSTANCE));
