...
public class BootstrapChecksTests extends AbstractBootstrapCheckTestCase {
    ...
    public void testEnforceLimitsWhenBoundToNonLocalAddress() {
        ...
        for (int i = 0; i < randomIntBetween(0, 7); i++) {
            final TransportAddress randomTransportAddress =
                    randomBoolean() ? buildNewFakeTransportAddress() : new TransportAddress(InetAddress.getLoopbackAddress(), i);
            transportAddresses.add(randomTransportAddress);
        }

        final TransportAddress publishAddress =
                randomBoolean() ? buildNewFakeTransportAddress() : new TransportAddress(InetAddress.getLoopbackAddress(), 0);
    ...
    public void testExceptionAggregation() {
        final List<BootstrapCheck> checks = Arrays.asList(context -> BootstrapCheck.BootstrapCheckResult.failure("first"),
                context -> BootstrapCheck.BootstrapCheckResult.failure("second"));

        final NodeValidationException e =
                expectThrows(NodeValidationException.class, () -> BootstrapChecks.check(emptyContext, true, checks));
    ...
    public void testMlockallCheck() throws NodeValidationException {
        ...
        for (final MlockallCheckTestCase testCase : testCases) {
            ...
            BootstrapContext bootstrapContext =
                    createTestContext(Settings.builder().put("bootstrap.memory_lock", testCase.mlockallSet).build(), null);
            if (testCase.shouldFail) {
                final NodeValidationException e = expectThrows(NodeValidationException.class,
                        () -> BootstrapChecks.check(bootstrapContext, true, Collections.singletonList(check)));
                assertThat(e.getMessage(), containsString("memory locking requested for fesen process but memory is not locked"));
            } else {
    ...
    public void testClientJvmCheck() throws NodeValidationException {
        ...
        final NodeValidationException e = expectThrows(NodeValidationException.class,
                () -> BootstrapChecks.check(emptyContext, true, Collections.singletonList(check)));
        assertThat(e.getMessage(), containsString("JVM is using the client VM [Java HotSpot(TM) 32-Bit Client VM] "
    ...
    public void testSystemCallFilterCheck() throws NodeValidationException {
        ...
        BootstrapContext context =
                randomBoolean() ? createTestContext(Settings.builder().put("bootstrap.system_call_filter", true).build(), null)
                        : emptyContext;
        ...
        assertThat(e.getMessage(), containsString("system call filters failed to install; "
    ...
    public void testMightForkCheck() throws NodeValidationException {
        ...
        runMightForkTest(check, isSystemCallFilterInstalled, () -> mightFork.set(false), () -> mightFork.set(true),
    ...
    public void testOnErrorCheck() throws NodeValidationException {
        ...
        runMightForkTest(check, isSystemCallFilterInstalled, () -> onError.set(randomBoolean() ? "" : null), () -> onError.set(command),
                e -> assertThat(e.getMessage(),
                        containsString("OnError [" + command + "] requires forking but is prevented by system call filters "
    ...
    public void testOnOutOfMemoryErrorCheck() throws NodeValidationException {
        ...
        final String command = randomAlphaOfLength(16);
        runMightForkTest(check, isSystemCallFilterInstalled, () -> onOutOfMemoryError.set(randomBoolean() ? "" : null),
                () -> onOutOfMemoryError.set(command),
                e -> assertThat(e.getMessage(),
                        containsString("OnOutOfMemoryError [" + command + "]"
                                + " requires forking but is prevented by system call filters ([bootstrap.system_call_filter=true]);"
                                + " upgrade to at least Java 8u92 and use ExitOnOutOfMemoryError")));
    }

    private void runMightForkTest(final BootstrapChecks.MightForkCheck check, final AtomicBoolean isSystemCallFilterInstalled,
            final Runnable disableMightFork, final Runnable enableMightFork, final Consumer<NodeValidationException> consumer)
    ...
    public void testAlwaysEnforcedChecks() {
        ...
        final NodeValidationException alwaysEnforced = expectThrows(NodeValidationException.class,
                () -> BootstrapChecks.check(emptyContext, randomBoolean(), Collections.singletonList(check)));
    ...
    public void testDiscoveryConfiguredCheck() throws NodeValidationException {
        ...
        final BootstrapContext zen1Context =
                createTestContext(Settings.builder().put(DiscoveryModule.DISCOVERY_TYPE_SETTING.getKey(), ZEN_DISCOVERY_TYPE).build(),
                        Metadata.EMPTY_METADATA);

        final BootstrapContext zen2Context =
                createTestContext(Settings.builder().put(DiscoveryModule.DISCOVERY_TYPE_SETTING.getKey(), ZEN2_DISCOVERY_TYPE).build(),
                        Metadata.EMPTY_METADATA);
        ...
        BootstrapChecks.check(createTestContext(Settings.builder()
                .put(DiscoveryModule.DISCOVERY_TYPE_SETTING.getKey(), randomFrom("single-node", randomAlphaOfLength(5))).build(),
                Metadata.EMPTY_METADATA), true, checks);

        assertThat(expectThrows(NodeValidationException.class, () -> BootstrapChecks.check(zen1Context, true, checks)),
                hasToString(containsString("discovery type [legacy-zen-for-testing-only-do-not-use] is unsuitable for production use")));

        final NodeValidationException e =
                expectThrows(NodeValidationException.class, () -> BootstrapChecks.check(zen2Context, true, checks));
        ...
        CheckedConsumer<Settings.Builder, NodeValidationException> ensureChecksPass = b -> {
            final BootstrapContext context = createTestContext(
                    b.put(DiscoveryModule.DISCOVERY_TYPE_SETTING.getKey(), ZEN2_DISCOVERY_TYPE).build(), Metadata.EMPTY_METADATA);
