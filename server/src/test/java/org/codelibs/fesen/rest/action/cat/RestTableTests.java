...
import org.codelibs.fesen.rest.action.cat.RestTable;
...
import static org.codelibs.fesen.rest.action.cat.RestTable.buildDisplayHeaders;
import static org.codelibs.fesen.rest.action.cat.RestTable.buildResponse;
...
public class RestTableTests extends ESTestCase {
    ...
    private static final String JSON_TABLE_BODY =
            "[{\"bulk.foo\":\"foo\",\"bulk.bar\":\"foo\",\"aliasedBulk\":\"foo\"," + "\"aliasedSecondBulk\":\"foo\",\"unmatched\":\"foo\","
    ...
    public void testCompareRow() {
        ...
        RestTable.TableIndexComparator comparator =
                new RestTable.TableIndexComparator(table, Collections.singletonList(new RestTable.ColumnOrderElement("compare", false)));
        assertTrue(comparator.compare(0, 1) < 0);
        assertTrue(comparator.compare(0, 2) == 0);
        assertTrue(comparator.compare(1, 2) > 0);

        RestTable.TableIndexComparator reverseComparator =
                new RestTable.TableIndexComparator(table, Collections.singletonList(new RestTable.ColumnOrderElement("compare", true)));
