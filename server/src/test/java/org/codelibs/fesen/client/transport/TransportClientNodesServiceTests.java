...
import org.codelibs.fesen.client.transport.TransportClientNodesService;
...
import static org.codelibs.fesen.test.transport.MockTransportService.createNewService;
...
public class TransportClientNodesServiceTests extends ESTestCase {
    ...
    private static class TestIteration implements Closeable {
                    ...
                    return new AsyncSender() {
                        ...
                        public <T extends TransportResponse> void sendRequest(Transport.Connection connection, String action,
                            ...
                            if (TransportLivenessAction.NAME.equals(action)) {
                                sender.sendRequest(connection, action, request, options,
        ...
        private <T extends TransportResponse> TransportResponseHandler wrapLivenessResponseHandler(TransportResponseHandler<T> handler,
                DiscoveryNode node, ClusterName clusterName) {
                ...
                public void handleResponse(T response) {
                    LivenessResponse livenessResponse = new LivenessResponse(clusterName,
                            new DiscoveryNode(node.getName(), node.getId(), node.getEphemeralId(), "liveness-hostname" + node.getId(),
                                    "liveness-hostaddress" + node.getId(), livenessAddress, node.getAttributes(), node.getRoles(),
                                    node.getVersion()));
        ...
        for (int i = 0; i < iters; i++) {
            try (TestIteration iteration = new TestIteration()) {
                ...
                iteration.transportClientNodesService.execute((node, retryListener) -> {
                    ...
                    iteration.transportService.sendRequest(node, "action", new TestRequest(), TransportRequestOptions.EMPTY,
    ...
    public void testSniffNodesSamplerClosesConnections() throws Exception {
        ...
        try (MockTransportService remoteService = createNewService(remoteSettings, Version.CURRENT, threadPool, null)) {
            ...
            Settings clientSettings = Settings.builder().put(TransportClient.CLIENT_TRANSPORT_SNIFF.getKey(), true)
                    .put(TransportClient.CLIENT_TRANSPORT_PING_TIMEOUT.getKey(), TimeValue.timeValueSeconds(1))
                    .put(TransportClient.CLIENT_TRANSPORT_NODES_SAMPLER_INTERVAL.getKey(), TimeValue.timeValueSeconds(30)).build();
            ...
            try (MockTransportService clientService = createNewService(clientSettings, Version.CURRENT, threadPool, null)) {
                ...
                clientService.addConnectBehavior(remoteService,
                        (transport, discoveryNode, profile, listener) -> transport.openConnection(discoveryNode, profile,
