...
import static org.codelibs.fesen.index.mapper.IdFieldMapper.ID_FIELD_DATA_DEPRECATION_MESSAGE;
...
public class IdFieldMapperTests extends ESSingleNodeTestCase {
    ...
    public void testIncludeInObjectNotAllowed() throws Exception {
        ...
        try {
            docMapper.parse(new SourceToParse("test", "type", "1",
                    BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("_id", "1").endObject()), XContentType.JSON));
    ...
    public void testEnableFieldData() throws IOException {
        ...
        client().admin().cluster().prepareUpdateSettings()
                .setTransientSettings(Settings.builder().put(IndicesService.INDICES_ID_FIELD_DATA_ENABLED_SETTING.getKey(), false)).get();
        try {
            ...
            client().admin().cluster().prepareUpdateSettings()
                    .setTransientSettings(Settings.builder().putNull(IndicesService.INDICES_ID_FIELD_DATA_ENABLED_SETTING.getKey())).get();
        }
    }
}
