...
public class MatchBoolPrefixQueryBuilderTests extends AbstractQueryTestCase<MatchBoolPrefixQueryBuilder> {
    ...
    protected MatchBoolPrefixQueryBuilder doCreateTestQueryBuilder() {
        ...
        final Object value = IntStream.rangeClosed(0, randomIntBetween(0, 3)).mapToObj(i -> randomAlphaOfLengthBetween(1, 10) + " ")
    ...
    protected void doAssertLuceneQuery(MatchBoolPrefixQueryBuilder queryBuilder, Query query, QueryShardContext context)
        ...
        if (query instanceof PrefixQuery) {
            ...
            final Set<Query> allQueriesExceptLast = IntStream.range(0, booleanQuery.clauses().size() - 1)
                    .mapToObj(booleanQuery.clauses()::get).map(BooleanClause::getQuery).collect(Collectors.toSet());
            assertThat(allQueriesExceptLast, anyOf(everyItem(instanceOf(TermQuery.class)), everyItem(instanceOf(SynonymQuery.class)),
    ...
    public void testFromSimpleJson() throws IOException {
        ...
        final String expected = "{" + "\"match_bool_prefix\": {" + "\"fieldName\": {" + "\"query\": \"fieldValue\","
                + "\"operator\": \"OR\"," + "\"prefix_length\": 0," + "\"max_expansions\": 50," + "\"fuzzy_transpositions\": true,"
                + "\"boost\": 1.0" + "}" + "}" + "}";
    ...
    public void testFromJson() throws IOException {
        final String expected = "{" + "\"match_bool_prefix\": {" + "\"fieldName\": {" + "\"query\": \"fieldValue\","
                + "\"analyzer\": \"simple\"," + "\"operator\": \"AND\"," + "\"minimum_should_match\": \"2\"," + "\"fuzziness\": \"1\","
                + "\"prefix_length\": 1," + "\"max_expansions\": 10," + "\"fuzzy_transpositions\": false,"
                + "\"fuzzy_rewrite\": \"constant_score\"," + "\"boost\": 2.0" + "}" + "}" + "}";
    ...
    public void testParseFailsWithMultipleFields() {
        {
            final String json = "{" + "\"match_bool_prefix\" : {" + "\"field_name_1\" : {" + "\"query\" : \"foo\"" + "},"
                    + "\"field_name_2\" : {" + "\"query\" : \"foo\"\n" + "}" + "}" + "}";
            final ParsingException e = expectThrows(ParsingException.class, () -> parseQuery(json));
            assertEquals("[match_bool_prefix] query doesn't support multiple fields, found [field_name_1] and [field_name_2]",
                    e.getMessage());
        }
    ...
    public void testAnalysis() throws Exception {
        ...
        assertBooleanQuery(query, asList(new TermQuery(new Term(TEXT_FIELD_NAME, "foo")), new TermQuery(new Term(TEXT_FIELD_NAME, "bar")),
    ...
    public void testAnalysisSynonym() throws Exception {
        ...
        assertBooleanQuery(query,
                asList(new TermQuery(new Term(TEXT_FIELD_NAME, "fox")),
