...
public class RangeFieldQueryStringQueryBuilderTests extends AbstractQueryTestCase<QueryStringQueryBuilder> {
    ...
    protected void initializeAdditionalMappings(MapperService mapperService) throws IOException {
        mapperService.merge("_doc",
                new CompressedXContent(Strings.toString(PutMappingRequest.buildFromSimplifiedDef("_doc", INTEGER_RANGE_FIELD_NAME,
                        "type=integer_range", LONG_RANGE_FIELD_NAME, "type=long_range", FLOAT_RANGE_FIELD_NAME, "type=float_range",
                        DOUBLE_RANGE_FIELD_NAME, "type=double_range", DATE_RANGE_FIELD_NAME, "type=date_range", IP_RANGE_FIELD_NAME,
                        "type=ip_range"))),
    ...
    public void testIntegerRangeQuery() throws Exception {
        ...
        Query dv = RangeType.INTEGER.dvRangeQuery(INTEGER_RANGE_FIELD_NAME, BinaryDocValuesRangeQuery.QueryType.INTERSECTS, -450, 45000,
                true, true);
    ...
    public void testLongRangeQuery() throws Exception {
        ...
        Query dv =
                RangeType.LONG.dvRangeQuery(LONG_RANGE_FIELD_NAME, BinaryDocValuesRangeQuery.QueryType.INTERSECTS, -450, 45000, true, true);
        assertEquals(new IndexOrDocValuesQuery(range, dv), query);
    }
    ...
    public void testFloatRangeQuery() throws Exception {
        ...
        Query dv = RangeType.FLOAT.dvRangeQuery(FLOAT_RANGE_FIELD_NAME, BinaryDocValuesRangeQuery.QueryType.INTERSECTS, -450.0f, 45000.0f,
                true, true);
        assertEquals(new IndexOrDocValuesQuery(range, dv), query);
    }
    ...
    public void testDoubleRangeQuery() throws Exception {
        ...
        Query dv = RangeType.DOUBLE.dvRangeQuery(DOUBLE_RANGE_FIELD_NAME, BinaryDocValuesRangeQuery.QueryType.INTERSECTS, -450.0, 45000.0,
                true, true);
        assertEquals(new IndexOrDocValuesQuery(range, dv), query);
    }
    ...
    public void testDateRangeQuery() throws Exception {
        ...
        Query range =
                LongRange.newIntersectsQuery(DATE_RANGE_FIELD_NAME, new long[] { parser.parse(lowerBoundExact, () -> 0).toEpochMilli() },
                        new long[] { parser.parse(upperBoundExact, () -> 0).toEpochMilli() });
        Query dv = RangeType.DATE.dvRangeQuery(DATE_RANGE_FIELD_NAME, BinaryDocValuesRangeQuery.QueryType.INTERSECTS,
                parser.parse(lowerBoundExact, () -> 0).toEpochMilli(), parser.parse(upperBoundExact, () -> 0).toEpochMilli(), true, true);
        assertEquals(new IndexOrDocValuesQuery(range, dv), query);
        ...
        Query controlQuery = LongPoint.newRangeQuery(DATE_FIELD_NAME, new long[] { parser.parse(lowerBoundExact, () -> 0).toEpochMilli() },
                new long[] { parser.parse(upperBoundExact, () -> 0).toEpochMilli() });

        Query controlDv = SortedNumericDocValuesField.newSlowRangeQuery(DATE_FIELD_NAME,
                parser.parse(lowerBoundExact, () -> 0).toEpochMilli(), parser.parse(upperBoundExact, () -> 0).toEpochMilli());
