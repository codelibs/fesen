...
import org.codelibs.fesen.index.engine.RecoverySourcePruneMergePolicy;
...
public class RecoverySourcePruneMergePolicyTests extends ESTestCase {
    ...
    public void testPruneAll() throws IOException {
        try (Directory dir = newDirectory()) {
            ...
            RecoverySourcePruneMergePolicy mp =
                    new RecoverySourcePruneMergePolicy("extra_source", MatchNoDocsQuery::new, newLogMergePolicy());
                ...
                try (DirectoryReader reader = DirectoryReader.open(writer)) {
                    ...
                    if (leafReader instanceof CodecReader && reader instanceof StandardDirectoryReader) {
                        ...
                        MergePolicy.MergeSpecification forcedMerges =
                                mp.findForcedDeletesMerges(segmentInfos, new MergePolicy.MergeContext() {
                                    @Override
    ...
    public void testPruneSome() throws IOException {
        try (Directory dir = newDirectory()) {
            IndexWriterConfig iwc = newIndexWriterConfig();
            iwc.setMergePolicy(new RecoverySourcePruneMergePolicy("extra_source", () -> new TermQuery(new Term("even", "true")),
                    iwc.getMergePolicy()));
            try (IndexWriter writer = new IndexWriter(dir, iwc)) {
                for (int i = 0; i < 20; i++) {
                    if (i > 0 && randomBoolean()) {
