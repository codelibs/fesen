...
import org.codelibs.fesen.index.mapper.BinaryRangeUtil;
...
public class BinaryRangeUtilTests extends ESTestCase {
    ...
    public void testDecodeDoubleRanges() throws IOException {
        ...
        for (int i = 0; i < iters; i++) {
            ...
            List<RangeFieldMapper.Range> decoded =
                    BinaryRangeUtil.decodeDoubleRanges(BinaryRangeUtil.encodeDoubleRanges(singleton(expected)));
            assertEquals(1, decoded.size());
            RangeFieldMapper.Range actual = decoded.get(0);
            assertEquals(expected, actual);
        }
    ...
    public void testDecodeFloatRanges() throws IOException {
        ...
        for (int i = 0; i < iters; i++) {
            ...
            List<RangeFieldMapper.Range> decoded =
                    BinaryRangeUtil.decodeFloatRanges(BinaryRangeUtil.encodeFloatRanges(singleton(expected)));
            assertEquals(1, decoded.size());
            RangeFieldMapper.Range actual = decoded.get(0);
            assertEquals(expected, actual);
        }
