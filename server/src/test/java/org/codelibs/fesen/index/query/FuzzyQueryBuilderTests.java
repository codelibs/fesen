...
public class FuzzyQueryBuilderTests extends AbstractQueryTestCase<FuzzyQueryBuilder> {
    ...
    public void testToQueryWithStringFieldDefinedWrongFuzziness() throws IOException {
        ...
        FesenParseException e =
                expectThrows(FesenParseException.class, () -> parseQuery(queryMissingFuzzinessUpLimit).toQuery(createShardContext()));
        ...
        String queryMissingFuzzinessUpLimit2 = "{\n" + "    \"fuzzy\":{\n" + "        \"" + TEXT_FIELD_NAME + "\":{\n"
                + "            \"value\":\"sh\",\n" + "            \"fuzziness\": \"AUTO:1,\",\n" + "            \"prefix_length\":1,\n"
                + "            \"boost\":2.0\n" + "        }\n" + "    }\n" + "}";
        e = expectThrows(FesenParseException.class, () -> parseQuery(queryMissingFuzzinessUpLimit2).toQuery(createShardContext()));
        assertTrue(e.getMessage() + " didn't contain: " + msg + " but: " + e.getMessage(), e.getMessage().contains(msg));

        String queryMissingFuzzinessLowLimit = "{\n" + "    \"fuzzy\":{\n" + "        \"" + TEXT_FIELD_NAME + "\":{\n"
                + "            \"value\":\"sh\",\n" + "            \"fuzziness\": \"AUTO:,5\",\n" + "            \"prefix_length\":1,\n"
                + "            \"boost\":2.0\n" + "        }\n" + "    }\n" + "}";
        e = expectThrows(FesenParseException.class, () -> parseQuery(queryMissingFuzzinessLowLimit).toQuery(createShardContext()));
