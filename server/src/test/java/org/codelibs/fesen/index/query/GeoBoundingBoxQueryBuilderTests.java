...
import org.codelibs.fesen.index.query.GeoExecType;
...
public class GeoBoundingBoxQueryBuilderTests extends AbstractQueryTestCase<GeoBoundingBoxQueryBuilder> {
    ...
    protected GeoBoundingBoxQueryBuilder doCreateTestQueryBuilder() {
        ...
        if (randomBoolean()) {
            ...
            case 0:
                ...
                break;
            case 1:
                builder.setCorners(GeohashUtils.encodeLatLon(box.getMaxY(), box.getMinX()),
            ...
            if (randomBoolean()) {
                ...
                builder.setCornersOGC(GeohashUtils.encodeLatLon(box.getMinY(), box.getMinX()),
    ...
    public void testValidation() {
        ...
        for (PointTester tester : testers) {
            ...
            assertNull("validation w/ coerce should ignore invalid " + tester.getClass().getName() + " coordinate: "
                    + tester.invalidCoordinate + " ", except);

            tester.invalidateCoordinate(builder.setValidationMethod(GeoValidationMethod.STRICT), false);
            except = builder.checkLatLon();
            assertNotNull("validation w/o coerce should detect invalid coordinate: " + tester.getClass().getName() + " coordinate: "
                    + tester.invalidCoordinate, except);
        }
    }
    ...
    protected void doAssertLuceneQuery(GeoBoundingBoxQueryBuilder queryBuilder, Query query, QueryShardContext context) throws IOException {
        ...
        if (fieldType == null) {
            ...
            assertEquals(LatLonPoint.newBoxQuery(expectedFieldName, queryBuilder.bottomRight().lat(), queryBuilder.topLeft().lat(),
                    queryBuilder.topLeft().lon(), queryBuilder.bottomRight().lon()), indexQuery);
            Query dvQuery = ((IndexOrDocValuesQuery) query).getRandomAccessQuery();
            assertEquals(LatLonDocValuesField.newSlowBoxQuery(expectedFieldName, queryBuilder.bottomRight().lat(),
                    queryBuilder.topLeft().lat(), queryBuilder.topLeft().lon(), queryBuilder.bottomRight().lon()), dvQuery);
        }
    }
