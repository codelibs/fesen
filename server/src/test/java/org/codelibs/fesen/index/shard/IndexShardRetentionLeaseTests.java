...
public class IndexShardRetentionLeaseTests extends IndexShardTestCase {
    ...
    public void testAddOrRenewRetentionLease() throws IOException {
        final IndexShard indexShard =
                newStartedShard(true, Settings.builder().put(IndexSettings.INDEX_SOFT_DELETES_SETTING.getKey(), true).build());
    ...
    public void testRemoveRetentionLease() throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                ...
                assertRetentionLeases(indexShard, length - i - 1, minimumRetainingSequenceNumbers, primaryTerm, 2 + length + i, true,
                        false);
            }
        } finally {
            closeShards(indexShard);
    ...
    private RetentionLease peerRecoveryRetentionLease(IndexShard indexShard) {
        return new RetentionLease(ReplicationTracker.getPeerRecoveryRetentionLeaseId(indexShard.routingEntry()), 0, currentTimeMillis.get(),
    ...
    private void runExpirationTest(final boolean primary) throws IOException {
        ...
        final Settings settings = Settings.builder().put(IndexSettings.INDEX_SOFT_DELETES_SETTING.getKey(), true)
        ...
        try {
            ...
            if (primary) {
                ...
                final RetentionLeases retentionLeases =
                        new RetentionLeases(primaryTerm, initialVersion, Arrays.asList(peerRecoveryRetentionLease(indexShard),
            ...
            if (primary) {
                ...
                final RetentionLeases retentionLeases =
                        new RetentionLeases(primaryTerm, initialVersion + 1, Arrays.asList(peerRecoveryRetentionLease(indexShard),
                                new RetentionLease("0", retainingSequenceNumbers[0], currentTimeMillis.get(), "test-0")));
                indexShard.updateRetentionLeasesOnReplica(retentionLeases);
            }
            ...
            currentTimeMillis
                    .set(currentTimeMillis.get() + randomLongBetween(retentionLeaseMillis, Long.MAX_VALUE - currentTimeMillis.get()));
            if (primary) {
    ...
    public void testPersistence() throws IOException {
        ...
        try {
            ...
            final IndexShard recoveredShard = reinitShard(indexShard,
            ...
            final IndexShard forceRecoveredShard = reinitShard(indexShard, ShardRoutingHelper.initWithSameId(indexShard.routingEntry(),
    ...
    public void testRetentionLeaseStats() throws IOException {
        ...
        try {
            ...
            assertRetentionLeases(stats.retentionLeases(), length, minimumRetainingSequenceNumbers, indexShard.getOperationPrimaryTerm(),
                    length + 1);
        } finally {
            closeShards(indexShard);
        }
    ...
    public void testRetentionLeasesActionsFailWithSoftDeletesDisabled() throws Exception {
        ...
        assertThat(
                expectThrows(AssertionError.class,
                        () -> shard.addRetentionLease(randomAlphaOfLength(10),
        ...
        assertThat(
                expectThrows(AssertionError.class,
                        () -> shard.renewRetentionLease(randomAlphaOfLength(10),
                                randomLongBetween(SequenceNumbers.NO_OPS_PERFORMED, Long.MAX_VALUE), "test")).getMessage(),
                equalTo("retention leases requires soft deletes but [index] does not have soft deletes enabled"));
        assertThat(
    ...
    private void assertRetentionLeases(final IndexShard indexShard, final int size, final long[] minimumRetainingSequenceNumbers,
            final long primaryTerm, final long version, final boolean primary, final boolean expireLeases) {
    ...
    private void assertRetentionLeases(final RetentionLeases retentionLeases, final int size, final long[] minimumRetainingSequenceNumbers,
            final long primaryTerm, final long version) {
