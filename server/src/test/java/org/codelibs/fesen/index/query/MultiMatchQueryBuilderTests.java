...
import org.codelibs.fesen.index.query.MultiMatchQueryBuilder.Type;
...
public class MultiMatchQueryBuilderTests extends AbstractQueryTestCase<MultiMatchQueryBuilder> {
    ...
    protected MultiMatchQueryBuilder doCreateTestQueryBuilder() {
        ...
        if (randomBoolean()) {
            if (fieldName.equals(TEXT_FIELD_NAME) || fieldName.equals(TEXT_ALIAS_FIELD_NAME) || fieldName.equals(KEYWORD_FIELD_NAME)) {
                ...
                query.type(randomValueOtherThanMany((type) -> type == Type.PHRASE_PREFIX || type == Type.BOOL_PREFIX,
    ...
    protected void doAssertLuceneQuery(MultiMatchQueryBuilder queryBuilder, Query query, QueryShardContext context) throws IOException {
        ...
        assertThat(query,
                anyOf(Arrays.asList(instanceOf(BoostQuery.class), instanceOf(TermQuery.class), instanceOf(BooleanQuery.class),
                        instanceOf(DisjunctionMaxQuery.class), instanceOf(FuzzyQuery.class), instanceOf(MultiPhrasePrefixQuery.class),
                        instanceOf(MatchAllDocsQuery.class), instanceOf(ExtendedCommonTermsQuery.class), instanceOf(MatchNoDocsQuery.class),
                        instanceOf(PhraseQuery.class), instanceOf(PointRangeQuery.class), instanceOf(IndexOrDocValuesQuery.class),
    ...
    public void testToQueryMultipleFieldsDisableDismax() throws Exception {
        Query query =
                multiMatchQuery("test").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).tieBreaker(1.0f).toQuery(createShardContext());
    ...
    public void testDefaultField() throws Exception {
        ...
        try {
            ...
            DisjunctionMaxQuery expected = new DisjunctionMaxQuery(Arrays.asList(new TermQuery(new Term(TEXT_FIELD_NAME, "hello")),
                    new BoostQuery(new TermQuery(new Term(KEYWORD_FIELD_NAME, "hello")), 5.0f)), 0.0f);
            assertEquals(expected, query);

            context.getIndexSettings().updateIndexMetadata(newIndexMeta("index", context.getIndexSettings().getSettings(), Settings
                    .builder().putList("index.query.default_field", TEXT_FIELD_NAME, KEYWORD_FIELD_NAME + "^5", INT_FIELD_NAME).build()));
    ...
    public void testWithStopWords() throws Exception {
        ...
        query = new MultiMatchQueryBuilder("the quick fox").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).analyzer("stop")
        ...
        expected = new DisjunctionMaxQuery(Arrays.asList(
                ...
                new BooleanQuery.Builder().add(new TermQuery(new Term(KEYWORD_FIELD_NAME, "quick")), BooleanClause.Occur.SHOULD)
                        .add(new TermQuery(new Term(KEYWORD_FIELD_NAME, "fox")), BooleanClause.Occur.SHOULD).build()),
                0f);
        assertEquals(expected, query);

        query = new MultiMatchQueryBuilder("the").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).analyzer("stop")
                .toQuery(createShardContext());
        ...
        query = new BoolQueryBuilder()
                .should(new MultiMatchQueryBuilder("the").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).analyzer("stop"))
                .toQuery(createShardContext());
    ...
    public void testNegativeFieldBoost() {
        IllegalArgumentException exc = expectThrows(IllegalArgumentException.class, () -> new MultiMatchQueryBuilder("the quick fox")
