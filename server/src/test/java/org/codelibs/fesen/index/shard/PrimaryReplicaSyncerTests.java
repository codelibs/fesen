...
public class PrimaryReplicaSyncerTests extends IndexShardTestCase {
    ...
    public void testSyncerSendsOffCorrectDocuments() throws Exception {
        ...
        for (int i = 0; i < numDocs; i++) {
            ...
            shard.applyIndexOperationOnPrimary(Versions.MATCH_ANY, VersionType.INTERNAL,
                    ...
                    SequenceNumbers.UNASSIGNED_SEQ_NO, 0, IndexRequest.UNSET_AUTO_GENERATED_TIMESTAMP, true);
        }
        ...
        if (syncNeeded) {
            ...
            assertThat("trimAboveSeqNo has to be specified in request #0 only",
                    resyncRequests.stream().mapToLong(ResyncReplicationRequest::getTrimAboveSeqNo)
                            .filter(seqNo -> seqNo != SequenceNumbers.UNASSIGNED_SEQ_NO).findFirst().isPresent(),
                    is(false));
    ...
    public void testSyncerOnClosingShard() throws Exception {
        ...
        PrimaryReplicaSyncer syncer =
                new PrimaryReplicaSyncer(new TaskManager(Settings.EMPTY, threadPool, Collections.emptySet()), syncAction);
    ...
    public void testDoNotSendOperationsWithoutSequenceNumber() throws Exception {
        ...
        for (int i = 0; i < numOps; i++) {
            operations.add(new Translog.Index("_doc", Integer.toString(i), randomBoolean() ? SequenceNumbers.UNASSIGNED_SEQ_NO : i,
                    primaryTerm, new byte[] { 1 }));
        }
    ...
    public void testStatusSerializationAsNamedWriteable() throws IOException {
        ...
        try (BytesStreamOutput out = new BytesStreamOutput()) {
            ...
            try (StreamInput in =
                    new NamedWriteableAwareStreamInput(new ByteBufferStreamInput(ByteBuffer.wrap(out.bytes().toBytesRef().bytes)),
