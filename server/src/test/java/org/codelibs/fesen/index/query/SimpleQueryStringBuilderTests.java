...
import org.codelibs.fesen.index.query.SimpleQueryStringFlag;
...
public class SimpleQueryStringBuilderTests extends AbstractQueryTestCase<SimpleQueryStringBuilder> {
    ...
    protected SimpleQueryStringBuilder doCreateTestQueryBuilder() {
        ...
        String queryText =
                randomValueOtherThanMany(s -> s.toLowerCase(Locale.ROOT).contains("now"), () -> randomAlphaOfLengthBetween(1, 10));
    ...
    public void testDefaults() {
        ...
        assertEquals("Wrong default default fuzzy prefix length field.", FuzzyQuery.defaultPrefixLength,
        ...
        assertEquals("Wrong default default fuzzy max expansions field.", FuzzyQuery.defaultMaxExpansions,
        ...
        assertEquals("Wrong default default fuzzy transpositions field.", FuzzyQuery.defaultTranspositions,
    ...
    public void testAnalyzerWithGraph() {
        ...
        for (Operator op : Operator.values()) {
            ...
            Query expectedQuery =
                    new BooleanQuery.Builder().add(new BooleanClause(new TermQuery(new Term(TEXT_FIELD_NAME, "guinea")), defaultOp))
            ...
            expectedQuery = new SpanOrQuery(new SpanNearQuery(new SpanQuery[] { span1, span2 }, 0, true),
            ...
            PhraseQuery pq1 = new PhraseQuery.Builder().add(new Term(TEXT_FIELD_NAME, "tiny")).add(new Term(TEXT_FIELD_NAME, "guinea"))
                    .add(new Term(TEXT_FIELD_NAME, "pig")).setSlop(2).build();
            PhraseQuery pq2 = new PhraseQuery.Builder().add(new Term(TEXT_FIELD_NAME, "tiny")).add(new Term(TEXT_FIELD_NAME, "cavy"))
                    .setSlop(2).build();

            expectedQuery = new BooleanQuery.Builder().add(new TermQuery(new Term(TEXT_FIELD_NAME, "big")), defaultOp)
                    .add(new BooleanQuery.Builder().add(pq1, BooleanClause.Occur.SHOULD).add(pq2, BooleanClause.Occur.SHOULD).build(),
                            defaultOp)
                    .build();
            assertThat(query, equalTo(expectedQuery));
        }
    ...
    public void testQuoteFieldSuffix() {
        ...
        parser = new SimpleQueryStringQueryParser(new MockSynonymAnalyzer(), Collections.singletonMap(TEXT_FIELD_NAME, 1.0f), -1, settings,
                createShardContext());
        assertEquals(new TermQuery(new Term(TEXT_FIELD_NAME, "bar")), parser.parse("bar"));
        assertEquals(new TermQuery(new Term(TEXT_FIELD_NAME, "bar")), parser.parse("\"bar\""));
    }
    ...
    public void testDefaultField() throws Exception {
        ...
        try {
            ...
            Query expected = new DisjunctionMaxQuery(Arrays.asList(new TermQuery(new Term(TEXT_FIELD_NAME, "hello")),
                    new BoostQuery(new TermQuery(new Term(KEYWORD_FIELD_NAME, "hello")), 5.0f)), 1.0f);
            assertEquals(expected, query);
        } finally {
    ...
    public void testToFuzzyQuery() throws Exception {
        Query query = new SimpleQueryStringBuilder("text~2").field(TEXT_FIELD_NAME).fuzzyPrefixLength(2).fuzzyMaxExpansions(5)
    ...
    public void testLenientToPrefixQuery() throws Exception {
        Query query = new SimpleQueryStringBuilder("t*").field(DATE_FIELD_NAME).field(TEXT_FIELD_NAME).lenient(true)
    ...
    public void testWithStopWords() throws Exception {
        ...
        query = new SimpleQueryStringBuilder("the quick fox").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).analyzer("stop")
                .toQuery(createShardContext());
        expected = new BooleanQuery.Builder()
                .add(new DisjunctionMaxQuery(Arrays.asList(new TermQuery(new Term(TEXT_FIELD_NAME, "quick")),
                        new TermQuery(new Term(KEYWORD_FIELD_NAME, "quick"))), 1.0f), BooleanClause.Occur.SHOULD)
        ...
        query = new SimpleQueryStringBuilder("the").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).analyzer("stop")
                .toQuery(createShardContext());
        ...
        query = new BoolQueryBuilder()
                .should(new SimpleQueryStringBuilder("the").field(TEXT_FIELD_NAME).field(KEYWORD_FIELD_NAME).analyzer("stop"))
                .toQuery(createShardContext());
        assertEquals(expected, query);
    }
    ...
    public void testWithPrefixStopWords() throws Exception {
        ...
        BooleanQuery expected =
                new BooleanQuery.Builder().add(new PrefixQuery(new Term(TEXT_FIELD_NAME, "the")), BooleanClause.Occur.SHOULD)
    ...
    public void testUnmappedFieldNoTokenWithAndOperator() throws IOException {
        Query query = new SimpleQueryStringBuilder("first & second").field(TEXT_FIELD_NAME).field("unmapped").field("another_unmapped")
        ...
        BooleanQuery expected = new BooleanQuery.Builder().add(new TermQuery(new Term(TEXT_FIELD_NAME, "first")), BooleanClause.Occur.MUST)
                .add(new TermQuery(new Term(TEXT_FIELD_NAME, "second")), BooleanClause.Occur.MUST).build();
        assertEquals(expected, query);
        query = new SimpleQueryStringBuilder("first & second").field("unmapped").field("another_unmapped").defaultOperator(Operator.AND)
                .toQuery(createShardContext());
        expected = new BooleanQuery.Builder().add(new MatchNoDocsQuery(), BooleanClause.Occur.MUST)
                .add(new MatchNoDocsQuery(), BooleanClause.Occur.MUST).add(new MatchNoDocsQuery(), BooleanClause.Occur.MUST).build();
        assertEquals(expected, query);
    }

    public void testNegativeFieldBoost() {
        IllegalArgumentException exc = expectThrows(IllegalArgumentException.class, () -> new SimpleQueryStringBuilder("the quick fox")
