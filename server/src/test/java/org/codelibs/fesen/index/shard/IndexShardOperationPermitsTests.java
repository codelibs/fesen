...
public class IndexShardOperationPermitsTests extends ESTestCase {
    ...
    public static void setupThreadPool() {
        ...
        threadPool = new TestThreadPool("IndexShardOperationPermitsTests",
                Settings.builder().put("thread_pool." + ThreadPool.Names.WRITE + ".size", writeThreadPoolSize)
                        .put("thread_pool." + ThreadPool.Names.WRITE + ".queue_size", writeThreadPoolQueueSize).build());
    ...
    public void testNoPermitsRemaining() throws InterruptedException {
        ...
        final IllegalStateException e =
                expectThrows(IllegalStateException.class, () -> this.permits.acquire(new ActionListener<Releasable>() {
                    @Override
                    public void onResponse(Releasable releasable) {
                        assert false;
                    }
     ...
     */
    private Runnable controlledAcquire(final CyclicBarrier barrier, final CountDownLatch operationExecutingLatch,
            final CountDownLatch operationLatch, final CountDownLatch operationCompleteLatch) {
        return () -> {
            try {
                barrier.await();
            } catch (final BrokenBarrierException | InterruptedException e) {
