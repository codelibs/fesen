...
import org.codelibs.fesen.index.shard.ShardSplittingQuery;
...
public class ShardSplittingQueryTests extends ESTestCase {
    ...
    public void testSplitOnID() throws IOException {
        ...
        IndexMetadata metadata =
                IndexMetadata.builder("test").settings(Settings.builder().put(IndexMetadata.SETTING_VERSION_CREATED, Version.CURRENT))
        ...
        for (int j = 0; j < numDocs; j++) {
            ...
            if (hasNested) {
                ...
                for (int i = 0; i < numNested; i++) {
                    docs.add(Arrays.asList(new StringField(IdFieldMapper.NAME, Uid.encodeId(Integer.toString(j)), Field.Store.YES),
                ...
                writer.addDocument(Arrays.asList(new StringField(IdFieldMapper.NAME, Uid.encodeId(Integer.toString(j)), Field.Store.YES),
                        new SortedNumericDocValuesField("shard_id", shardId), sequenceIDFields.primaryTerm));
            }
        }
        writer.commit();
    ...
    public void testSplitOnIdOrRouting() throws IOException {
        ...
        for (int j = 0; j < numDocs; j++) {
            ...
            if (randomBoolean()) {
                ...
                rootDoc = Arrays.asList(new StringField(IdFieldMapper.NAME, Uid.encodeId(Integer.toString(j)), Field.Store.YES),
                        new StringField(RoutingFieldMapper.NAME, routing, Field.Store.YES),
                        new SortedNumericDocValuesField("shard_id", shardId), sequenceIDFields.primaryTerm);
            } else {
    ...
    public void testSplitOnRoutingPartitioned() throws IOException {
        ...
        IndexMetadata metadata = IndexMetadata.builder("test")
                .settings(Settings.builder().put(IndexMetadata.SETTING_VERSION_CREATED, Version.CURRENT)).numberOfShards(numShards)
                .setRoutingNumShards(numShards * 1000000).routingPartitionSize(randomIntBetween(1, 10)).numberOfReplicas(0).build();
        boolean hasNested = randomBoolean();
        int targetShardId = randomIntBetween(0, numShards - 1);
        for (int j = 0; j < numDocs; j++) {
            String routing = randomRealisticUnicodeOfCodepointLengthBetween(1, 5);
