...
public class CompletionFieldMapperTests extends MapperTestCase {
    ...
    public void testParsingFailure() throws Exception {
        ...
        MapperParsingException e =
                expectThrows(MapperParsingException.class, () -> defaultMapper.parse(source(b -> b.field("field", 1.0))));
    ...
    public void testKeywordWithSubCompletionAndContext() throws Exception {
        ...
        assertThat(indexableFields.getFields("field"), arrayContainingInAnyOrder(keywordField("key1"), sortedSetDocValuesField("key1"),
                keywordField("key2"), sortedSetDocValuesField("key2"), keywordField("key3"), sortedSetDocValuesField("key3")));
        assertThat(indexableFields.getFields("field.subsuggest"),
    ...
    public void testCompletionWithContextAndSubCompletion() throws Exception {
...

        {
            ...
            ParseContext.Document indexableFields = parsedDocument.rootDoc();
            assertThat(indexableFields.getFields("field"),
    ...
    public void testCompletionTypeWithSubKeywordFieldAndObjectInsert() throws Exception {
        ...
        ParseContext.Document indexableFields = parsedDocument.rootDoc();
        assertThat(indexableFields.getFields("field"), arrayContainingInAnyOrder(suggestField("New York"), suggestField("NY")));
        assertThat(indexableFields.getFields("field.analyzed"), arrayContainingInAnyOrder(keywordField("New York"),
                sortedSetDocValuesField("New York"), keywordField("NY"), sortedSetDocValuesField("NY")));
        //unable to assert about weight, covered in a REST test
    }
    ...
    public void testCompletionTypeWithSubKeywordFieldAndStringInsert() throws Exception {
        ...
        ParseContext.Document indexableFields = parsedDocument.rootDoc();
        assertThat(indexableFields.getFields("field"), arrayContainingInAnyOrder(suggestField("suggestion")));
        assertThat(indexableFields.getFields("field.analyzed"),
    ...
    public void testParsingMixed() throws Exception {
        ...
        IndexableField[] fields = parsedDocument.rootDoc().getFields(fieldMapper.name());
        assertThat(fields, arrayContainingInAnyOrder(suggestField("suggestion1"), suggestField("suggestion2"), suggestField("suggestion3"),
                suggestField("suggestion4"), suggestField("suggestion5"), suggestField("suggestion6")));
    }
    ...
    public void testFuzzyQueryType() throws Exception {
        ...
        Mapper fieldMapper = defaultMapper.mappers().getMapper("field");
        CompletionFieldMapper completionFieldMapper = (CompletionFieldMapper) fieldMapper;
        Query prefixQuery = completionFieldMapper.fieldType().fuzzyQuery("co", Fuzziness.fromEdits(FuzzyCompletionQuery.DEFAULT_MAX_EDITS),
    ...
    public void testRegexQueryType() throws Exception {
        ...
        CompletionFieldMapper completionFieldMapper = (CompletionFieldMapper) fieldMapper;
        Query prefixQuery =
                completionFieldMapper.fieldType().regexpQuery(new BytesRef("co"), RegExp.ALL, Operations.DEFAULT_MAX_DETERMINIZED_STATES);
