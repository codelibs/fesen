...
import org.codelibs.fesen.index.shard.IndexShardTests;
...
public class IndexLevelReplicationTests extends ESIndexLevelReplicationTestCase {
        ...
        try (ReplicationGroup shards = createGroup(randomInt(3))) {
            ...
            for (IndexShard shard : shards) {
                ...
                if (shardRouting.primary()) {
                    ...
                    globalCheckpointMatcher =
                            numDocs == 0 ? equalTo(SequenceNumbers.NO_OPS_PERFORMED) : anyOf(equalTo(numDocs - 1L), equalTo(numDocs - 2L));
                }
            ...
            for (IndexShard shard : shards) {
                ...
                final ShardRouting shardRouting = shard.routingEntry();
                assertThat(shardRouting + " local checkpoint mismatch", shardStats.getLocalCheckpoint(), equalTo(numDocs - 1L));
                assertThat(shardRouting + " global checkpoint mismatch", shardStats.getGlobalCheckpoint(),
    ...
    public void testDocumentFailureReplication() throws Exception {
        ...
        try (ReplicationGroup shards = new ReplicationGroup(buildIndexMetadata(0)) {
            ...
            for (IndexShard shard : shards) {
                ...
                try (Translog.Snapshot snapshot = shard.getHistoryOperations("test",
                        shard.indexSettings().isSoftDeleteEnabled() ? Engine.HistorySource.INDEX : Engine.HistorySource.TRANSLOG, 0)) {
                    assertThat(snapshot, SnapshotMatchers.containsOperationsInAnyOrder(expectedTranslogOps));
                }
            }
    ...
    public void testRequestFailureReplication() throws Exception {
        try (ReplicationGroup shards = createGroup(0)) {
            ...
            BulkItemResponse response =
                    shards.index(new IndexRequest(index.getName(), "type", "1").source("{}", XContentType.JSON).version(2));
    ...
    public void testSeqNoCollision() throws Exception {
        try (ReplicationGroup shards = createGroup(2,
                Settings.builder().put(IndexSettings.INDEX_SOFT_DELETES_SETTING.getKey(), true)
    ...
    public void testLateDeliveryAfterGCTriggeredOnReplica() throws Exception {
        ...
        try (ReplicationGroup shards = createGroup(1)) {
            ...
            final BulkShardRequest indexRequest =
                    indexOnPrimary(new IndexRequest(index.getName(), "type", "d1").source("{}", XContentType.JSON), primary);
    ...
    private void updateGCDeleteCycle(IndexShard shard, TimeValue interval) {
        ...
        builder.settings(Settings.builder().put(shard.indexSettings().getSettings()).put(IndexSettings.INDEX_GC_DELETES_SETTING.getKey(),
    ...
    public void testOutOfOrderDeliveryForAppendOnlyOperations() throws Exception {
        try (ReplicationGroup shards = createGroup(1)) {
            ...
            final BulkShardRequest indexRequest =
                    indexOnPrimary(new IndexRequest(index.getName(), "type", null).source("{}", XContentType.JSON), primary);
