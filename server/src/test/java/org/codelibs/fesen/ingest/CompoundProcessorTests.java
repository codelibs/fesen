...
public class CompoundProcessorTests extends ESTestCase {
    ...
    public void testSingleProcessorWithException() throws Exception {
        ...
        assertThat(((FesenException) holder[0]).getRootCause().getMessage(), equalTo("error"));
        assertThat(processor.getInvokedCounter(), equalTo(1));
    ...
    public void testSingleProcessorWithOnFailureProcessor() throws Exception {
        ...
        when(relativeTimeProvider.getAsLong()).thenReturn(0L, TimeUnit.MILLISECONDS.toNanos(1));
        CompoundProcessor compoundProcessor =
                new CompoundProcessor(false, singletonList(processor1), singletonList(processor2), relativeTimeProvider);
        compoundProcessor.execute(ingestDocument, (result, e) -> {});
        verify(relativeTimeProvider, times(2)).getAsLong();
    ...
    public void testSingleProcessorWithNestedFailures() throws Exception {
        ...
        LongSupplier relativeTimeProvider = mock(LongSupplier.class);
        when(relativeTimeProvider.getAsLong()).thenReturn(0L);
        CompoundProcessor compoundOnFailProcessor =
                new CompoundProcessor(false, singletonList(processorToFail), singletonList(lastProcessor), relativeTimeProvider);
        CompoundProcessor compoundProcessor =
                new CompoundProcessor(false, singletonList(processor), singletonList(compoundOnFailProcessor), relativeTimeProvider);
        compoundProcessor.execute(ingestDocument, (result, e) -> {});
    ...
    public void testCompoundProcessorExceptionFailWithoutOnFailure() throws Exception {
        ...
        CompoundProcessor compoundProcessor =
                new CompoundProcessor(false, singletonList(failCompoundProcessor), singletonList(secondProcessor), relativeTimeProvider);
        compoundProcessor.execute(ingestDocument, (result, e) -> {});
    ...
    public void testCompoundProcessorExceptionFail() throws Exception {
        ...
        CompoundProcessor failCompoundProcessor =
                new CompoundProcessor(false, singletonList(firstProcessor), singletonList(failProcessor), relativeTimeProvider);
    ...
    public void testNewCompoundProcessorExceptionPipelineOrigin() {
        Pipeline pipeline2 =
                new Pipeline("2", null, null, new CompoundProcessor(new TestProcessor("my_tag", "my_type", null, new RuntimeException())));
