...
import org.codelibs.fesen.common.logging.JsonThrowablePatternConverter;
...
public class JsonThrowablePatternConverterTests extends ESTestCase {
    ...
    public void testNoStacktrace() throws IOException {
        ...
        JsonLogLine jsonLogLine = JsonLogsStream.from(new BufferedReader(new StringReader(result))).findFirst()
    ...
    public void testStacktraceWithJson() throws IOException {
        ...
        String json = "{" + LINE_SEPARATOR + "  \"terms\" : {" + LINE_SEPARATOR + "    \"user\" : [" + LINE_SEPARATOR + "      \"u1\","
                + LINE_SEPARATOR + "      \"u2\"," + LINE_SEPARATOR + "      \"u3\"" + LINE_SEPARATOR + "    ]," + LINE_SEPARATOR
                + "    \"boost\" : 1.0" + LINE_SEPARATOR + "  }" + LINE_SEPARATOR + "}";
        ...
        assertThat("stacktrace should formatted in multiple lines", jsonLogLine.stacktrace().size(),
