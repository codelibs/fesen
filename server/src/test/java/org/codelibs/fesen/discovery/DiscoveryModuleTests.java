...
public class DiscoveryModuleTests extends ESTestCase {
    ...
    private DiscoveryModule newModule(Settings settings, List<DiscoveryPlugin> plugins) {
        return new DiscoveryModule(settings, threadPool, transportService, namedWriteableRegistry, null, masterService, clusterApplier,
                clusterSettings, plugins, null, createTempDir().toAbsolutePath(), gatewayMetaState, mock(RerouteService.class), null);
    }
    ...
    public void testLegacyHostsProvider() {
        ...
        newModule(settings, Collections.singletonList(plugin));
        assertTrue(created.get());
        assertWarnings("[discovery.zen.hosts_provider] setting was deprecated in Fesen and will be removed in a future release! "
    ...
    public void testDuplicateSeedsProvider() {
        ...
        IllegalArgumentException e =
                expectThrows(IllegalArgumentException.class, () -> newModule(Settings.EMPTY, Arrays.asList(plugin1, plugin2)));
    ...
    public void testMultipleSeedsProviders() {
        ...
        Settings settings =
                Settings.builder().putList(DiscoveryModule.DISCOVERY_SEED_PROVIDERS_SETTING.getKey(), "provider1", "provider3").build();
