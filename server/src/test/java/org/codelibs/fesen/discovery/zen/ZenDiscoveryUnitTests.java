...
import org.codelibs.fesen.discovery.zen.ZenPing;
import org.codelibs.fesen.discovery.zen.PublishClusterStateActionTests.AssertingAckListener;
import org.codelibs.fesen.discovery.zen.ZenDiscovery.ZenNodeRemovalClusterStateTaskExecutor;
...
import static org.codelibs.fesen.cluster.routing.RoutingTableTests.updateActiveAllocations;
import static org.codelibs.fesen.cluster.service.MasterServiceTests.discoveryState;
import static org.codelibs.fesen.discovery.zen.ElectMasterService.DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING;
import static org.codelibs.fesen.discovery.zen.ZenDiscovery.shouldIgnoreOrRejectNewClusterState;
...
public class ZenDiscoveryUnitTests extends ESTestCase {
    ...
    public void testFilterNonMasterPingResponse() {
        ...
        for (int i = randomIntBetween(10, 20); i >= 0; i--) {
            ...
            DiscoveryNode node = new DiscoveryNode("node_" + i, "id_" + i, buildNewFakeTransportAddress(), Collections.emptyMap(), roles,
                    Version.CURRENT);
    ...
    public void testNodesUpdatedAfterClusterStatePublished() throws Exception {
        ...
        Settings settings =
                Settings.builder().put(DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING.getKey(), Integer.toString(minMasterNodes)).build();
        ...
        try {
            ...
            Settings settingsWithClusterName = Settings.builder().put(settings)
                    .put(ClusterName.CLUSTER_NAME_SETTING.getKey(), discoveryState(masterMasterService).getClusterName().value()).build();
            ...
            ClusterState newState = ClusterState.builder(discoveryState(masterMasterService)).incrementVersion()
                    .nodes(DiscoveryNodes.builder(state.nodes()).add(otherNode).masterNodeId(masterNode.getId())).build();
    ...
    public void testPendingCSQueueIsClearedWhenClusterStatePublished() throws Exception {
        ...
        try {
            ...
            // was updated by the presence of this node in NodesFaultDetection)
            ClusterState newState = ClusterState.builder(discoveryState(masterMasterService)).incrementVersion()
                    .nodes(DiscoveryNodes.builder(discoveryState(masterMasterService).nodes()).masterNodeId(masterNode.getId())).build();
    ...
    private ZenDiscovery buildZenDiscovery(Settings settings, TransportService service, MasterService masterService,
        ...
        ZenDiscovery zenDiscovery =
                new ZenDiscovery(settings, threadPool, service, new NamedWriteableRegistry(ClusterModule.getNamedWriteables()),
        ...
        for (int i = 0; i < iters; i++) {
            ...
            IndexMetadata indexMetadata =
                    IndexMetadata.builder("test")
                            .settings(Settings.builder().put(SETTING_VERSION_CREATED,
                            ...
                            .state(IndexMetadata.State.OPEN).build();
            ...
            if (incompatible) {
                ...
                assertEquals(
                        "index [test] version not supported: "
                                ...
                                + " minimum compatible index version is: " + Version.CURRENT.minimumIndexCompatibilityVersion(),
                        ex.getMessage());
            } else {
    ...
    public void testIncomingClusterStateValidation() throws Exception {
        ...
        DiscoveryNodes.Builder currentNodes = DiscoveryNodes.builder()
                .add(new DiscoveryNode("a", buildNewFakeTransportAddress(), emptyMap(), emptySet(), Version.CURRENT)).localNodeId("a");
        ...
        try {
            ZenDiscovery.validateIncomingState(logger,
                    ClusterState.builder(new ClusterName(randomAlphaOfLength(10))).nodes(previousState.nodes()).build(), previousState);
        ...
        try {
            state = ClusterState.builder(previousState)
                    .nodes(DiscoveryNodes.builder(previousState.nodes()).localNodeId("_non_existing_").build()).incrementVersion().build();
            ZenDiscovery.validateIncomingState(logger, state, previousState);
        ...
        final String message = String.format(Locale.ROOT, "rejecting cluster state version [%d] uuid [%s] received from [%s]",
        ...
        state = ClusterState.builder(previousState)
                .nodes(DiscoveryNodes.builder(previousState.nodes()).masterNodeId("_new_master_").build()).build();
