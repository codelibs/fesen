...
import org.codelibs.fesen.discovery.zen.NodeJoinController;
...
import static org.codelibs.fesen.cluster.ESAllocationTestCase.createAllocationService;
...
import static org.codelibs.fesen.test.VersionUtils.getPreviousVersion;
import static org.codelibs.fesen.test.VersionUtils.randomCompatibleVersion;
...
public class NodeJoinControllerTests extends ESTestCase {
    ...
    private static ClusterState initialState(boolean withMaster) {
        ...
        ClusterState initialClusterState = ClusterState
                .builder(new ClusterName(ClusterServiceUtils.class.getSimpleName())).nodes(DiscoveryNodes.builder().add(localNode)
                        .localNodeId(localNode.getId()).masterNodeId(withMaster ? localNode.getId() : null))
                ...
                nodeJoinController.waitToBeElectedAsMaster(requiredJoins, TimeValue.timeValueHours(30),
                        new NodeJoinController.ElectionCallback() {
                            ...
                            public void onElectedAsMaster(ClusterState state) {
                                assertThat("callback called with elected as master, but state disagrees",
                                        state.nodes().isLocalNodeElectedMaster(), equalTo(true));
    ...
    public void testNewClusterStateOnExistingNodeJoin() throws InterruptedException, ExecutionException {
        ...
        final DiscoveryNode other_node =
                new DiscoveryNode("other_node", buildNewFakeTransportAddress(), emptyMap(), emptySet(), Version.CURRENT);
    ...
    public void testRejectingJoinWithSameIdButDifferentNode() throws InterruptedException, ExecutionException {
        ...
        ClusterState state = discoveryState(masterService);
        final DiscoveryNode existing = randomFrom(StreamSupport.stream(state.nodes().spliterator(), false).collect(Collectors.toList()));
        final DiscoveryNode other_node = new DiscoveryNode(randomBoolean() ? existing.getName() : "other_name", existing.getId(),
    ...
    public void testRejectingRestartedNodeJoinsBeforeProcessingNodeLeft() throws InterruptedException, ExecutionException {
        ...
        final DiscoveryNode other_node =
                new DiscoveryNode(existing.getId(), existing.getAddress(), existing.getAttributes(), existing.getRoles(), Version.CURRENT);
    ...
    public void testElectionBasedOnConflictingNodes() throws InterruptedException, ExecutionException {
        ...
        if (randomBoolean()) {
            IndexMetadata indexMetadata = IndexMetadata.builder("test")
                    .settings(Settings.builder().put(SETTING_VERSION_CREATED, Version.CURRENT).put(SETTING_NUMBER_OF_SHARDS, 1)
                            .put(SETTING_NUMBER_OF_REPLICAS, 1).put(SETTING_CREATION_DATE, System.currentTimeMillis()))
                    .build();
    ...
    private void addNodes(int count) {
        ...
        for (int i = 0; i < count; i++) {
            final DiscoveryNode node = new DiscoveryNode("node_" + state.nodes().getSize() + i, buildNewFakeTransportAddress(), emptyMap(),
                    new HashSet<>(randomSubsetOf(DiscoveryNodeRole.BUILT_IN_ROLES)), Version.CURRENT);
