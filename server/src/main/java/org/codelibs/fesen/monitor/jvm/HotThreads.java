...
public class HotThreads {
    ...
    private static boolean isIdleThread(ThreadInfo threadInfo) {
        ...
        for (StackTraceElement frame : threadInfo.getStackTrace()) {
            ...
            if (className.equals("org.codelibs.fesen.threadpool.ThreadPool$CachedTimeThread") && methodName.equals("run")) {
                return true;
            }
            if (className.equals("org.codelibs.fesen.indices.ttl.IndicesTTLService$Notifier") && methodName.equals("await")) {
                return true;
            }
            ...
            for (int i = 0; i < threadElementsSnapshotCount; i++) {
                ...
                if (allInfos[i][t] != null) {
                    ...
                    if (count == 1) {
                        ...
                        sb.append(String.format(Locale.ROOT, "  %d/%d snapshots sharing following %d elements%n", count,
                                threadElementsSnapshotCount, maxSim));
