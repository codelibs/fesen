 ...
 * <h2>Preliminaries</h2>
 *
 * <p>The {@link org.codelibs.fesen.repositories.blobstore.BlobStoreRepository} forms the basis of implementations of
 * {@link org.codelibs.fesen.repositories.Repository} on top of a blob store. A blobstore can be used as the basis for an implementation
 ...
 * GET operations.
 * These operations are formally defined as specified by the {@link org.codelibs.fesen.common.blobstore.BlobContainer} interface that
 * any {@code BlobStoreRepository} implementation must provide via its implementation of
 * {@link org.codelibs.fesen.repositories.blobstore.BlobStoreRepository#getBlobContainer()}.</p>
 *
 ...
 * the documentation of the {@link org.codelibs.fesen.snapshots} package.</p>
 *
 ...
 * {@code
 *   STORE_ROOT
 *   |- index-N           - JSON serialized {@link org.codelibs.fesen.repositories.RepositoryData} containing a list of all snapshot ids
 *   |                      and the indices belonging to each snapshot, N is the generation of the file
 *   |- index.latest      - contains the numeric value of the latest generation of the index file (i.e. N from above)
 *   |- incompatible-snapshots - list of all snapshot ids that are no longer compatible with the current version of the cluster
 *   |- snap-20131010.dat - SMILE serialized {@link org.codelibs.fesen.snapshots.SnapshotInfo} for snapshot "20131010"
 *   |- meta-20131010.dat - SMILE serialized {@link org.codelibs.fesen.cluster.metadata.Metadata} for snapshot "20131010"
 *   |                      (includes only global metadata)
 *   |- snap-20131011.dat - SMILE serialized {@link org.codelibs.fesen.snapshots.SnapshotInfo} for snapshot "20131011"
 *   |- meta-20131011.dat - SMILE serialized {@link org.codelibs.fesen.cluster.metadata.Metadata} for snapshot "20131011"
 *   .....
 ...
 *      |  |             in the repository
 *      |  |- meta-20131010.dat - JSON Serialized {@link org.codelibs.fesen.cluster.metadata.IndexMetadata} for index "foo"
 ...
 *      |  |  .....
 *      |  |  |- snap-20131010.dat - SMILE serialized {@link org.codelibs.fesen.index.snapshots.blobstore.BlobStoreIndexShardSnapshot} for
 *      |  |  |                      snapshot "20131010"
 *      |  |  |- snap-20131011.dat - SMILE serialized {@link org.codelibs.fesen.index.snapshots.blobstore.BlobStoreIndexShardSnapshot} for
 *      |  |  |                      snapshot "20131011"
 *      |  |  |- index-123         - SMILE serialized {@link org.codelibs.fesen.index.snapshots.blobstore.BlobStoreIndexShardSnapshots} for
 ...
 *
 * <p>Loading the {@link org.codelibs.fesen.repositories.RepositoryData} that holds the list of all snapshots as well as the mapping of
 * indices' names to their repository {@link org.codelibs.fesen.repositories.IndexId} is done by invoking
 * {@link org.codelibs.fesen.repositories.blobstore.BlobStoreRepository#getRepositoryData} and implemented as follows:</p>
 * <ol>
 * <li>
 * <ol>
 ...
 * <li>For each {@link org.codelibs.fesen.repositories.blobstore.BlobStoreRepository} an entry of type
 * {@link org.codelibs.fesen.cluster.metadata.RepositoryMetadata} exists in the cluster state. It tracks the current valid
 ...
 * it is assumed to be empty and {@link org.codelibs.fesen.repositories.RepositoryData#EMPTY} is returned.</li>
 * </ol>
 * </li>
 * </ol>
 *
 ...
 *
 * <p>Writing an updated {@link org.codelibs.fesen.repositories.RepositoryData} to a blob store repository is an operation that uses
 ...
 *
 * <ol>
 * <li>Write an updated value of {@link org.codelibs.fesen.cluster.metadata.RepositoryMetadata} for the repository that has the same
 * {@link org.codelibs.fesen.cluster.metadata.RepositoryMetadata#generation()} as the existing entry and has a value of
 * {@link org.codelibs.fesen.cluster.metadata.RepositoryMetadata#pendingGeneration()} one greater than the {@code pendingGeneration} of the
 * existing entry.</li>
 ...
 * {@link org.codelibs.fesen.repositories.blobstore.BlobStoreRepository} as a read-only url repository.</li>
 ...
 * {@link org.codelibs.fesen.snapshots.SnapshotsService#NO_REPO_INITIALIZE_VERSION}, creating a snapshot in the repository starts with a
 * call to {@link org.codelibs.fesen.repositories.Repository#initializeSnapshot} which the blob store repository implements via the
 * following actions:</p>
 * <ol>
 ...
 * to the repository by invoking {@link org.codelibs.fesen.repositories.Repository#snapshotShard} on the data-nodes that hold the primaries
 ...
 * <li>Get the {@link org.codelibs.fesen.index.snapshots.blobstore.BlobStoreIndexShardSnapshots} blob
 * with name {@code index-${uuid}} with the {@code uuid} generation returned by
 * {@link org.codelibs.fesen.repositories.ShardGenerations#getShardGen} to get the information of what segment files are
 ...
 * {@link org.codelibs.fesen.index.snapshots.blobstore.BlobStoreIndexShardSnapshot} with name {@code snap-${snapshot-uuid}.dat} is written to
 ...
 * to finalizing the snapshot by invoking {@link org.codelibs.fesen.repositories.Repository#finalizeSnapshot}. This method executes the
 * following actions in order:</p>
 * <ol>
 * <li>Write a blob containing the cluster metadata to the root of the blob store repository at {@code /meta-${snapshot-uuid}.dat}</li>
 * <li>Write the metadata for each index to a blob in that index's directory at
 * {@code /indices/${index-snapshot-uuid}/meta-${snapshot-uuid}.dat}</li>
 * <li>Write the {@link org.codelibs.fesen.snapshots.SnapshotInfo} blob for the given snapshot to the key {@code /snap-${snapshot-uuid}.dat}
 ...
 * action when {@link org.codelibs.fesen.repositories.blobstore.BlobStoreRepository#deleteSnapshots} is invoked:</p>
 *
 * <ol>
 ...
 * the {@link org.codelibs.fesen.repositories.RepositoryData} in the root {@code index-${N}} blob of the repository.</li>
