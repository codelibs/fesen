...
import static org.codelibs.fesen.indices.breaker.BreakerSettings.CIRCUIT_BREAKER_LIMIT_SETTING;
import static org.codelibs.fesen.indices.breaker.BreakerSettings.CIRCUIT_BREAKER_OVERHEAD_SETTING;
...
public class HierarchyCircuitBreakerService extends CircuitBreakerService {
    ...
    private static final String CHILD_LOGGER_PREFIX = "org.codelibs.fesen.indices.breaker.";
    ...
    HierarchyCircuitBreakerService(Settings settings, List<BreakerSettings> customBreakers, ClusterSettings clusterSettings,
        ...
        childCircuitBreakers.put(CircuitBreaker.FIELDDATA, validateAndCreateBreaker(new BreakerSettings(CircuitBreaker.FIELDDATA,
                FIELDDATA_CIRCUIT_BREAKER_LIMIT_SETTING.get(settings).getBytes(), FIELDDATA_CIRCUIT_BREAKER_OVERHEAD_SETTING.get(settings),
                FIELDDATA_CIRCUIT_BREAKER_TYPE_SETTING.get(settings), CircuitBreaker.Durability.PERMANENT)));
        childCircuitBreakers.put(CircuitBreaker.IN_FLIGHT_REQUESTS,
                validateAndCreateBreaker(new BreakerSettings(CircuitBreaker.IN_FLIGHT_REQUESTS,
                        ...
                        IN_FLIGHT_REQUESTS_CIRCUIT_BREAKER_TYPE_SETTING.get(settings), CircuitBreaker.Durability.TRANSIENT)));
        childCircuitBreakers.put(CircuitBreaker.REQUEST, validateAndCreateBreaker(new BreakerSettings(CircuitBreaker.REQUEST,
                REQUEST_CIRCUIT_BREAKER_LIMIT_SETTING.get(settings).getBytes(), REQUEST_CIRCUIT_BREAKER_OVERHEAD_SETTING.get(settings),
                REQUEST_CIRCUIT_BREAKER_TYPE_SETTING.get(settings), CircuitBreaker.Durability.TRANSIENT)));
        childCircuitBreakers.put(CircuitBreaker.ACCOUNTING,
                validateAndCreateBreaker(
                        new BreakerSettings(CircuitBreaker.ACCOUNTING, ACCOUNTING_CIRCUIT_BREAKER_LIMIT_SETTING.get(settings).getBytes(),
                                ACCOUNTING_CIRCUIT_BREAKER_OVERHEAD_SETTING.get(settings),
                                ACCOUNTING_CIRCUIT_BREAKER_TYPE_SETTING.get(settings), CircuitBreaker.Durability.PERMANENT)));
        for (BreakerSettings breakerSettings : customBreakers) {
            if (childCircuitBreakers.containsKey(breakerSettings.getName())) {
                throw new IllegalArgumentException("More than one circuit breaker with the name [" + breakerSettings.getName()
        ...
        this.parentSettings = new BreakerSettings(CircuitBreaker.PARENT, TOTAL_CIRCUIT_BREAKER_LIMIT_SETTING.get(settings).getBytes(), 1.0,
        ...
        clusterSettings.addAffixUpdateConsumer(CIRCUIT_BREAKER_LIMIT_SETTING, CIRCUIT_BREAKER_OVERHEAD_SETTING,
                (name, updatedValues) -> updateCircuitBreakerSettings(name, updatedValues.v1(), updatedValues.v2()), (s, t) -> {});
    ...
    private void validateTotalCircuitBreakerLimit(ByteSizeValue byteSizeValue) {
        BreakerSettings newParentSettings =
                new BreakerSettings(CircuitBreaker.PARENT, byteSizeValue.getBytes(), 1.0, CircuitBreaker.Type.PARENT, null);
    ...
    public AllCircuitBreakerStats stats() {
        ...
        allStats.add(new CircuitBreakerStats(CircuitBreaker.PARENT, parentSettings.getLimit(), memoryUsed(0L).totalUsage, 1.0,
                parentTripCount.get()));
    ...
    public void checkParentLimit(long newBytesReserved, String label) throws CircuitBreakingException {
        ...
        if (memoryUsed.totalUsage > parentLimit && overLimitStrategy.overLimit(memoryUsed).totalUsage > parentLimit) {
            ...
            final StringBuilder message = new StringBuilder("[parent] Data too large, data for [" + label + "]" + " would be ["
                    + memoryUsed.totalUsage + "/" + new ByteSizeValue(memoryUsed.totalUsage) + "]" + ", which is larger than the limit of ["
            ...
            CircuitBreaker.Durability durability =
                    memoryUsed.transientChildUsage >= memoryUsed.permanentChildUsage ? CircuitBreaker.Durability.TRANSIENT
    ...
    private CircuitBreaker validateAndCreateBreaker(BreakerSettings breakerSettings) {
        ...
        return breakerSettings.getType() == CircuitBreaker.Type.NOOP ? new NoopCircuitBreaker(breakerSettings.getName())
                : new ChildMemoryCircuitBreaker(breakerSettings, LogManager.getLogger(CHILD_LOGGER_PREFIX + breakerSettings.getName()),
    ...
    static LongSupplier createYoungGcCountSupplier() {
        List<GarbageCollectorMXBean> youngBeans = ManagementFactory.getGarbageCollectorMXBeans().stream()
    ...
    static class G1OverLimitStrategy implements OverLimitStrategy {
        ...
        G1OverLimitStrategy(JvmInfo jvmInfo, LongSupplier currentMemoryUsageSupplier, LongSupplier gcCountSupplier,
