    ...
    public synchronized void addPending(ClusterState state) {
        ...
        if (pendingStates.size() > maxQueueSize) {
            ...
            if (context.committed()) {
                context.listener.onNewClusterStateFailed(new FesenException("too many pending states ([{}] pending)", maxQueueSize));
            }
        }
    }
        ...
        for (int index = 0; index < pendingStates.size(); index++) {
            ...
            if (Objects.equals(currentMaster, pendingMasterNode) == false) {
                ...
                if (pendingContext.committed()) {
                    ...
                    pendingContext.listener
                            .onNewClusterStateFailed(new IllegalStateException("cluster state from a different master than the current one,"
                                    + " rejecting (received " + pendingMasterNode + ", current " + currentMaster + ")"));
                } else {
                    logger.trace(
                            "removing non-committed state with uuid[{}]/v[{}] from [{}] - a state from"
                ...
                logger.trace("processing pending state uuid[{}]/v[{}] together with state uuid[{}]/v[{}]", pendingState.stateUUID(),
                        pendingState.version(), state.stateUUID(), state.version());
                contextsToRemove.add(pendingContext);
                if (pendingContext.committed()) {
                    pendingContext.listener.onNewClusterStateProcessed();
                }
    ...
    public synchronized ClusterState getNextClusterStateToProcess() {
        ...
        if (coordinationMetadata.getLastAcceptedConfiguration().equals(coordinationMetadata.getLastCommittedConfiguration())) {
            ...
            return ClusterState.builder(committedState)
                    .metadata(Metadata.builder(committedState.metadata())
                            .coordinationMetadata(CoordinationMetadata.builder(coordinationMetadata)
                                    .lastCommittedConfiguration(committedState.getLastAcceptedConfiguration()).build()))
                    .build();
        }
    }
