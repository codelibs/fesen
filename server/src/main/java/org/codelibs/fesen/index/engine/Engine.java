...
public abstract class Engine implements Closeable {
    ...
    public void verifyEngineBeforeIndexClosing() throws IllegalStateException {
        ...
        if (globalCheckpoint != maxSeqNo) {
            throw new IllegalStateException("Global checkpoint [" + globalCheckpoint + "] mismatches maximum sequence number [" + maxSeqNo
                    + "] on index shard " + shardId);
        }
    }
    ...
    protected final GetResult getFromSearcher(Get get, BiFunction<String, SearcherScope, Engine.Searcher> searcherFactory,
        ...
        if (docIdAndVersion != null) {
            ...
            if (get.getIfSeqNo() != SequenceNumbers.UNASSIGNED_SEQ_NO
                ...
                Releasables.close(searcher);
                throw new VersionConflictEngineException(shardId, get.id(), get.getIfSeqNo(), get.getIfPrimaryTerm(), docIdAndVersion.seqNo,
    ...
    public Searcher acquireSearcher(String source, SearcherScope scope, Function<Searcher, Searcher> wrapper) throws EngineException {
        ...
        try {
            ...
            releasable = null;
            return new Searcher(source, searcher.getDirectoryReader(), searcher.getSimilarity(), searcher.getQueryCache(),
                    searcher.getQueryCachingPolicy(), () -> Releasables.close(searcher, reader));
        } finally {
            Releasables.close(releasable);
        }
    }
     ...
     */
    public abstract Translog.Snapshot newChangesSnapshot(String source, MapperService mapperService, long fromSeqNo, long toSeqNo,
     ...
     */
    public abstract Translog.Snapshot readHistoryOperations(String reason, HistorySource historySource, MapperService mapperService,
     ...
     */
    public abstract int estimateNumberOfHistoryOperations(String reason, HistorySource historySource, MapperService mapperService,
            long startingSeqNo) throws IOException;

    /**
     * Checks if this engine has every operations since  {@code startingSeqNo}(inclusive) in its history (either Lucene or translog)
     */
    public abstract boolean hasCompleteOperationHistory(String reason, HistorySource historySource, MapperService mapperService,
            long startingSeqNo) throws IOException;
    ...
    private ImmutableOpenMap<String, Long> getSegmentFileSizes(SegmentReader segmentReader) {
        ...
        if (useCompoundFile) {
            try {
                ...
                logger.warn(() -> new ParameterizedMessage(
                        "Error when opening compound reader for Directory [{}] and " + "SegmentCommitInfo [{}]", segmentReader.directory(),
                        segmentCommitInfo), e);
        ...
        if (useCompoundFile) {
            ...
            try {
                ...
                logger.warn(() -> new ParameterizedMessage("Couldn't list Directory from SegmentReader [{}] and SegmentInfo [{}]",
        ...
        for (String file : files) {
            ...
            try {
                ...
                final Directory finalDirectory = directory;
                logger.warn(() -> new ParameterizedMessage("Tried to query fileLength but file is gone [{}] [{}]", finalDirectory, file),
                        e);
            } catch (IOException e) {
                final Directory finalDirectory = directory;
    ...
    public abstract void forceMerge(boolean flush, int maxNumSegments, boolean onlyExpungeDeletes, boolean upgrade,
            boolean upgradeOnlyAncientSegments, @Nullable String forceMergeUUID) throws EngineException, IOException;
        ...
        if (failEngineLock.tryLock()) {
            try {
                if (failedEngine.get() != null) {
                    logger.warn(() -> new ParameterizedMessage("tried to fail engine but engine is already failed. ignoring. [{}]", reason),
                            failure);
                    return;
                }
                ...
                try {
                    ...
                    if (Lucene.isCorruptionException(failure)) {
                        if (store.tryIncRef()) {
                            ...
                            logger.warn(() -> new ParameterizedMessage("tried to mark store as corrupted but store is already closed. [{}]",
                                    reason), failure);
                        }
                    }
            ...
            logger.debug(() -> new ParameterizedMessage(
                    "tried to fail engine but could not acquire lock - engine should " + "be failed by now [{}]", reason), failure);
        }
    }
    ...
    public static final class Searcher extends IndexSearcher implements Releasable {
        ...
        public Searcher(String source, IndexReader reader, Similarity similarity, QueryCache queryCache,
                QueryCachingPolicy queryCachingPolicy, Closeable onClose) {
            super(reader);
    ...
    public abstract static class Operation {
        ...
        public enum Origin {
            PRIMARY, REPLICA, PEER_RECOVERY, LOCAL_TRANSLOG_RECOVERY, LOCAL_RESET;
    ...
    public static class Index extends Operation {
        ...
        public Index(Term uid, ParsedDocument doc, long seqNo, long primaryTerm, long version, VersionType versionType, Origin origin,
            ...
            assert (origin == Origin.PRIMARY) || (ifSeqNo == UNASSIGNED_SEQ_NO
                    && ifPrimaryTerm == UNASSIGNED_PRIMARY_TERM) : "cas operations are only allowed if origin is primary. get [" + origin
                            + "]";
            this.doc = doc;
        ...
        Index(Term uid, long primaryTerm, ParsedDocument doc, long version) {
            this(uid, doc, UNASSIGNED_SEQ_NO, primaryTerm, version, VersionType.INTERNAL, Origin.PRIMARY, System.nanoTime(), -1, false,
                    UNASSIGNED_SEQ_NO, 0);
        } // TEST ONLY
        ...
        /**
         * Returns a positive timestamp if the ID of this document is auto-generated by fesen.
    ...
    public static class Delete extends Operation {
        ...
        public Delete(String type, String id, Term uid, long seqNo, long primaryTerm, long version, VersionType versionType, Origin origin,
                long startTime, long ifSeqNo, long ifPrimaryTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert ifPrimaryTerm >= 0 : "ifPrimaryTerm [" + ifPrimaryTerm + "] must be non negative";
            assert ifSeqNo == UNASSIGNED_SEQ_NO || ifSeqNo >= 0 : "ifSeqNo [" + ifSeqNo + "] must be non negative or unset";
        ...
        public Delete(String type, String id, Term uid, long primaryTerm) {
            this(type, id, uid, UNASSIGNED_SEQ_NO, primaryTerm, Versions.MATCH_ANY, VersionType.INTERNAL, Origin.PRIMARY, System.nanoTime(),
                    UNASSIGNED_SEQ_NO, 0);
        }

        public Delete(Delete template, VersionType versionType) {
            this(template.type(), template.id(), template.uid(), template.seqNo(), template.primaryTerm(), template.version(), versionType,
                    template.origin(), template.startTime(), UNASSIGNED_SEQ_NO, 0);
        }
