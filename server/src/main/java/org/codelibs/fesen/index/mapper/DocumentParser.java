...
import static org.codelibs.fesen.index.mapper.FieldMapper.IGNORE_MALFORMED_SETTING;
...
final class DocumentParser {
    ...
    ParsedDocument parseDocument(SourceToParse source, MetadataFieldMapper[] metadataFieldsMappers) throws MapperParsingException {
        ...
        try (XContentParser parser = XContentHelper.createParser(docMapperParser.getXContentRegistry(), LoggingDeprecationHandler.INSTANCE,
    ...
    private void validateType(SourceToParse source) {
        ...
        if (Objects.equals(source.type(), docMapper.type()) == false && MapperService.SINGLE_MAPPING_NAME.equals(source.type()) == false) { // used by typeless APIs
    ...
    private static ParsedDocument parsedDocument(SourceToParse source, ParseContext.InternalParseContext context, Mapping update) {
        return new ParsedDocument(context.version(), context.seqID(), context.sourceToParse().id(), context.sourceToParse().type(),
                source.routing(), context.docs(), context.sourceToParse().source(), context.sourceToParse().getXContentType(), update);
    }
    ...
    private static void innerParseObject(ParseContext context, ObjectMapper mapper, XContentParser parser, String currentFieldName,
    ...
    private static void parseValue(final ParseContext context, ObjectMapper parentMapper, String currentFieldName,
            XContentParser.Token token, String[] paths) throws IOException {
    ...
    private static Mapper.Builder<?> createBuilderFromDynamicValue(final ParseContext context, XContentParser.Token token,
            ...
            if (parseableAsLong && context.root().numericDetection()) {
                ...
                for (DateFormatter dateTimeFormatter : context.root().dynamicDateTimeFormatters()) {
                    ...
                    if (builder == null) {
                        ...
                        builder = new DateFieldMapper.Builder(currentFieldName, DateFieldMapper.Resolution.MILLISECONDS, dateTimeFormatter,
                                ignoreMalformed, Version.indexCreated(context.indexSettings().getSettings()));
                    }
                    return builder;
    ...
    private static void parseDynamicValue(final ParseContext context, ObjectMapper parentMapper, String currentFieldName,
            XContentParser.Token token) throws IOException {
        ObjectMapper.Dynamic dynamic = dynamicOrDefault(parentMapper, context);
        if (dynamic == ObjectMapper.Dynamic.STRICT) {
    ...
    private static Mapper getMapper(final ParseContext context, ObjectMapper objectMapper, String fieldName, String[] subfields) {
        ...
        for (int i = 0; i < subfields.length - 1; ++i) {
            ...
            if (objectMapper.nested().isNested()) {
                throw new MapperParsingException("Cannot add a value for field [" + fieldName
                        + "] since one of the intermediate objects is mapped as a nested object: [" + mapper.name() + "]");
            }
        }
