...
public class MultiMatchQuery extends MatchQuery {
    ...
    private List<Query> buildCrossFieldQuery(MultiMatchQueryBuilder.Type type, Map<String, Float> fieldNames, Object value,
            String minimumShouldMatch, float tieBreaker) throws IOException {
        ...
        for (Map.Entry<Analyzer, List<FieldAndBoost>> group : groups.entrySet()) {
            ...
            if (group.getValue().size() == 1) {
                builder = new MatchQueryBuilder(group.getKey(), group.getValue().get(0).fieldType, enablePositionIncrements,
                        autoGenerateSynonymsPhraseQuery);
            } else {
                builder = new BlendedQueryBuilder(group.getKey(), group.getValue(), tieBreaker, enablePositionIncrements,
                        autoGenerateSynonymsPhraseQuery);
            }
    ...
    private class BlendedQueryBuilder extends MatchQueryBuilder {
        ...
        BlendedQueryBuilder(Analyzer analyzer, List<FieldAndBoost> blendedFields, float tieBreaker, boolean enablePositionIncrements,
    ...
    static Query blendTerm(QueryShardContext context, BytesRef value, Float commonTermsCutoff, float tieBreaker, boolean lenient,
    ...
    static Query blendTerms(QueryShardContext context, BytesRef[] values, Float commonTermsCutoff, float tieBreaker, boolean lenient,
            List<FieldAndBoost> blendedFields) {
