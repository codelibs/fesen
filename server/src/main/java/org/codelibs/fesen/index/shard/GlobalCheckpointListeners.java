...
public class GlobalCheckpointListeners implements Closeable {
    ...
    public interface GlobalCheckpointListener {
         ...
         * will be set to {@link org.codelibs.fesen.index.seqno.SequenceNumbers#UNASSIGNED_SEQ_NO} and the exception will be non-null and an
    ...
    synchronized void globalCheckpointUpdated(final long globalCheckpoint) {
        ...
        assert globalCheckpoint > lastKnownGlobalCheckpoint : "updated global checkpoint [" + globalCheckpoint + "]"
    ...
    private void notifyListeners(final long globalCheckpoint, final IndexShardClosedException e) {
        ...
        if (globalCheckpoint != UNASSIGNED_SEQ_NO) {
            listenersToNotify = listeners.entrySet().stream().filter(entry -> entry.getValue().v1() <= globalCheckpoint)
    ...
    private void notifyListener(final GlobalCheckpointListener listener, final long globalCheckpoint, final Exception e) {
        ...
        listener.executor().execute(() -> {
            try {
                ...
                if (globalCheckpoint != UNASSIGNED_SEQ_NO) {
                    logger.warn(new ParameterizedMessage("error notifying global checkpoint listener of updated global checkpoint [{}]",
                            globalCheckpoint), caught);
