    ...
    static {
        ObjectParser.Parser<ReindexRequest, Void> sourceParser = (parser, request, context) -> {
            ...
            try (InputStream stream = BytesReference.bytes(builder).streamInput();
                    XContentParser innerParser = parser.contentType().xContent().createParser(parser.getXContentRegistry(),
                            parser.getDeprecationHandler(), stream)) {
    ...
    static RemoteInfo buildRemoteInfo(Map<String, Object> source) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "[host] must be of the form [scheme]://[host]:[port](/[pathPrefix])? but was [" + hostInRequest + "]", ex);
        }
        ...
        return new RemoteInfo(scheme, host, port, pathPrefix, RemoteInfo.queryForRemote(source), username, password, headers, socketTimeout,
                connectTimeout);
    }
