...
public class GeoBoundingBox implements ToXContentObject, Writeable {
     ...
     */
    public static GeoBoundingBox parseBoundingBox(XContentParser parser) throws IOException, FesenParseException {
        XContentParser.Token token = parser.currentToken();
        if (token != XContentParser.Token.START_OBJECT) {
            throw new FesenParseException("failed to parse bounding box. Expected start object but found [{}]", token);
        }
            ...
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (WKT_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                    ...
                    if (TOP_LEFT_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                        ...
                        throw new FesenParseException("failed to parse bounding box. unexpected field [{}]", currentFieldName);
                    }
                }
            } else {
                throw new FesenParseException("failed to parse bounding box. field name expected but [{}] found", token);
            }
        }
        if (envelope != null) {
