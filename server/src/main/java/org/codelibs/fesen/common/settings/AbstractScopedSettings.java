...
public abstract class AbstractScopedSettings {
    ...
    protected AbstractScopedSettings(final Settings settings, final Set<Setting<?>> settingsSet,
            final Set<SettingUpgrader<?>> settingUpgraders, final Setting.Property scope) {
        this.logger = LogManager.getLogger(this.getClass());
        this.settings = settings;
        this.lastSettingsApplied = Settings.EMPTY;

        this.settingUpgraders = Collections
                .unmodifiableMap(settingUpgraders.stream().collect(Collectors.toMap(SettingUpgrader::getSetting, Function.identity())));
    ...
    */
    ...
    public synchronized <A, B> void addAffixUpdateConsumer(Setting.AffixSetting<A> settingA, Setting.AffixSetting<B> settingB,
            BiConsumer<String, Tuple<A, B>> consumer, BiConsumer<String, Tuple<A, B>> validator) {
            ...
            public Map<String, Tuple<A, B>> getValue(Settings current, Settings previous) {
                ...
                BiConsumer<String, B> bConsumer = (key, value) -> {
                    ...
                    if (abTuple != null) {
                        ...
                        assert settingA.getConcreteSettingForNamespace(key).get(current)
                                .equals(settingA.getConcreteSettingForNamespace(key).get(previous)) : "expected: "
                                        + settingA.getConcreteSettingForNamespace(key).get(current) + " but was "
     ...
     * <p>
     * Note: Only settings registered in {@link org.codelibs.fesen.cluster.ClusterModule} can be changed dynamically.
     * </p>
     */
     ...
     * @see Setting#getSettingsDependencies(String)
     */
    public final void validate(final Settings settings, final boolean validateDependencies, final boolean ignorePrivateSettings,
     ...
     */
    public final void validate(final Settings settings, final boolean validateDependencies, final boolean ignorePrivateSettings,
            final boolean ignoreArchivedSettings, final boolean validateInternalOrPrivateIndex) {
        ...
        if (setting == null) {
            ...
            if (validateDependencies && settingsDependencies.isEmpty() == false) {
                for (final Setting.SettingDependency settingDependency : settingsDependencies) {
                    ...
                    if (dependency.existsOrFallbackExists(settings) == false) {
                        final String message = String.format(Locale.ROOT, "missing required setting [%s] for setting [%s]",
                                dependency.getKey(), setting.getKey());
            ...
            if (validateInternalOrPrivateIndex) {
                if (setting.isInternalIndex()) {
                    ...
                    throw new IllegalArgumentException(
                            "can not update private setting [" + setting.getKey() + "]; this setting is managed by Fesen");
                }
            }
        }
    ...
    private boolean assertMatcher(String key, int numComplexMatchers) {
        ...
        assert list.size() == numComplexMatchers : "Expected " + numComplexMatchers + " complex matchers to match key [" + key
                + "] but got: " + list.toString();
        return true;
    }
     ...
     */
    public Settings archiveUnknownOrInvalidSettings(final Settings settings, final Consumer<Map.Entry<String, String>> unknownConsumer,
