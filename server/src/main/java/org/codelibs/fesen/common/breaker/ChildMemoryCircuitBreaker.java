...
public class ChildMemoryCircuitBreaker implements CircuitBreaker {
    ...
    public void circuitBreak(String fieldName, long bytesNeeded) {
        ...
        final String message = "[" + this.name + "] Data too large, data for [" + fieldName + "]" + " would be [" + bytesNeeded + "/"
                + new ByteSizeValue(bytesNeeded) + "]" + ", which is larger than the limit of [" + memoryBytesLimit + "/"
    ...
    private long noLimit(long bytes, String label) {
        ...
        logger.trace(() -> new ParameterizedMessage("[{}] Adding [{}][{}] to used bytes [new used: [{}], limit: [-1b]]", this.name,
    ...
    private long limit(long bytes, String label, double overheadConstant, long memoryBytesLimit) {
        ...
        do {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("[{}] Adding [{}][{}] to used bytes [new used: [{}], limit: {} [{}], estimate: {} [{}]]", this.name,
            ...
            if (memoryBytesLimit > 0 && newUsedWithOverhead > memoryBytesLimit) {
                logger.warn("[{}] New used memory {} [{}] for data of [{}] would be larger than configured breaker: {} [{}], breaking",
                        this.name, newUsedWithOverhead, new ByteSizeValue(newUsedWithOverhead), label, memoryBytesLimit,
                        new ByteSizeValue(memoryBytesLimit));
