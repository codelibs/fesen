...
import static org.codelibs.fesen.common.inject.internal.Annotations.findScopeAnnotation;
...
import org.codelibs.fesen.common.Classes;
import org.codelibs.fesen.common.inject.internal.Annotations;
import org.codelibs.fesen.common.inject.internal.BindingImpl;
import org.codelibs.fesen.common.inject.internal.Errors;
import org.codelibs.fesen.common.inject.internal.ErrorsException;
import org.codelibs.fesen.common.inject.internal.InstanceBindingImpl;
import org.codelibs.fesen.common.inject.internal.InternalContext;
import org.codelibs.fesen.common.inject.internal.InternalFactory;
import org.codelibs.fesen.common.inject.internal.LinkedBindingImpl;
import org.codelibs.fesen.common.inject.internal.LinkedProviderBindingImpl;
import org.codelibs.fesen.common.inject.internal.MatcherAndConverter;
import org.codelibs.fesen.common.inject.internal.Scoping;
import org.codelibs.fesen.common.inject.internal.SourceProvider;
import org.codelibs.fesen.common.inject.internal.ToStringBuilder;
import org.codelibs.fesen.common.inject.spi.BindingTargetVisitor;
import org.codelibs.fesen.common.inject.spi.ConvertedConstantBinding;
import org.codelibs.fesen.common.inject.spi.Dependency;
import org.codelibs.fesen.common.inject.spi.ProviderBinding;
...
class InjectorImpl implements Injector, Lookups {
    ...
    private <T> BindingImpl<MembersInjector<T>> createMembersInjectorBinding(Key<MembersInjector<T>> key, Errors errors)
        ...
        TypeLiteral<T> instanceType =
                (TypeLiteral<T>) TypeLiteral.get(((ParameterizedType) membersInjectorType).getActualTypeArguments()[0]);
    ...
    private static class ConvertedConstantBindingImpl<T> extends BindingImpl<T> implements ConvertedConstantBinding<T> {
        ...
        @Override
        public String toString() {
            return new ToStringBuilder(ConvertedConstantBinding.class).add("key", getKey()).add("sourceKey", getSourceKey())
    ...
    <T> BindingImpl<T> createProvidedByBinding(Key<T> key, Scoping scoping, ProvidedBy providedBy, Errors errors) throws ErrorsException {
        ...
        return new LinkedProviderBindingImpl<>(this, key, rawType /* source */, Scopes.<T> scope(key, this, internalFactory, scoping),
                scoping, providerKey);
    }
    ...
    <T> BindingImpl<T> createImplementedByBinding(Key<T> key, Scoping scoping, ImplementedBy implementedBy, Errors errors)
        ...
        return new LinkedBindingImpl<>(this, key, rawType /* source */, Scopes.<T> scope(key, this, internalFactory, scoping), scoping,
                targetKey);
    }
