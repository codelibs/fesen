...
public class GeoWKTParser {
    ...
    private static ShapeBuilder parseGeometry(StreamTokenizer stream, GeoShapeType shapeType, final boolean ignoreZValue,
        ...
        if (shapeType != null && shapeType != GeoShapeType.GEOMETRYCOLLECTION) {
            if (type.wktName().equals(shapeType.wktName()) == false) {
                throw new FesenParseException("Expected geometry type [{}] but found [{}]", shapeType, type);
            }
        }
        switch (type) {
            case POINT:
    ...
    private static EnvelopeBuilder parseBBox(StreamTokenizer stream) throws IOException, FesenParseException {
    ...
    private static List<Coordinate> parseCoordinateList(StreamTokenizer stream, final boolean ignoreZValue, final boolean coerce)
        ...
        if (isOpenParen && nextCloser(stream).equals(RPAREN) == false) {
            throw new FesenParseException("expected: [{}]" + RPAREN + " but found: [{}]" + tokenString(stream), stream.lineno());
        }
        ...
        if (coordinates.size() >= coordinatesNeeded) {
            if (!coordinates.get(0).equals(coordinates.get(coordinates.size() - 1))) {
                if (coerce) {
                    ...
                    throw new FesenParseException("invalid LinearRing found (coordinates are not closed)");
                }
            }
        }
    ...
    private static double nextNumber(StreamTokenizer stream) throws IOException, FesenParseException {
        if (stream.nextToken() == StreamTokenizer.TT_WORD) {
            if (stream.sval.equalsIgnoreCase(NAN)) {
                ...
                try {
                    ...
                    throw new FesenParseException("invalid number found: " + stream.sval, stream.lineno());
                }
            }
        }
        throw new FesenParseException("expected number but found: " + tokenString(stream), stream.lineno());
    }
    ...
    private static String nextEmptyOrOpen(StreamTokenizer stream) throws IOException, FesenParseException {
    ...
    private static String nextComma(StreamTokenizer stream) throws IOException, FesenParseException {
        if (nextWord(stream).equals(COMMA)) {
            return COMMA;
        }
        throw new FesenParseException("expected " + COMMA + " but found: " + tokenString(stream), stream.lineno());
    }

    private static String nextCloserOrComma(StreamTokenizer stream) throws IOException, FesenParseException {
