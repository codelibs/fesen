...
public class RepositoriesMetadata extends AbstractNamedDiffable<Custom> implements Custom {
     ...
     * in {@link org.codelibs.fesen.action.admin.cluster.repositories.get.GetRepositoriesResponse}.
     */
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (parser.nextToken() != XContentParser.Token.START_OBJECT) {
                    throw new FesenParseException("failed to parse repository [{}], expected object", name);
                }
                String type = null;
                    ...
                    if (token == XContentParser.Token.FIELD_NAME) {
                        ...
                        if ("type".equals(currentFieldName)) {
                            if (parser.nextToken() != XContentParser.Token.VALUE_STRING) {
                                throw new FesenParseException("failed to parse repository [{}], unknown type", name);
                            }
                            type = parser.text();
                        } else if ("settings".equals(currentFieldName)) {
                            if (parser.nextToken() != XContentParser.Token.START_OBJECT) {
                                throw new FesenParseException("failed to parse repository [{}], incompatible params", name);
                            }
                ...
                if (type == null) {
                    throw new FesenParseException("failed to parse repository [{}], missing repository type", name);
                }
                repository.add(new RepositoryMetadata(name, type, settings, generation, pendingGeneration));
            } else {
                throw new FesenParseException("failed to parse repositories");
            }
        }
