...
public class InternalClusterInfoService implements ClusterInfoService, ClusterStateListener {
    ...
    public static final Setting<TimeValue> INTERNAL_CLUSTER_INFO_TIMEOUT_SETTING = Setting
            .positiveTimeSetting("cluster.info.update.timeout", TimeValue.timeValueSeconds(15), Property.Dynamic, Property.NodeScope);
    ...
    public ClusterInfo getClusterInfo() {
        ...
        return new ClusterInfo(leastAvailableSpaceUsages, mostAvailableSpaceUsages, indicesStatsSummary.shardSizes,
    ...
    public final ClusterInfo refresh() {
        ...
        final CountDownLatch nodeLatch = updateNodeStats(new ActionListener<NodesStatsResponse>() {
            ...
            public void onResponse(NodesStatsResponse nodesStatsResponse) {
                ...
                fillDiskUsagePerNode(logger, adjustNodesStats(nodesStatsResponse.getNodes()), leastAvailableUsagesBuilder,
    ...
    static void buildShardLevelInfo(Logger logger, ShardStats[] stats, ImmutableOpenMap.Builder<String, Long> shardSizes,
        ...
        for (ShardStats s : stats) {
            ...
            if (reserved != StoreStats.UNKNOWN_RESERVED_BYTES) {
                final ClusterInfo.ReservedSpace.Builder reservedSpaceBuilder =
                        reservedSpaceByShard.computeIfAbsent(new ClusterInfo.NodeAndPath(shardRouting.currentNodeId(), s.getDataPath()),
            ...
            if (nodeStats.getFs() == null) {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            ...
                            nodeId, mostAvailablePath.getTotal(), mostAvailablePath.getAvailable(), leastAvailablePath.getTotal(),
                            leastAvailablePath.getAvailable());
                }
                if (leastAvailablePath.getTotal().getBytes() < 0) {
                    if (logger.isTraceEnabled()) {
                        logger.trace("node: [{}] least available path has less than 0 total bytes of disk [{}], skipping", nodeId,
                ...
                if (mostAvailablePath.getTotal().getBytes() < 0) {
                    if (logger.isTraceEnabled()) {
                        logger.trace("node: [{}] most available path has less than 0 total bytes of disk [{}], skipping", nodeId,
    ...
    private static class IndicesStatsSummary {
        ...
        IndicesStatsSummary(ImmutableOpenMap<String, Long> shardSizes, ImmutableOpenMap<ShardRouting, String> shardRoutingToDataPath,
