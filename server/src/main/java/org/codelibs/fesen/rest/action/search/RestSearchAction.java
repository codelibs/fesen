...
import static org.codelibs.fesen.search.suggest.SuggestBuilders.termSuggestion;
...
import org.codelibs.fesen.action.search.SearchContextId;
...
import org.codelibs.fesen.search.suggest.term.TermSuggestionBuilder.SuggestMode;
...
public class RestSearchAction extends BaseRestHandler {
    ...
    public List<Route> routes() {
        return unmodifiableList(asList(new Route(GET, "/_search"), new Route(POST, "/_search"), new Route(GET, "/{index}/_search"),
     ...
     */
    public static void parseSearchRequest(SearchRequest searchRequest, RestRequest request, XContentParser requestContentParser,
            NamedWriteableRegistry namedWriteableRegistry, IntConsumer setSize) throws IOException {
        ...
        if (request.hasParam("max_concurrent_shard_requests")) {
            ...
            final int maxConcurrentShardRequests =
                    request.paramAsInt("max_concurrent_shard_requests", searchRequest.getMaxConcurrentShardRequests());
        ...
        if (searchRequest.pointInTimeBuilder() != null) {
            ...
            searchRequest
                    .setCcsMinimizeRoundtrips(request.paramAsBoolean("ccs_minimize_roundtrips", searchRequest.isCcsMinimizeRoundtrips()));
        }
    }
    ...
    private static void parseSearchSource(final SearchSourceBuilder searchSourceBuilder, RestRequest request, IntConsumer setSize) {
        ...
        if (request.hasParam("track_total_hits")) {
            if (Booleans.isBoolean(request.param("track_total_hits"))) {
                ...
                searchSourceBuilder
                        .trackTotalHitsUpTo(request.paramAsInt("track_total_hits", SearchContext.DEFAULT_TRACK_TOTAL_HITS_UP_TO));
            }
        }
    ...
    static void preparePointInTime(SearchRequest request, RestRequest restRequest, NamedWriteableRegistry namedWriteableRegistry) {
        ...
        final IndicesOptions stricterIndicesOptions = IndicesOptions.fromOptions(indicesOptions.ignoreUnavailable(),
                indicesOptions.allowNoIndices(), false, false, false, true, true, indicesOptions.ignoreThrottled());
