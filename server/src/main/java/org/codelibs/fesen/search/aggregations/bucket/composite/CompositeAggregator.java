...
import static org.codelibs.fesen.search.aggregations.MultiBucketConsumerService.MAX_BUCKET_SETTING;
...
import org.codelibs.fesen.search.aggregations.BucketCollector;
...
final class CompositeAggregator extends BucketsAggregator {
    ...
    CompositeAggregator(String name, AggregatorFactories factories, SearchContext context, Aggregator parent, Map<String, Object> metadata,
        ...
        if (size > bucketLimit) {
            throw new MultiBucketConsumerService.TooManyBucketsException(
                    "Trying to create too many buckets. Must be less than or equal" + " to: [" + bucketLimit + "] but was [" + size
                            + "]. This limit can be set by changing the [" + MAX_BUCKET_SETTING.getKey() + "] cluster level setting.",
                    bucketLimit);
        }
        this.sourceConfigs = sourceConfigs;
        for (int i = 0; i < sourceConfigs.length; i++) {
            this.sources[i] = sourceConfigs[i].createValuesSource(context.bigArrays(), context.searcher().getIndexReader(), size,
    ...
    public InternalAggregation[] buildAggregations(long[] owningBucketOrds) throws IOException {
        ...
        return new InternalAggregation[] { new InternalComposite(name, size, sourceNames, formats, Arrays.asList(buckets), lastBucket,
                reverseMuls, earlyTerminated, metadata()) };
    }
    ...
    private Sort buildIndexSortPrefix(LeafReaderContext context) throws IOException {
        ...
        for (int i = 0; i < end; i++) {
            ...
            if (source.fieldType == null
                    ...
                    || source.missingBucket || indexSortField.getField().equals(source.fieldType.name()) == false
                    || isMaybeMultivalued(context, indexSortField) || sourceConfig.hasScript()) {
                break;
            }
                                ...
                                return new LongLeafComparator(context) {
                                    ...
                                    protected NumericDocValues getNumericDocValues(LeafReaderContext context, String field)
                                        ...
                                        NumericDocValues dvs =
                                                SortedNumericSelector.wrap(DocValues.getSortedNumeric(context.reader(), field),
    ...
    private void processLeafFromQuery(LeafReaderContext ctx, Sort indexSortPrefix) throws IOException {
        ...
        BooleanQuery newQuery = new BooleanQuery.Builder().add(context.query(), BooleanClause.Occur.MUST)
                .add(new SearchAfterSortedDocQuery(applySortFieldRounding(indexSortPrefix), fieldDoc), BooleanClause.Occur.FILTER).build();
        ...
        if (scorer != null) {
            ...
            final LeafBucketCollector inner =
                    queue.getLeafCollector(ctx, getFirstPassCollector(docIdSetBuilder, indexSortPrefix.getSort().length));
    ...
    protected LeafBucketCollector getLeafCollector(LeafReaderContext ctx, LeafBucketCollector sub) throws IOException {
        ...
        SortedDocsProducer sortedDocsProducer =
                sortPrefixLen == 0 ? sources[0].createSortedDocsProducerOrNull(ctx.reader(), context.query()) : null;
