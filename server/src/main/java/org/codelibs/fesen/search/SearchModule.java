...
import static org.codelibs.fesen.index.query.CommonTermsQueryBuilder.COMMON_TERMS_QUERY_DEPRECATION_MSG;
...
import org.codelibs.fesen.common.geo.ShapesAvailability;
...
import org.codelibs.fesen.common.xcontent.ParseFieldRegistry;
...
import org.codelibs.fesen.index.query.DisMaxQueryBuilder;
import org.codelibs.fesen.index.query.DistanceFeatureQueryBuilder;
import org.codelibs.fesen.index.query.ExistsQueryBuilder;
import org.codelibs.fesen.index.query.FieldMaskingSpanQueryBuilder;
import org.codelibs.fesen.index.query.FuzzyQueryBuilder;
import org.codelibs.fesen.index.query.GeoBoundingBoxQueryBuilder;
import org.codelibs.fesen.index.query.GeoDistanceQueryBuilder;
import org.codelibs.fesen.index.query.GeoPolygonQueryBuilder;
import org.codelibs.fesen.index.query.GeoShapeQueryBuilder;
import org.codelibs.fesen.index.query.IdsQueryBuilder;
import org.codelibs.fesen.index.query.IntervalQueryBuilder;
import org.codelibs.fesen.index.query.IntervalsSourceProvider;
import org.codelibs.fesen.index.query.MatchAllQueryBuilder;
import org.codelibs.fesen.index.query.MatchBoolPrefixQueryBuilder;
...
import org.codelibs.fesen.index.query.MultiMatchQueryBuilder;
...
import org.codelibs.fesen.index.query.ScriptQueryBuilder;
import org.codelibs.fesen.index.query.SimpleQueryStringBuilder;
import org.codelibs.fesen.index.query.SpanContainingQueryBuilder;
import org.codelibs.fesen.index.query.SpanFirstQueryBuilder;
import org.codelibs.fesen.index.query.SpanMultiTermQueryBuilder;
import org.codelibs.fesen.index.query.SpanNearQueryBuilder;
import org.codelibs.fesen.index.query.SpanNearQueryBuilder.SpanGapQueryBuilder;
import org.codelibs.fesen.index.query.SpanNotQueryBuilder;
import org.codelibs.fesen.index.query.SpanOrQueryBuilder;
import org.codelibs.fesen.index.query.SpanTermQueryBuilder;
import org.codelibs.fesen.index.query.SpanWithinQueryBuilder;
import org.codelibs.fesen.index.query.TermQueryBuilder;
import org.codelibs.fesen.index.query.TermsQueryBuilder;
import org.codelibs.fesen.index.query.TermsSetQueryBuilder;
import org.codelibs.fesen.index.query.TypeQueryBuilder;
import org.codelibs.fesen.index.query.WildcardQueryBuilder;
import org.codelibs.fesen.index.query.WrapperQueryBuilder;
import org.codelibs.fesen.index.query.functionscore.ExponentialDecayFunctionBuilder;
import org.codelibs.fesen.index.query.functionscore.FieldValueFactorFunctionBuilder;
...
import org.codelibs.fesen.index.query.functionscore.LinearDecayFunctionBuilder;
import org.codelibs.fesen.index.query.functionscore.RandomScoreFunctionBuilder;
import org.codelibs.fesen.index.query.functionscore.ScoreFunctionBuilder;
import org.codelibs.fesen.index.query.functionscore.ScriptScoreFunctionBuilder;
import org.codelibs.fesen.index.query.functionscore.ScriptScoreQueryBuilder;
import org.codelibs.fesen.index.query.functionscore.WeightBuilder;
import org.codelibs.fesen.plugins.SearchPlugin;
import org.codelibs.fesen.plugins.SearchPlugin.AggregationSpec;
import org.codelibs.fesen.plugins.SearchPlugin.FetchPhaseConstructionContext;
import org.codelibs.fesen.plugins.SearchPlugin.PipelineAggregationSpec;
import org.codelibs.fesen.plugins.SearchPlugin.QuerySpec;
import org.codelibs.fesen.plugins.SearchPlugin.RescorerSpec;
import org.codelibs.fesen.plugins.SearchPlugin.ScoreFunctionSpec;
import org.codelibs.fesen.plugins.SearchPlugin.SearchExtSpec;
import org.codelibs.fesen.plugins.SearchPlugin.SearchExtensionSpec;
import org.codelibs.fesen.plugins.SearchPlugin.SignificanceHeuristicSpec;
import org.codelibs.fesen.plugins.SearchPlugin.SuggesterSpec;
...
import org.codelibs.fesen.search.aggregations.bucket.adjacency.InternalAdjacencyMatrix;
import org.codelibs.fesen.search.aggregations.bucket.composite.CompositeAggregationBuilder;
import org.codelibs.fesen.search.aggregations.bucket.composite.InternalComposite;
...
import org.codelibs.fesen.search.aggregations.bucket.filter.InternalFilter;
import org.codelibs.fesen.search.aggregations.bucket.filter.InternalFilters;
...
import org.codelibs.fesen.search.aggregations.bucket.global.InternalGlobal;
...
import org.codelibs.fesen.search.aggregations.bucket.histogram.InternalAutoDateHistogram;
import org.codelibs.fesen.search.aggregations.bucket.histogram.InternalDateHistogram;
import org.codelibs.fesen.search.aggregations.bucket.histogram.InternalHistogram;
import org.codelibs.fesen.search.aggregations.bucket.histogram.InternalVariableWidthHistogram;
import org.codelibs.fesen.search.aggregations.bucket.histogram.VariableWidthHistogramAggregationBuilder;
import org.codelibs.fesen.search.aggregations.bucket.missing.InternalMissing;
import org.codelibs.fesen.search.aggregations.bucket.missing.MissingAggregationBuilder;
import org.codelibs.fesen.search.aggregations.bucket.nested.InternalNested;
import org.codelibs.fesen.search.aggregations.bucket.nested.InternalReverseNested;
...
import org.codelibs.fesen.search.aggregations.bucket.range.InternalBinaryRange;
import org.codelibs.fesen.search.aggregations.bucket.range.InternalDateRange;
import org.codelibs.fesen.search.aggregations.bucket.range.InternalGeoDistance;
import org.codelibs.fesen.search.aggregations.bucket.range.InternalRange;
...
import org.codelibs.fesen.search.aggregations.bucket.sampler.UnmappedSampler;
import org.codelibs.fesen.search.aggregations.bucket.terms.DoubleTerms;
import org.codelibs.fesen.search.aggregations.bucket.terms.LongRareTerms;
import org.codelibs.fesen.search.aggregations.bucket.terms.LongTerms;
import org.codelibs.fesen.search.aggregations.bucket.terms.RareTermsAggregationBuilder;
...
import org.codelibs.fesen.search.aggregations.bucket.terms.StringRareTerms;
...
import org.codelibs.fesen.search.aggregations.bucket.terms.UnmappedRareTerms;
import org.codelibs.fesen.search.aggregations.bucket.terms.UnmappedSignificantTerms;
import org.codelibs.fesen.search.aggregations.bucket.terms.UnmappedTerms;
import org.codelibs.fesen.search.aggregations.bucket.terms.heuristic.ChiSquare;
import org.codelibs.fesen.search.aggregations.bucket.terms.heuristic.GND;
import org.codelibs.fesen.search.aggregations.bucket.terms.heuristic.JLHScore;
import org.codelibs.fesen.search.aggregations.bucket.terms.heuristic.MutualInformation;
import org.codelibs.fesen.search.aggregations.bucket.terms.heuristic.PercentageScore;
import org.codelibs.fesen.search.aggregations.bucket.terms.heuristic.ScriptHeuristic;
...
import org.codelibs.fesen.search.aggregations.metrics.InternalAvg;
import org.codelibs.fesen.search.aggregations.metrics.InternalCardinality;
import org.codelibs.fesen.search.aggregations.metrics.InternalExtendedStats;
import org.codelibs.fesen.search.aggregations.metrics.InternalGeoBounds;
import org.codelibs.fesen.search.aggregations.metrics.InternalGeoCentroid;
...
import org.codelibs.fesen.search.aggregations.metrics.InternalMedianAbsoluteDeviation;
import org.codelibs.fesen.search.aggregations.metrics.InternalMin;
import org.codelibs.fesen.search.aggregations.metrics.InternalScriptedMetric;
import org.codelibs.fesen.search.aggregations.metrics.InternalStats;
import org.codelibs.fesen.search.aggregations.metrics.InternalSum;
...
import org.codelibs.fesen.search.aggregations.metrics.InternalTopHits;
import org.codelibs.fesen.search.aggregations.metrics.InternalValueCount;
import org.codelibs.fesen.search.aggregations.metrics.InternalWeightedAvg;
...
import org.codelibs.fesen.search.aggregations.pipeline.AvgBucketPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.AvgBucketPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.BucketScriptPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.BucketScriptPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.BucketSelectorPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.BucketSelectorPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.BucketSortPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.BucketSortPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.CumulativeSumPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.CumulativeSumPipelineAggregator;
...
import org.codelibs.fesen.search.aggregations.pipeline.EwmaModel;
import org.codelibs.fesen.search.aggregations.pipeline.ExtendedStatsBucketParser;
import org.codelibs.fesen.search.aggregations.pipeline.ExtendedStatsBucketPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.ExtendedStatsBucketPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.HoltLinearModel;
import org.codelibs.fesen.search.aggregations.pipeline.HoltWintersModel;
...
import org.codelibs.fesen.search.aggregations.pipeline.InternalPercentilesBucket;
import org.codelibs.fesen.search.aggregations.pipeline.InternalSimpleValue;
import org.codelibs.fesen.search.aggregations.pipeline.InternalStatsBucket;
import org.codelibs.fesen.search.aggregations.pipeline.LinearModel;
import org.codelibs.fesen.search.aggregations.pipeline.MaxBucketPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.MaxBucketPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.MinBucketPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.MinBucketPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.MovAvgModel;
import org.codelibs.fesen.search.aggregations.pipeline.MovAvgPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.MovAvgPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.MovFnPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.MovFnPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.PercentilesBucketPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.PercentilesBucketPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.PipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.SerialDiffPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.SerialDiffPipelineAggregator;
...
import org.codelibs.fesen.search.aggregations.pipeline.StatsBucketPipelineAggregator;
import org.codelibs.fesen.search.aggregations.pipeline.SumBucketPipelineAggregationBuilder;
import org.codelibs.fesen.search.aggregations.pipeline.SumBucketPipelineAggregator;
...
import org.codelibs.fesen.search.fetch.subphase.FetchDocValuesPhase;
import org.codelibs.fesen.search.fetch.subphase.FetchFieldsPhase;
import org.codelibs.fesen.search.fetch.subphase.FetchScorePhase;
import org.codelibs.fesen.search.fetch.subphase.FetchSourcePhase;
import org.codelibs.fesen.search.fetch.subphase.FetchVersionPhase;
import org.codelibs.fesen.search.fetch.subphase.MatchedQueriesPhase;
import org.codelibs.fesen.search.fetch.subphase.ScriptFieldsPhase;
import org.codelibs.fesen.search.fetch.subphase.SeqNoPrimaryTermPhase;
import org.codelibs.fesen.search.fetch.subphase.highlight.FastVectorHighlighter;
import org.codelibs.fesen.search.fetch.subphase.highlight.HighlightPhase;
...
import org.codelibs.fesen.search.sort.GeoDistanceSortBuilder;
import org.codelibs.fesen.search.sort.ScoreSortBuilder;
import org.codelibs.fesen.search.sort.ScriptSortBuilder;
import org.codelibs.fesen.search.sort.SortBuilder;
import org.codelibs.fesen.search.sort.SortValue;
...
import org.codelibs.fesen.search.suggest.completion.CompletionSuggestionBuilder;
import org.codelibs.fesen.search.suggest.phrase.Laplace;
import org.codelibs.fesen.search.suggest.phrase.LinearInterpolation;
import org.codelibs.fesen.search.suggest.phrase.PhraseSuggestion;
import org.codelibs.fesen.search.suggest.phrase.PhraseSuggestionBuilder;
import org.codelibs.fesen.search.suggest.phrase.SmoothingModel;
import org.codelibs.fesen.search.suggest.phrase.StupidBackoff;
...
public class SearchModule {
    public static final Setting<Integer> INDICES_MAX_CLAUSE_COUNT_SETTING =
            Setting.intSetting("indices.query.bool.max_clause_count", 1024, 1, Integer.MAX_VALUE, Setting.Property.NodeScope);

    private final boolean transportClient;
    private final Map<String, Highlighter> highlighters;
    private final ParseFieldRegistry<MovAvgModel.AbstractModelParser> movingAverageModelParserRegistry =
            new ParseFieldRegistry<>("moving_avg_model");
    ...
    private ValuesSourceRegistry registerAggregations(List<SearchPlugin> plugins) {
        ...
        registerAggregation(new AggregationSpec(AvgAggregationBuilder.NAME, AvgAggregationBuilder::new, AvgAggregationBuilder.PARSER)
                .addResultReader(InternalAvg::new).setAggregatorRegistrar(AvgAggregationBuilder::registerAggregators), builder);
        registerAggregation(new AggregationSpec(WeightedAvgAggregationBuilder.NAME, WeightedAvgAggregationBuilder::new,
                WeightedAvgAggregationBuilder.PARSER).addResultReader(InternalWeightedAvg::new)
                        .setAggregatorRegistrar(WeightedAvgAggregationBuilder::registerUsage),
                builder);
        registerAggregation(new AggregationSpec(SumAggregationBuilder.NAME, SumAggregationBuilder::new, SumAggregationBuilder.PARSER)
                .addResultReader(InternalSum::new).setAggregatorRegistrar(SumAggregationBuilder::registerAggregators), builder);
        registerAggregation(new AggregationSpec(MinAggregationBuilder.NAME, MinAggregationBuilder::new, MinAggregationBuilder.PARSER)
                .addResultReader(InternalMin::new).setAggregatorRegistrar(MinAggregationBuilder::registerAggregators), builder);
        registerAggregation(new AggregationSpec(MaxAggregationBuilder.NAME, MaxAggregationBuilder::new, MaxAggregationBuilder.PARSER)
                .addResultReader(InternalMax::new).setAggregatorRegistrar(MaxAggregationBuilder::registerAggregators), builder);
        registerAggregation(new AggregationSpec(StatsAggregationBuilder.NAME, StatsAggregationBuilder::new, StatsAggregationBuilder.PARSER)
                .addResultReader(InternalStats::new).setAggregatorRegistrar(StatsAggregationBuilder::registerAggregators), builder);
        ...
        registerAggregation(new AggregationSpec(PercentilesAggregationBuilder.NAME, PercentilesAggregationBuilder::new,
                PercentilesAggregationBuilder::parse).addResultReader(InternalTDigestPercentiles.NAME, InternalTDigestPercentiles::new)
        ...
        registerAggregation(
                new AggregationSpec(MedianAbsoluteDeviationAggregationBuilder.NAME, MedianAbsoluteDeviationAggregationBuilder::new,
                        MedianAbsoluteDeviationAggregationBuilder.PARSER).addResultReader(InternalMedianAbsoluteDeviation::new)
        ...
        registerAggregation(
                new AggregationSpec(MissingAggregationBuilder.NAME, MissingAggregationBuilder::new, MissingAggregationBuilder.PARSER)
                        .addResultReader(InternalMissing::new).setAggregatorRegistrar(MissingAggregationBuilder::registerAggregators),
                builder);
        registerAggregation(
        ...
        registerAggregation(new AggregationSpec(TermsAggregationBuilder.NAME, TermsAggregationBuilder::new, TermsAggregationBuilder.PARSER)
                .addResultReader(StringTerms.NAME, StringTerms::new).addResultReader(UnmappedTerms.NAME, UnmappedTerms::new)
                .addResultReader(LongTerms.NAME, LongTerms::new).addResultReader(DoubleTerms.NAME, DoubleTerms::new)
        ...
        registerAggregation(new AggregationSpec(SignificantTermsAggregationBuilder.NAME, SignificantTermsAggregationBuilder::new,
                SignificantTermsAggregationBuilder::parse).addResultReader(SignificantStringTerms.NAME, SignificantStringTerms::new)
        ...
        registerAggregation(new AggregationSpec(RangeAggregationBuilder.NAME, RangeAggregationBuilder::new, RangeAggregationBuilder.PARSER)
                .addResultReader(InternalRange::new).setAggregatorRegistrar(RangeAggregationBuilder::registerAggregators), builder);
        registerAggregation(
                new AggregationSpec(DateRangeAggregationBuilder.NAME, DateRangeAggregationBuilder::new, DateRangeAggregationBuilder.PARSER)
                        .addResultReader(InternalDateRange::new).setAggregatorRegistrar(DateRangeAggregationBuilder::registerAggregators),
                builder);
        registerAggregation(
                new AggregationSpec(IpRangeAggregationBuilder.NAME, IpRangeAggregationBuilder::new, IpRangeAggregationBuilder.PARSER)
                        .addResultReader(InternalBinaryRange::new).setAggregatorRegistrar(IpRangeAggregationBuilder::registerAggregators),
                builder);
        registerAggregation(
                new AggregationSpec(HistogramAggregationBuilder.NAME, HistogramAggregationBuilder::new, HistogramAggregationBuilder.PARSER)
                        .addResultReader(InternalHistogram::new).setAggregatorRegistrar(HistogramAggregationBuilder::registerAggregators),
                builder);
        ...
        registerAggregation(new AggregationSpec(AutoDateHistogramAggregationBuilder.NAME, AutoDateHistogramAggregationBuilder::new,
                ...
                builder);
        registerAggregation(
                new AggregationSpec(VariableWidthHistogramAggregationBuilder.NAME, VariableWidthHistogramAggregationBuilder::new,
                        VariableWidthHistogramAggregationBuilder.PARSER).addResultReader(InternalVariableWidthHistogram::new)
        ...
        registerAggregation(new AggregationSpec(GeoDistanceAggregationBuilder.NAME, GeoDistanceAggregationBuilder::new,
                GeoDistanceAggregationBuilder::parse).addResultReader(InternalGeoDistance::new)
                        .setAggregatorRegistrar(GeoDistanceAggregationBuilder::registerAggregators),
                builder);
        ...
        registerAggregation(
                new AggregationSpec(CompositeAggregationBuilder.NAME, CompositeAggregationBuilder::new, CompositeAggregationBuilder.PARSER)
                        .addResultReader(InternalComposite::new).setAggregatorRegistrar(CompositeAggregationBuilder::registerAggregators),
                builder);
    ...
    private void registerAggregation(AggregationSpec spec, ValuesSourceRegistry.Builder builder) {
        ...
        namedWriteables
                .add(new NamedWriteableRegistry.Entry(AggregationBuilder.class, spec.getName().getPreferredName(), spec.getReader()));
    ...
    private void registerPipelineAggregations(List<SearchPlugin> plugins) {
        registerPipelineAggregation(new PipelineAggregationSpec(DerivativePipelineAggregationBuilder.NAME,
                DerivativePipelineAggregationBuilder::new, DerivativePipelineAggregator::new, DerivativePipelineAggregationBuilder::parse)
                        .addResultReader(InternalDerivative::new));
        registerPipelineAggregation(new PipelineAggregationSpec(MaxBucketPipelineAggregationBuilder.NAME,
                MaxBucketPipelineAggregationBuilder::new, MaxBucketPipelineAggregator::new, MaxBucketPipelineAggregationBuilder.PARSER)
        ...
        registerPipelineAggregation(new PipelineAggregationSpec(MinBucketPipelineAggregationBuilder.NAME,
                MinBucketPipelineAggregationBuilder::new, MinBucketPipelineAggregator::new, MinBucketPipelineAggregationBuilder.PARSER)
        /* Uses InternalBucketMetricValue */);
        registerPipelineAggregation(new PipelineAggregationSpec(AvgBucketPipelineAggregationBuilder.NAME,
                AvgBucketPipelineAggregationBuilder::new, AvgBucketPipelineAggregator::new, AvgBucketPipelineAggregationBuilder.PARSER)
                        // This bucket is used by many pipeline aggreations.
                        .addResultReader(InternalSimpleValue.NAME, InternalSimpleValue::new));
        registerPipelineAggregation(new PipelineAggregationSpec(SumBucketPipelineAggregationBuilder.NAME,
                SumBucketPipelineAggregationBuilder::new, SumBucketPipelineAggregator::new, SumBucketPipelineAggregationBuilder.PARSER)
        /* Uses InternalSimpleValue */);
        registerPipelineAggregation(
                new PipelineAggregationSpec(StatsBucketPipelineAggregationBuilder.NAME, StatsBucketPipelineAggregationBuilder::new,
                        StatsBucketPipelineAggregator::new, StatsBucketPipelineAggregationBuilder.PARSER)
                                .addResultReader(InternalStatsBucket::new));
        registerPipelineAggregation(new PipelineAggregationSpec(ExtendedStatsBucketPipelineAggregationBuilder.NAME,
                ExtendedStatsBucketPipelineAggregationBuilder::new, ExtendedStatsBucketPipelineAggregator::new,
                new ExtendedStatsBucketParser()).addResultReader(InternalExtendedStatsBucket::new));
        registerPipelineAggregation(new PipelineAggregationSpec(PercentilesBucketPipelineAggregationBuilder.NAME,
                PercentilesBucketPipelineAggregationBuilder::new, PercentilesBucketPipelineAggregator::new,
                PercentilesBucketPipelineAggregationBuilder.PARSER).addResultReader(InternalPercentilesBucket::new));
        registerPipelineAggregation(
                new PipelineAggregationSpec(MovAvgPipelineAggregationBuilder.NAME, MovAvgPipelineAggregationBuilder::new,
                        MovAvgPipelineAggregator::new, (XContentParser parser, String name) -> MovAvgPipelineAggregationBuilder
                                .parse(movingAverageModelParserRegistry, name, parser))/* Uses InternalHistogram for buckets */);
        registerPipelineAggregation(
                new PipelineAggregationSpec(CumulativeSumPipelineAggregationBuilder.NAME, CumulativeSumPipelineAggregationBuilder::new,
                        CumulativeSumPipelineAggregator::new, CumulativeSumPipelineAggregationBuilder.PARSER));
        registerPipelineAggregation(
                new PipelineAggregationSpec(BucketScriptPipelineAggregationBuilder.NAME, BucketScriptPipelineAggregationBuilder::new,
                        BucketScriptPipelineAggregator::new, BucketScriptPipelineAggregationBuilder.PARSER));
        registerPipelineAggregation(
                new PipelineAggregationSpec(BucketSelectorPipelineAggregationBuilder.NAME, BucketSelectorPipelineAggregationBuilder::new,
                        BucketSelectorPipelineAggregator::new, BucketSelectorPipelineAggregationBuilder::parse));
        registerPipelineAggregation(new PipelineAggregationSpec(BucketSortPipelineAggregationBuilder.NAME,
                BucketSortPipelineAggregationBuilder::new, BucketSortPipelineAggregator::new, BucketSortPipelineAggregationBuilder::parse));
        registerPipelineAggregation(new PipelineAggregationSpec(SerialDiffPipelineAggregationBuilder.NAME,
                SerialDiffPipelineAggregationBuilder::new, SerialDiffPipelineAggregator::new, SerialDiffPipelineAggregationBuilder::parse));
        registerPipelineAggregation(new PipelineAggregationSpec(MovFnPipelineAggregationBuilder.NAME, MovFnPipelineAggregationBuilder::new,
                MovFnPipelineAggregator::new, MovFnPipelineAggregationBuilder.PARSER));
    ...
    private void registerPipelineAggregation(PipelineAggregationSpec spec) {
        ...
        if (spec.getAggregatorReader() != null) {
            namedWriteables.add(new NamedWriteableRegistry.Entry(PipelineAggregator.class, spec.getName().getPreferredName(),
                    spec.getAggregatorReader()));
        }
    ...
    private void registerSuggesters(List<SearchPlugin> plugins) {
        ...
        registerSuggester(new SuggesterSpec<>(TermSuggestionBuilder.SUGGESTION_NAME, TermSuggestionBuilder::new,
                TermSuggestionBuilder::fromXContent, TermSuggestion::new));

        registerSuggester(new SuggesterSpec<>(PhraseSuggestionBuilder.SUGGESTION_NAME, PhraseSuggestionBuilder::new,
                PhraseSuggestionBuilder::fromXContent, PhraseSuggestion::new));

        registerSuggester(new SuggesterSpec<>(CompletionSuggestionBuilder.SUGGESTION_NAME, CompletionSuggestionBuilder::new,
                CompletionSuggestionBuilder::fromXContent, CompletionSuggestion::new));
    ...
    private void registerSuggester(SuggesterSpec<?> suggester) {
        ...
        namedWriteables.add(new NamedWriteableRegistry.Entry(Suggest.Suggestion.class, suggester.getName().getPreferredName(),
    ...
    private void registerScoreFunctions(List<SearchPlugin> plugins) {
        ...
        namedWriteables.add(new NamedWriteableRegistry.Entry(ScriptScoreFunctionBuilder.class, ScriptScoreFunctionBuilder.NAME,
    ...
    private void registerScoreFunction(ScoreFunctionSpec<?> scoreFunction) {
        namedWriteables.add(new NamedWriteableRegistry.Entry(ScoreFunctionBuilder.class, scoreFunction.getName().getPreferredName(),
    ...
    private <T extends SignificanceHeuristic> void registerSignificanceHeuristic(SignificanceHeuristicSpec<?> spec) {
        namedXContents
                .add(new NamedXContentRegistry.Entry(SignificanceHeuristic.class, spec.getName(), p -> spec.getParser().apply(p, null)));
        namedWriteables
                .add(new NamedWriteableRegistry.Entry(SignificanceHeuristic.class, spec.getName().getPreferredName(), spec.getReader()));
    }
    ...
    private void registerQueryParsers(List<SearchPlugin> plugins) {
        ...
        registerQuery(new QuerySpec<>(MatchBoolPrefixQueryBuilder.NAME, MatchBoolPrefixQueryBuilder::new,
                MatchBoolPrefixQueryBuilder::fromXContent));
