...
import org.codelibs.fesen.search.fetch.subphase.highlight.SearchHighlightContext.Field;
import org.codelibs.fesen.search.fetch.subphase.highlight.SearchHighlightContext.FieldOptions;
...
public class FastVectorHighlighter implements Highlighter {
    ...
    public HighlightField highlight(FieldHighlightContext fieldContext) throws IOException {
        ...
        if (entry == null) {
            ...
            if (field.fieldOptions().requireFieldMatch()) {
                 ...
                 */
                entry.fieldMatchFieldQuery = new CustomFieldQuery(fieldContext.query, hitContext.topLevelReader(), true,
                 ...
                 * with use caching it across hits (and across readers...)
                 */
                entry.noFieldMatchFieldQuery = new CustomFieldQuery(fieldContext.query, hitContext.topLevelReader(), true,
                        field.fieldOptions().requireFieldMatch());
            }
        ...
        int numberOfFragments =
                field.fieldOptions().numberOfFragments() == 0 ? Integer.MAX_VALUE : field.fieldOptions().numberOfFragments();
        ...
        if (field.fieldOptions().matchedFields() != null && !field.fieldOptions().matchedFields().isEmpty()) {
            fragments = cache.fvh.getBestFragments(fieldQuery, hitContext.reader(), hitContext.docId(), fieldType.name(),
                    field.fieldOptions().matchedFields(), fragmentCharSize, numberOfFragments, entry.fragListBuilder, fragmentsBuilder,
                    field.fieldOptions().preTags(), field.fieldOptions().postTags(), encoder);
        } else {
            fragments = cache.fvh.getBestFragments(fieldQuery, hitContext.reader(), hitContext.docId(), fieldType.name(), fragmentCharSize,
        ...
        if (noMatchSize > 0) {
            ...
            fragments = fragmentsBuilder.createFragments(hitContext.reader(), hitContext.docId(), fieldType.name(), fieldFragList, 1,
                    field.fieldOptions().preTags(), field.fieldOptions().postTags(), encoder);
            if (CollectionUtils.isEmpty(fragments) == false) {
                return new HighlightField(fieldContext.fieldName, Text.convertFromStringArray(fragments));
            }
    ...
    private Function<SourceLookup, FragmentsBuilder> fragmentsBuilderSupplier(SearchHighlightContext.Field field, MappedFieldType fieldType,
            boolean forceSource) {
        ...
        if (!forceSource && fieldType.isStored()) {
            ...
            if (options.numberOfFragments() != 0 && options.scoreOrdered()) {
                supplier = lookup -> new SourceScoreOrderFragmentsBuilder(fieldType, lookup, options.preTags(), options.postTags(),
                        boundaryScanner);
            } else {
                supplier = lookup -> new SourceSimpleFragmentsBuilder(fieldType, lookup, options.preTags(), options.postTags(),
                        boundaryScanner);
            }
        }
    ...
    private static BoundaryScanner getBoundaryScanner(Field field) {
        ...
        final Locale boundaryScannerLocale =
                fieldOptions.boundaryScannerLocale() != null ? fieldOptions.boundaryScannerLocale() : Locale.ROOT;
