...
import static org.codelibs.fesen.search.sort.FieldSortBuilder.validateMaxChildrenExistOnlyInTopLevelNestedSort;
import static org.codelibs.fesen.search.sort.NestedSortBuilder.NESTED_FIELD;
...
import org.codelibs.fesen.index.fielddata.plain.AbstractLatLonPointIndexFieldData.LatLonPointIndexFieldData;
import org.codelibs.fesen.index.mapper.MappedFieldType;
import org.codelibs.fesen.index.query.GeoValidationMethod;
...
public class GeoDistanceSortBuilder extends SortBuilder<GeoDistanceSortBuilder> {
    ...
    /**
     * The distance unit to use. Defaults to {@link org.codelibs.fesen.common.unit.DistanceUnit#METERS}
     */
    ...
    /**
     * Returns the distance unit to use. Defaults to {@link org.codelibs.fesen.common.unit.DistanceUnit#METERS}
     */
    ...
    public boolean equals(Object object) {
        ...
        return Objects.equals(fieldName, other.fieldName) && Objects.deepEquals(points, other.points)
                && Objects.equals(geoDistance, other.geoDistance) && Objects.equals(unit, other.unit)
                && Objects.equals(sortMode, other.sortMode) && Objects.equals(order, other.order)
                && Objects.equals(nestedFilter, other.nestedFilter) && Objects.equals(nestedPath, other.nestedPath)
                && Objects.equals(validation, other.validation) && Objects.equals(nestedSort, other.nestedSort)
                && ignoreUnmapped == other.ignoreUnmapped;
    }

    @Override
    public int hashCode() {
        return Objects.hash(this.fieldName, this.points, this.geoDistance, this.unit, this.sortMode, this.order, this.nestedFilter,
     ...
     * {@link org.codelibs.fesen.common.xcontent.XContent} format.
     *
            ...
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (NESTED_FILTER_FIELD.match(currentName, parser.getDeprecationHandler())) {
                    ...
                    if (fieldName != null && fieldName.equals(currentName) == false) {
                        throw new ParsingException(parser.getTokenLocation(), "Trying to reset fieldName to [{}], already set to [{}].",
                ...
                if (ORDER_FIELD.match(currentName, parser.getDeprecationHandler())) {
                    ...
                    throw new ParsingException(parser.getTokenLocation(), "Only geohashes of type string supported for field [{}]",
                            currentName);
                } else {
    ...
    public SortFieldAndFormat build(QueryShardContext context) throws IOException {
        ...
        if (geoIndexFieldData.getClass() == LatLonPointIndexFieldData.class // only works with 5.x geo_point
                && nested == null && localSortMode == MultiValueMode.MIN // LatLonDocValuesField internally picks the closest point
                && unit == DistanceUnit.METERS && reverse == false && localPoints.length == 1) {
            return new SortFieldAndFormat(LatLonDocValuesField.newDistanceSort(fieldName, localPoints[0].lat(), localPoints[0].lon()),
                    DocValueFormat.RAW);
        }
    ...
    public BucketedSort buildBucketedSort(QueryShardContext context, int bucketSize, BucketedSort.ExtraData extra) throws IOException {
        ...
        return comparatorSource(localPoints, localSortMode, geoIndexFieldData, nested).newBucketedSort(context.bigArrays(), order,
                DocValueFormat.RAW, bucketSize, extra);
    }
        ...
        if (GeoValidationMethod.isIgnoreMalformed(validation) == false) {
            for (GeoPoint point : localPoints) {
                if (GeoUtils.isValidLatitude(point.lat()) == false) {
                    throw new FesenParseException("illegal latitude value [{}] for [GeoDistanceSort] for field [{}].", point.lat(),
                            fieldName);
                }
                if (GeoUtils.isValidLongitude(point.lon()) == false) {
                    throw new FesenParseException("illegal longitude value [{}] for [GeoDistanceSort] for field [{}].", point.lon(),
                            fieldName);
                }
            }
        }
        ...
        return new IndexFieldData.XFieldComparatorSource(null, localSortMode, nested) {
            ...
            private NumericDoubleValues getNumericDoubleValues(LeafReaderContext context) throws IOException {
                ...
                if (nested == null) {
                    ...
                    return localSortMode.select(distanceValues, Double.POSITIVE_INFINITY, rootDocs, innerDocs, context.reader().maxDoc(),
                            maxChildren);
                }
            }
        ...
        while (!parser.nextToken().equals(XContentParser.Token.END_ARRAY)) {
            if (parser.currentToken() == XContentParser.Token.VALUE_NUMBER) {
                ...
                if (!parser.currentToken().equals(XContentParser.Token.VALUE_NUMBER)) {
                    throw new FesenParseException("geo point parsing: expected second number but got [{}] instead", parser.currentToken());
                }
