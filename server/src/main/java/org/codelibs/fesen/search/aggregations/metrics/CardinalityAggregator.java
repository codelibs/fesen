...
import org.codelibs.fesen.common.util.BitArray;
...
public class CardinalityAggregator extends NumericMetricsAggregator.SingleValue {
    ...
    public CardinalityAggregator(String name, ValuesSourceConfig valuesSourceConfig, int precision, SearchContext context,
    ...
    private Collector pickCollector(LeafReaderContext ctx) throws IOException {
        ...
        if (valuesSource instanceof ValuesSource.Numeric) {
            ...
            MurmurHash3Values hashValues = source.isFloatingPoint() ? MurmurHash3Values.hash(source.doubleValues(ctx))
        ...
        public void postCollect() throws IOException {
            try (BitArray allVisitedOrds = new BitArray(maxOrd, bigArrays)) {
                ...
                try (LongArray hashes = bigArrays.newLongArray(maxOrd, false)) {
                    ...
                    for (long ord = allVisitedOrds.nextSetBit(0); ord < Long.MAX_VALUE; ord =
                            ord + 1 < maxOrd ? allVisitedOrds.nextSetBit(ord + 1) : Long.MAX_VALUE) {
                    ...
                    for (long bucket = visitedOrds.size() - 1; bucket >= 0; --bucket) {
                        final BitArray bits = visitedOrds.get(bucket);
                        if (bits != null) {
                            for (long ord = bits.nextSetBit(0); ord < Long.MAX_VALUE; ord =
                                    ord + 1 < maxOrd ? bits.nextSetBit(ord + 1) : Long.MAX_VALUE) {
