...
class ExtendedStatsAggregatorFactory extends ValuesSourceAggregatorFactory {
    ...
    ExtendedStatsAggregatorFactory(String name, ValuesSourceConfig config, double sigma, QueryShardContext queryShardContext,
    ...
    protected Aggregator doCreateInternal(SearchContext searchContext, Aggregator parent, CardinalityUpperBound cardinality,
        ...
        return queryShardContext.getValuesSourceRegistry().getAggregator(ExtendedStatsAggregationBuilder.REGISTRY_KEY, config).build(name,
                config, searchContext, parent, sigma, metadata);
    }
}
