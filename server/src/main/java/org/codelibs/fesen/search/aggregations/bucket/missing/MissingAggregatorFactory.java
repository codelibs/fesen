...
public class MissingAggregatorFactory extends ValuesSourceAggregatorFactory {
    ...
    public MissingAggregatorFactory(String name, ValuesSourceConfig config, QueryShardContext queryShardContext, AggregatorFactory parent,
    ...
    @Override
    protected MissingAggregator doCreateInternal(SearchContext searchContext, Aggregator parent, CardinalityUpperBound cardinality,
            Map<String, Object> metadata) throws IOException {
        return queryShardContext.getValuesSourceRegistry().getAggregator(MissingAggregationBuilder.REGISTRY_KEY, config).build(name,
                factories, config, searchContext, parent, cardinality, metadata);
    }
}
