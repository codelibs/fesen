...
public class IncludeExclude implements Writeable, ToXContentFragment {
        ...
        if (token == XContentParser.Token.VALUE_STRING) {
            ...
            while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                if (token == XContentParser.Token.FIELD_NAME) {
                    ...
                    throw new FesenParseException("Unknown parameter in Include/Exclude clause: " + currentFieldName);
                }
            }
            if (partition == null) {
                throw new IllegalArgumentException(
    ...
    private static Set<BytesRef> parseArrayToSet(XContentParser parser) throws IOException {
        ...
        if (parser.currentToken() != XContentParser.Token.START_ARRAY) {
            throw new FesenParseException("Missing start of array in include/exclude clause");
        }
        while (parser.nextToken() != XContentParser.Token.END_ARRAY) {
            if (!parser.currentToken().isValue()) {
                throw new FesenParseException("Array elements in include/exclude clauses should be string values");
            }
