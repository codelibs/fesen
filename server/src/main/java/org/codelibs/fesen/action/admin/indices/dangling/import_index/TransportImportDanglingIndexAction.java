...
package org.codelibs.fesen.action.admin.indices.dangling.import_index;
...
import org.codelibs.fesen.action.admin.indices.dangling.find.FindDanglingIndexRequest;
import org.codelibs.fesen.action.admin.indices.dangling.find.FindDanglingIndexResponse;
import org.codelibs.fesen.action.admin.indices.dangling.find.NodeFindDanglingIndexResponse;
...
public class TransportImportDanglingIndexAction extends HandledTransportAction<ImportDanglingIndexRequest, AcknowledgedResponse> {
    ...
    @Inject
    public TransportImportDanglingIndexAction(ActionFilters actionFilters, TransportService transportService,
            LocalAllocateDangledIndices danglingIndexAllocator, NodeClient nodeClient) {
    ...
    protected void doExecute(Task task, ImportDanglingIndexRequest importRequest, ActionListener<AcknowledgedResponse> importListener) {
        findDanglingIndex(importRequest, new ActionListener<IndexMetadata>() {
            ...
            public void onResponse(IndexMetadata indexMetaDataToImport) {
                ...
                danglingIndexAllocator.allocateDangled(singletonList(indexMetaDataToImport),
        ...
        this.nodeClient.execute(FindDanglingIndexAction.INSTANCE, new FindDanglingIndexRequest(indexUUID),
                new ActionListener<FindDanglingIndexResponse>() {
                    ...
                    public void onResponse(FindDanglingIndexResponse response) {
                        ...
                        logger.debug("Metadata versions {} found for index UUID [{}], selecting the highest",
                                metaDataSortedByVersion.stream().map(IndexMetadata::getVersion).collect(Collectors.toList()), indexUUID);
