...
import static org.codelibs.fesen.action.search.TransportSearchHelper.internalScrollSearchRequest;
...
abstract class SearchScrollAsyncAction<T extends SearchPhaseResult> implements Runnable {
    ...
    protected SearchScrollAsyncAction(ParsedScrollId scrollId, Logger logger, DiscoveryNodes nodes, ActionListener<SearchResponse> listener,
            SearchPhaseController searchPhaseController, SearchScrollRequest request, SearchTransportService searchTransportService) {
    ...
    public final void run() {
        ...
        if (context.length == 0) {
            ...
            collectNodesAndRun(Arrays.asList(context), nodes, searchTransportService,
    ...
    static void collectNodesAndRun(final Iterable<SearchContextIdForNode> scrollIds, DiscoveryNodes nodes,
            SearchTransportService searchTransportService, ActionListener<BiFunction<String, String, DiscoveryNode>> listener) {
        ...
        if (clusters.isEmpty()) { // no remote clusters
            ...
            remoteClusterService.collectNodes(clusters, ActionListener.map(listener, nodeFunction -> (clusterAlias,
                    node) -> clusterAlias == null ? nodes.get(node) : nodeFunction.apply(clusterAlias, node)));
        }
    }
    ...
    private void run(BiFunction<String, String, DiscoveryNode> clusterNodeLookup, final SearchContextIdForNode[] context) {
        ...
        for (int i = 0; i < context.length; i++) {
            ...
            try {
                ...
                onShardFailure("query", counter, target.getSearchContextId(), ex, null,
    ...
    protected final void sendResponse(SearchPhaseController.ReducedQueryPhase queryPhase,
        ...
        try {
            final InternalSearchResponse internalResponse =
                    searchPhaseController.merge(true, queryPhase, fetchResults.asList(), fetchResults::get);
            ...
            listener.onResponse(new SearchResponse(internalResponse, scrollId, this.scrollId.getContext().length, successfulOps.get(), 0,
                    buildTookInMillis(), buildShardFailures(), SearchResponse.Clusters.EMPTY, null));
        } catch (Exception e) {
    ...
    protected void onShardFailure(String phaseName, final CountDown counter, final ShardSearchContextId searchId, Exception failure,
            @Nullable SearchShardTarget searchShardTarget, Supplier<SearchPhase> nextPhaseSupplier) {
