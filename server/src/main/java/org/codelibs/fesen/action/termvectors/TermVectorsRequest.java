            ...
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (FIELDS.match(currentFieldName, parser.getDeprecationHandler())) {
                    if (token == XContentParser.Token.START_ARRAY) {
                        ...
                        throw new FesenParseException("failed to parse term vectors request. field [fields] must be an array");
                    }
                    ...
                    throw new FesenParseException("failed to parse term vectors request. unknown field [{}]", currentFieldName);
                }
            }
        }
        if (fields.size() > 0) {
