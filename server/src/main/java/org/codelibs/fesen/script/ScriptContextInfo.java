...
public class ScriptContextInfo implements ToXContentObject, Writeable {
    ...
    public static final ConstructingObjectParser<ScriptContextInfo, Void> PARSER = new ConstructingObjectParser<>("script_context_info",
            true, (m, name) -> new ScriptContextInfo((String) m[0], (List<ScriptMethodInfo>) m[1]));

    static {
        PARSER.declareString(constructorArg(), new ParseField(NAME_FIELD));
        PARSER.declareObjectArray(constructorArg(), (parser, ctx) -> ScriptMethodInfo.PARSER.apply(parser, ctx),
    ...
    public static class ScriptMethodInfo implements ToXContentObject, Writeable {
        ...
        @SuppressWarnings("unchecked")
        private static final ConstructingObjectParser<ScriptMethodInfo, Void> PARSER = new ConstructingObjectParser<>("method", true,
        ...
        static {
            ...
            PARSER.declareObjectArray(constructorArg(), (parser, ctx) -> ParameterInfo.PARSER.apply(parser, ctx),
        ...
        public static class ParameterInfo implements ToXContentObject, Writeable {
            ...
            private static final ConstructingObjectParser<ParameterInfo, Void> PARSER =
                    new ConstructingObjectParser<>("parameters", true, (p) -> new ParameterInfo((String) p[0], (String) p[1]));
        ...
        static ScriptMethodInfo executeFromContext(Class<?> clazz) {
            ...
            if (parameterTypes.length > 0) {
                ...
                if (!parameterNamesField.getType().equals(String[].class)) {
                    throw new IllegalArgumentException("Expected a constant [String[] PARAMETERS] on instance class [" + clazz.getName()
                            + "] for method [" + name + "] with [" + parameterTypes.length + "] parameters, found ["
                ...
                if (argumentNames.length != parameterTypes.length) {
                    throw new IllegalArgumentException("Expected argument names [" + argumentNames.length + "] to have the same arity ["
                            + parameterTypes.length + "] for method [" + name + "] of class [" + clazz.getName() + "]");
                }
        ...
        static Set<ScriptMethodInfo> gettersFromContext(Class<?> clazz) {
            ...
            for (java.lang.reflect.Method m : clazz.getMethods()) {
                if (!m.isDefault() && m.getName().startsWith("get") && !m.getName().equals("getClass")
                        && !Modifier.isStatic(m.getModifiers()) && m.getParameters().length == 0) {
