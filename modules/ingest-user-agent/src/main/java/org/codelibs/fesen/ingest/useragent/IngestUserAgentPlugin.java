...
public class IngestUserAgentPlugin extends Plugin implements IngestPlugin {
    ...
    private final Setting<Long> CACHE_SIZE_SETTING =
            Setting.longSetting("ingest.user_agent.cache_size", 1000, 0, Setting.Property.NodeScope);
    ...
    static Map<String, UserAgentParser> createUserAgentParsers(Path userAgentConfigDirectory, UserAgentCache cache) throws IOException {
        ...
        UserAgentParser defaultParser =
                new UserAgentParser(DEFAULT_PARSER_NAME, IngestUserAgentPlugin.class.getResourceAsStream("/regexes.yml"), cache);
        ...
        if (Files.exists(userAgentConfigDirectory) && Files.isDirectory(userAgentConfigDirectory)) {
            ...
            try (Stream<Path> regexFiles =
                    Files.find(userAgentConfigDirectory, 1, (path, attr) -> attr.isRegularFile() && pathMatcher.matches(path))) {
