...
import org.codelibs.fesen.common.util.URIPattern;
...
public class URLRepository extends BlobStoreRepository {
    ...
    public static final Setting<List<String>> SUPPORTED_PROTOCOLS_SETTING = Setting.listSetting("repositories.url.supported_protocols",
            Arrays.asList("http", "https", "ftp", "file", "jar"), Function.identity(), Property.NodeScope);
    ...
    public static final Setting<URL> REPOSITORIES_URL_SETTING = new Setting<>("repositories.url.url",
            (s) -> s.get("repositories.uri.url", "http:"), URLRepository::parseURL, Property.NodeScope);
     ...
     */
    public URLRepository(RepositoryMetadata metadata, Environment environment, NamedXContentRegistry namedXContentRegistry,
        ...
        url = URL_SETTING.exists(metadata.settings()) ? URL_SETTING.get(metadata.settings())
