...
public class GrokProcessorTests extends ESTestCase {
    ...
    public void testNoMatchingPatternName() {
        ...
        Exception e = expectThrows(IllegalArgumentException.class,
                () -> new GrokProcessor(randomAlphaOfLength(10), null, Collections.singletonMap("ONE", "1"),
                        Collections.singletonList("%{NOTONE:not_one}"), fieldName, false, false, MatcherWatchdog.noop()));
    ...
    public void testTraceWithOnePattern() throws Exception {
        ...
        Map<String, String> patternBank = new HashMap<>();
        patternBank.put("ONE", "1");
        GrokProcessor processor = new GrokProcessor(randomAlphaOfLength(10), null, patternBank, Arrays.asList("%{ONE:one}"), fieldName,
                true, false, MatcherWatchdog.noop());
        processor.execute(doc);
    ...
    public void testFirstWinNamedCapture() throws Exception {
        ...
        GrokProcessor processor =
                new GrokProcessor(randomAlphaOfLength(10), null, patternBank, Collections.singletonList("%{ONETWO:first}%{ONETWO:first}"),
                        fieldName, randomBoolean(), randomBoolean(), MatcherWatchdog.noop());
        processor.execute(doc);
        assertThat(doc.getFieldValue("first", String.class), equalTo("1"));
    }
    ...
    public void testUnmatchedNamesNotIncludedInDocument() throws Exception {
        ...
        patternBank.put("THREE", "3");
        GrokProcessor processor =
                new GrokProcessor(randomAlphaOfLength(10), null, patternBank, Collections.singletonList("%{ONETWO:first}|%{THREE:second}"),
                        fieldName, randomBoolean(), randomBoolean(), MatcherWatchdog.noop());
        processor.execute(doc);
