...
import org.codelibs.fesen.ingest.IngestTestPlugin;
...
public class CancelTests extends ReindexTestCase {
    ...
    private void testCancel(String action, AbstractBulkByScrollRequestBuilder<?, ?> builder, CancelAssertion assertion,
        ...
        indexRandom(true, false, true, IntStream.range(0, numDocs)
                .mapToObj(i -> client().prepareIndex(INDEX, TYPE, String.valueOf(i)).setSource("n", i)).collect(Collectors.toList()));
        ...
        assertBusy(() -> assertTrue("updates blocked", ALLOWED_OPERATIONS.hasQueuedThreads() && ALLOWED_OPERATIONS.availablePermits() == 0),
        ...
        if (builder.request().getSlices() > 1) {
            ...
            ListTasksResponse sliceList =
                    client().admin().cluster().prepareListTasks().setParentTaskId(mainTask.getTaskId()).setDetailed(true).get();
        ...
        try {
            ...
            String tasks =
                    client().admin().cluster().prepareListTasks().setParentTaskId(mainTask.getTaskId()).setDetailed(true).get().toString();
    ...
    public void testReindexCancelWithWorkers() throws Exception {
        testCancel(ReindexAction.NAME, reindex().source(INDEX).filter(QueryBuilders.matchAllQuery()).destination("dest", TYPE).setSlices(5),
                (response, total, modified) -> {
