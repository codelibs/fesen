...
import org.codelibs.fesen.test.PosixPermissionsResetter;
...
public class InstallPluginCommandTests extends ESTestCase {
    ...
    static void writePlugin(String name, Path structure, String... additionalProps) throws IOException {
        String[] properties = Stream.concat(
                Stream.of("description", "fake desc", "name", name, "version", "1.0", "elasticsearch.version", Version.CURRENT.toString(),
                        "java.version", System.getProperty("java.specification.version"), "classname", "FakePlugin"),
    ...
    void installPlugins(final List<String> pluginUrls, final Path home, final InstallPluginCommand command) throws Exception {
        ...
        command.execute(terminal, pluginUrls, false, false, env);
    }
    ...
    public void testTransaction() throws Exception {
        ...
        Path pluginDir = createPluginDir(temp);
        String pluginZip = createPluginUrl("fake", pluginDir);
        final FileNotFoundException e = expectThrows(FileNotFoundException.class,
    ...
    public void testInstallFailsIfPreviouslyRemovedPluginFailed() throws Exception {
        ...
        final String expected = String.format(Locale.ROOT,
                "found file [%s] from a failed attempt to remove the plugin [failed]; execute [fesen-plugin remove failed]", removing);
    ...
    public void testJarHell() throws Exception {
        ...
        IllegalStateException e =
                expectThrows(IllegalStateException.class, () -> installPlugin(pluginZip, environment.v1(), defaultCommand));
    ...
    public void testInstallXPack() throws IOException {
        ...
        runInstallXPackTest(Build.Flavor.OSS, UserException.class,
    ...
    private <T extends Exception> void runInstallXPackTest(final Build.Flavor flavor, final Class<T> clazz, final String expectedMessage)
        ...
        final T exception =
                expectThrows(clazz, () -> flavorCommand.execute(terminal, Collections.singletonList("x-pack"), false, false, environment));
    ...
    public void testPluginAlreadyInstalled() throws Exception {
        ...
        final UserException e =
                expectThrows(UserException.class, () -> installPlugin(pluginZip, env.v1(), randomFrom(skipJarHellCommand, defaultCommand)));
        assertThat(e.getMessage(), equalTo("plugin directory [" + env.v2().pluginsFile().resolve("fake") + "] already exists; "
    ...
    private void installPlugin(MockTerminal terminal, boolean isBatch) throws Exception {
        ...
        skipJarHellCommand.execute(terminal, Collections.singletonList(pluginZip), isBatch, false, env.v2());
    }

    void assertInstallPluginFromUrl(final String pluginId, final String name, final String url, final String stagingHash,
            final boolean isSnapshot, final String shaExtension, final Function<byte[], String> shaCalculator, final PGPSecretKey secretKey,
    ...
    public void assertInstallPluginFromUrl(final String pluginId, final String name, final String url, final String stagingHash,
    ...
    public void testOfficialPluginSnapshot() throws Exception {
        String url = String.format(Locale.ROOT,
                "https://snapshots.elastic.co/%s-abc123/downloads/elasticsearch-plugins/analysis-icu/analysis-icu-%s.zip", Version.CURRENT,
    ...
    public void testInstallReleaseBuildOfPluginOnSnapshotBuild() {
        ...
        assertThat(e, hasToString(containsString("attempted to install release build of official plugin on snapshot build of Fesen")));
    }
    ...
    public void testOfficialPlatformPluginSnapshot() throws Exception {
        String url = String.format(Locale.ROOT,
                ...
                Version.CURRENT, Platforms.PLATFORM_NAME, Build.CURRENT.getQualifiedVersion());
        assertInstallPluginFromUrl("analysis-icu", "analysis-icu", url, "abc123", true);
    }
    ...
    public void testMavenChecksumWithoutFilename() throws Exception {
        ...
        assertInstallPluginFromUrl("mygroup:myplugin:1.0.0", "myplugin", url, null, false, ".sha512", checksum(digest), null,
                (b, p) -> null);
    }
    ...
    public void testOfficialChecksumWithoutFilename() throws Exception {
        String url = "https://artifacts.elastic.co/downloads/elasticsearch-plugins/analysis-icu/analysis-icu-"
                + Build.CURRENT.getQualifiedVersion() + ".zip";
        MessageDigest digest = MessageDigest.getInstance("SHA-512");
        UserException e = expectThrows(UserException.class, () -> assertInstallPluginFromUrl("analysis-icu", "analysis-icu", url, null,
                false, ".sha512", checksum(digest), null, (b, p) -> null));
    ...
    public void testOfficialShaMissing() throws Exception {
        ...
        MessageDigest digest = MessageDigest.getInstance("SHA-1");
        UserException e = expectThrows(UserException.class, () -> assertInstallPluginFromUrl("analysis-icu", "analysis-icu", url, null,
                false, ".sha1", checksum(digest), null, (b, p) -> null));
        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
    ...
    public void testMavenShaMissing() throws Exception {
        String url = "https://repo1.maven.org/maven2/mygroup/myplugin/1.0.0/myplugin-1.0.0.zip";
        UserException e = expectThrows(UserException.class, () -> assertInstallPluginFromUrl("mygroup:myplugin:1.0.0", "myplugin", url,
                null, false, ".dne", bytes -> null, null, (b, p) -> null));
        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
    ...
    public void testInvalidShaFileMismatchFilename() throws Exception {
        ...
        UserException e = expectThrows(UserException.class,
                () -> assertInstallPluginFromUrl("analysis-icu", "analysis-icu", url, null, false, ".sha512",
                        checksumAndString(digest, "  repository-s3-" + Build.CURRENT.getQualifiedVersion() + ".zip"), null,
                        (b, p) -> null));
        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
    ...
    public void testSha512Mismatch() throws Exception {
        String url = "https://artifacts.elastic.co/downloads/elasticsearch-plugins/analysis-icu/analysis-icu-"
                + Build.CURRENT.getQualifiedVersion() + ".zip";
        UserException e = expectThrows(UserException.class, () -> assertInstallPluginFromUrl("analysis-icu", "analysis-icu", url, null,
                false, ".sha512", bytes -> "foobar  analysis-icu-" + Build.CURRENT.getQualifiedVersion() + ".zip", null, (b, p) -> null));
        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
    ...
    public void testSha1Mismatch() throws Exception {
        String url = "https://repo1.maven.org/maven2/mygroup/myplugin/1.0.0/myplugin-1.0.0.zip";
        UserException e = expectThrows(UserException.class, () -> assertInstallPluginFromUrl("mygroup:myplugin:1.0.0", "myplugin", url,
                null, false, ".sha1", bytes -> "foobar", null, (b, p) -> null));
        assertEquals(ExitCodes.IO_ERROR, e.exitCode);
    ...
    public void testPublicKeyIdMismatchToExpectedPublicKeyId() throws Exception {
        ...
        final String url = "https://artifacts.elastic.co/downloads/elasticsearch-plugins/analysis-icu/" + icu + "-"
                + Build.CURRENT.getQualifiedVersion() + ".zip";
        final MessageDigest digest = MessageDigest.getInstance("SHA-512");
        /*
        ...
        final IllegalStateException e = expectThrows(IllegalStateException.class, () -> assertInstallPluginFromUrl(icu, icu, url, null,
                false, ".sha512", checksumAndFilename(digest, url), verifyingKey, signature));
    ...
    public void testFailedSignatureVerification() throws Exception {
        ...
        final IllegalStateException e = expectThrows(IllegalStateException.class, () -> assertInstallPluginFromUrl(icu, icu, url, null,
                false, ".sha512", checksumAndFilename(digest, url), newSecretKey(), signature));
    ...
    public PGPSecretKey newSecretKey() throws NoSuchAlgorithmException, NoSuchProviderException, PGPException {
        ...
        return new PGPSecretKey(PGPSignature.DEFAULT_CERTIFICATION, pkp, "example@example.com", sha1Calc, null, null,
    ...
    private String signature(final byte[] bytes, final PGPSecretKey secretKey) {
        try {
            final PGPPrivateKey privateKey =
                    secretKey.extractPrivateKey(new JcePBESecretKeyDecryptorBuilder(new JcaPGPDigestCalculatorProviderBuilder().build())
                            .build("passphrase".toCharArray()));
