...
call "%~dp0fesen-env.bat" %NOJAVA% || exit /b 1

set EXECUTABLE=%ES_HOME%\bin\fesen-service-x64.exe
if "%SERVICE_ID%" == "" set SERVICE_ID=fesen-service-x64
set ARCH=64-bit

if EXIST "%EXECUTABLE%" goto okExe
echo fesen-service-x64.exe was not found...
exit /B 1

:okExe
set ES_VERSION=7.10.2
...
:displayUsage
echo.
echo Usage: fesen-service.bat install^|remove^|start^|stop^|manager [SERVICE_ID]
goto:eof
...
:doManagment
set EXECUTABLE_MGR=%ES_HOME%\bin\fesen-service-mgr
...
if not defined ES_TMPDIR (
  for /f "tokens=* usebackq" %%a in (`CALL %JAVA% -cp "!ES_CLASSPATH!" "org.codelibs.fesen.tools.launchers.TempDirectory"`) do set ES_TMPDIR=%%a
)

rem The JVM options parser produces the final JVM options to start
rem Fesen. It does this by incorporating JVM options in the following
rem way:
...
@setlocal
for /F "usebackq delims=" %%a in (`CALL %JAVA% -cp "!ES_CLASSPATH!" "org.codelibs.fesen.tools.launchers.JvmOptionsParser" "!ES_PATH_CONF!" ^|^| echo jvm_options_parser_failed`) do set ES_JAVA_OPTS=%%a
...
set ES_PARAMS=-Dfesen;-Des.path.home="%ES_HOME%";-Des.path.conf="%ES_PATH_CONF%";-Des.distribution.flavor="%ES_DISTRIBUTION_FLAVOR%";-Des.distribution.type="%ES_DISTRIBUTION_TYPE%";-Des.bundled_jdk="%ES_BUNDLED_JDK%"

if "%ES_START_TYPE%" == "" set ES_START_TYPE=manual
if "%ES_STOP_TIMEOUT%" == "" set ES_STOP_TIMEOUT=0

if "%SERVICE_DISPLAY_NAME%" == "" set SERVICE_DISPLAY_NAME=Fesen %ES_VERSION% (%SERVICE_ID%)
if "%SERVICE_DESCRIPTION%" == "" set SERVICE_DESCRIPTION=Fesen %ES_VERSION% Windows Service - https://fess.codelibs.org
...
"%EXECUTABLE%" //IS//%SERVICE_ID% --Startup %ES_START_TYPE% --StopTimeout %ES_STOP_TIMEOUT% --StartClass org.codelibs.fesen.bootstrap.Fesen --StartMethod main ++StartParams --quiet --StopClass org.codelibs.fesen.bootstrap.Fesen --StopMethod close --Classpath "%ES_CLASSPATH%" --JvmMs %JVM_MS% --JvmMx %JVM_MX% --JvmSs %JVM_SS% --JvmOptions %OTHER_JAVA_OPTS% ++JvmOptions %ES_PARAMS% %LOG_OPTS% --PidFile "%SERVICE_ID%.pid" --DisplayName "%SERVICE_DISPLAY_NAME%" --Description "%SERVICE_DESCRIPTION%" --Jvm "%JAVA_HOME%%JVM_DLL%" --StartMode jvm --StopMode jvm --StartPath "%ES_HOME%" %SERVICE_PARAMS% ++Environment HOSTNAME="%%COMPUTERNAME%%"
