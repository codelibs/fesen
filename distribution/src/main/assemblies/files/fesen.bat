...
CALL "%~dp0fesen-env.bat" || exit /b 1
IF ERRORLEVEL 1 (
...
IF "%checkpassword%"=="Y" (
  CALL "%~dp0fesen-keystore.bat" has-passwd --silent
  IF !ERRORLEVEL! EQU 0 (
    SET /P KEYSTORE_PASSWORD=Fesen keystore password:
    IF !ERRORLEVEL! NEQ 0 (
...
ECHO.!KEYSTORE_PASSWORD!| %JAVA% %ES_JAVA_OPTS% -Delasticsearch ^
  ...
  -cp "%ES_CLASSPATH%" "org.codelibs.fesen.bootstrap.Elasticsearch" !newparams!
